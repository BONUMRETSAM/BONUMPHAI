...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      const needsFormal = isEducational || isMathProblem || this.isSeriousQuestion(prompt);
      ...
      if (imageUrl) {
        ...
        // Image analysis is always formal/educational
        needsFormal = true;
      } else {
  ...
  async handleRealtimeQuestion(senderId, prompt, token) {
    ...
    try {
      ...
      if (data && data.answer) {
        ...
        // Check if needs formal or conversational
        if (this.isSeriousQuestion(prompt) || this.isEducationalRequest(prompt)) {
          cleanResponse = this.makeFormal(cleanResponse);
        } else {
          cleanResponse = this.makeConversational(cleanResponse);
        }
  ...
  async callGeminiAPI(prompt, imageUrl) {
    try {
      ...
      // Gemini responses are always formal/educational for image analysis
      processed = this.makeFormal(processed);
      ...
      let cleaned = this.cleanResponse(response || 'Ayy sorry, di ko ma-analyze yung image. Try mo ulit.');
      return this.makeFormal(cleaned);
    }
  },
...
USER QUESTION: ${userPrompt || 'Analyze this image and provide a comprehensive response.'}`;
  ...
  buildFinalPrompt(prompt, previousResponse, previousPrompt, isReply, wantsDetailed, needsFormal) {
    ...
    if (previousResponse) {
      const shortContext = previousResponse.substring(0, 300) + (previousResponse.length > 300 ? '...' : '');

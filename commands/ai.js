...
module.exports = {
  ...
  buildGeminiPrompt(userPrompt, language = 'english') {
    ...
    if (language === 'tagalog' || language === 'bisaya' || language === 'cebuano') {
      ...
      prompt += `   - Sagutin ang BAWAT tanong\n`;
      prompt += `   - Panatilihin ang format ng activity sheet\n\n`;
      ...
      prompt += `   - Ilarawan sa 1 pangungusap\n`;
      prompt += `   - Kung may malalim na kahulugan: Ipaliwanag sa 1-2 pangungusap\n\n`;
      ...
      prompt += `7. DEEP MEANING / SYMBOLIC ART:\n`;
      prompt += `   - Ipaliwanag ang kahulugan nang DIREKTA at MAIKLI\n\n`;
      prompt += `MAHALAGANG PANUNTUNAN:\n`;
      prompt += `- Tukuyin MUNA ang klase ng image bago sumagot\n`;
      prompt += `- DIREKTA sa mga sagot, WALANG intro\n`;
      prompt += `- Kung activity sheet: Sagutin LAHAT ng tanong\n`;
      prompt += `- Kung casual image: Maikli lang\n`;
      ...
      prompt += `   - Maintain the activity sheet format\n\n`;
      prompt += `2. MATH PROBLEM / EQUATION / GRAPH:\n`;
      ...
      prompt += `7. DEEP MEANING / SYMBOLIC ART:\n`;
      prompt += `   - Explain the meaning DIRECTLY and BRIEFLY\n\n`;
      prompt += `IMPORTANT RULES:\n`;
      prompt += `- Identify the image type FIRST\n`;
      prompt += `- DIRECTLY provide answers, NO intro\n`;
      prompt += `- If activity sheet: Answer ALL questions\n`;
      prompt += `- If casual image: Keep description brief\n`;

...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // Use corrected prompt for processing, but keep original for history
  ...
  correctTypos(prompt) {
    ...
    // Check for whole-word replacements first
    for (const [typo, correct] of Object.entries(typoMap)) {
      if (lower.includes(typo)) {
        // Replace with preserved case
  ...
  async callGeminiAPI(prompt, imageUrl, detectedLanguage = 'english') {
    try {
      // First, detect the language of the image content
...
USER QUESTION: ${userPrompt || 'Analyze this image'}`;
  ...
  isNewTopic(prompt, previousPrompt, originalPrompt) {
    ...
    // If prompt is a typo of a follow-up phrase, it's NOT a new topic
    const corrected = this.correctTypos(prompt);

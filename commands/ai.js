...
module.exports = {
  ...
  solveLogicPuzzleDirectly(prompt) {
    // Extract sequence of letters
    const letterMatch = prompt.match(/[A-Z]\s*[A-Z]\s*[A-Z]\s*[A-Z]/g);
    if (!letterMatch) return null;
    
    const sequence = letterMatch[0].split(/\s+/);
    if (sequence.length < 2) return null;
    
    // Check if it's an alphabet sequence
    ...
    if (restrictionMatch) {
      ...
      if (nextLetter === forbidden) {
        // Find alternative that implies the forbidden letter
    ...
    // No restriction or restriction doesn't apply
  ...
  solveMoneyProblemDirectly(prompt) {
    ...
    // If still not found, use first two numbers
    if (!amount || !payment) {
      // Find the smaller and larger numbers
      const sorted = [...numValues].sort((a, b) => a - b);
      
      // Check if there's a "coin" or "pesos" indicator
      let hasCoin = false;
      let hasPesos = false;
      for (const num of numValues) {
        ...
        if (after.includes('coin') || after.includes('barya')) hasCoin = true;
        if (after.includes('pesos') || after.includes('piso')) hasPesos = true;
      }
      
      if (hasCoin || hasPesos) {
        // The amount is the one with coin/pesos indicator
        for (const num of numValues) {
          const str = num.toString();
          const index = prompt.indexOf(str);
          const after = prompt.substring(index + str.length, index + str.length + 10);
          if (after.includes('coin') || after.includes('barya') || after.includes('pesos') || after.includes('piso')) {
            if (!amount) amount = num;
            else if (num !== amount) payment = num;
          }
        }
      }
      
      // Final fallback: first is amount, second is payment
      if (!amount || !payment) {
        amount = sorted[0];
        payment = sorted[1];
      }
    ...
    // Ensure amount is the smaller (what you need to pay)
    if (amount > payment) {
      [amount, payment] = [payment, amount];
    }
    
    // If payment is less than amount, user doesn't have enough
    if (payment < amount) {
  ...
  formatMoneyAnswer(amount, payment) {
    ...
    if (change > 0) {
      const bestOption = this.getBestPaymentOption(amount);
      if (bestOption) {

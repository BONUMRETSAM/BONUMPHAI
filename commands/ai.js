...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // ===== SOLVE MATH PROBLEM =====
      const mathAnswer = this.solveMathProblem(prompt);
      if (mathAnswer) {
        await this.sendChunks(senderId, mathAnswer, token);
        conversationHistory[senderId] = {
          lastPrompt: prompt,
          lastResponse: mathAnswer,
          timestamp: Date.now()
        };
      ...
      // ===== SOLVE LOGIC PUZZLE =====
      const logicAnswer = this.solveLogicPuzzle(prompt);
      if (logicAnswer) {
        await this.sendChunks(senderId, logicAnswer, token);
        conversationHistory[senderId] = {
          lastPrompt: prompt,
          lastResponse: logicAnswer,
          timestamp: Date.now()
        };
        this.cleanOldHistory();
        return;
      ...
      // ===== SOLVE MONEY PROBLEM =====
      const moneyAnswer = this.solveMoneyProblem(prompt);
      if (moneyAnswer) {
        await this.sendChunks(senderId, moneyAnswer, token);
        conversationHistory[senderId] = {
          lastPrompt: prompt,
          lastResponse: moneyAnswer,
          timestamp: Date.now()
        };
        this.cleanOldHistory();
        return;
      ...
      // ===== SOLVE COMPARISON LOGIC =====
      const comparisonAnswer = this.solveComparisonLogic(prompt);
      if (comparisonAnswer) {
        await this.sendChunks(senderId, comparisonAnswer, token);
        conversationHistory[senderId] = {
          lastPrompt: prompt,
          lastResponse: comparisonAnswer,
          timestamp: Date.now()
        };
        this.cleanOldHistory();
        return;
      ...
      // ===== SHORTEN CASUAL CONVERSATION RESPONSES =====
      if (this.isCasualConversation(prompt) || this.isSimpleQuestion(prompt)) {
  ...
  isCasualConversation(prompt) {
    const casualPhrases = [
      // Greetings
      'kamusta', 'musta', 'hello', 'hi', 'hey', 'oy', 'ui',
      'good morning', 'good afternoon', 'good evening', 'good day',
      'magandang umaga', 'magandang tanghali', 'magandang hapon', 'magandang gabi',
      
      // Responses
      'salamat', 'thank you', 'thanks', 'tnx', 'ty',
      'nice', 'okay', 'sige', 'cge', 'ge', 'okei',
      'ahhh', 'haha', 'hehe', 'lol', 'lmao', 'hahaha',
      'talaga', 'seryoso', 'grabe', 'grabeha',
      'wow', 'shet', 'gagi', 'lala', 'hala',
      'ganun', 'ganyan', 'ganito', 'gayon',
      'gets', 'nagets', 'naintindihan',
      'tama', 'sakto', 'wasto', 'tumpak',
      'oo', 'opo', 'hindi', 'dili', 'no', 'yes',
      'so', 'eh', 'ay', 'ah', 'oh', 'ha',
      'hay', 'sus', 'ayoko', 'gusto', 'ayaw',
      'sige na', 'cge na', 'ge na',
      
      // Questions
      'ano gawa mo', 'what are you doing', 'musta ka',
      'how are you', 'kumusta ka', 'musta na',
      'ano na', 'what\'s up', 'anyare',
      'saan ka', 'where are you', 'nasaan ka',
      'ano yan', 'what\'s that', 'bakit', 'why',
      'paano', 'how', 'sino', 'who', 'kailan', 'when',
      'saan', 'where', 'alin', 'which', 'magkano', 'how much',
      
      // Tagalog casual
      'pre', 'bro', 'par', 'tol', 'men', 'dude',
      'ate', 'kuya', 'manong', 'manang',
      'ganyan pala', 'ganun pala', 'ganito pala',
      'ah ganun', 'ah ganon', 'ay ganun',
      
      // Bisaya casual
      'unsa', 'ngano', 'giunsa', 'kanus-a', 'asa', 'kinsa', 'hain', 'pila',
      'mao', 'mao na', 'mao ni', 'mao to', 'mao diay',
      'sama', 'ingon ana', 'ingon ani',
      'lagi', 'jud', 'gyud', 'ba', 'kaha',
      'payts', 'payts ra', 'ok ra',
      'gimingaw', 'gikapoy', 'gutum', 'giuhaw',
      
      // English casual
      'cool', 'awesome', 'amazing', 'great', 'good',
      'bad', 'sad', 'happy', 'excited', 'tired',
      'ok', 'fine', 'alright', 'sure', 'definitely',
      'maybe', 'perhaps', 'probably', 'hopefully',
      'honestly', 'seriously', 'literally', 'basically'
    ];
    ...
    if (words <= 5 && casualPhrases.some(p => lower.includes(p))) {
      return true;
    }
    
    // Very short phrases
    if (lower.length < 20 && !lower.includes('?') && !lower.includes('ano') && !lower.includes('what')) {
      return true;
    }
    
    // Single word responses
    if (words === 1 && casualPhrases.some(p => lower === p)) {
      return true;
    }
  ...
  isSimpleQuestion(prompt) {
    ...
    if (lower.length < 25 && lower.includes('?')) {
      const complexKeywords = ['explain', 'solve', 'compute', 'calculate', 'define', 'meaning', 'ibig sabihin'];
    ...
    const simpleQuestions = [
      ...
      'ano yan', 'what\'s that', 'bakit', 'why',
      'paano', 'how', 'sino', 'who', 'kailan', 'when',
      'saan', 'where', 'alin', 'which',
      'magkano', 'how much', 'pila',
      'unsa', 'ngano', 'giunsa', 'kanus-a', 'asa', 'kinsa', 'hain',
      'musta na ka', 'kumusta na', 'kumusta ka na',
      'okay ka lang', 'are you okay', 'ok ka lang',
      'busy ka', 'are you busy', 'free ka',
      'tulog ka na', 'are you sleeping', 'gising ka pa',
      'kain ka na', 'have you eaten', 'kumain ka na'
    ];
    
    if (simpleQuestions.some(q => lower.includes(q))) {
      return true;
    }
  ...
  // ==================== MATH PROBLEM SOLVER ====================
  solveMathProblem(prompt) {
    const lower = prompt.toLowerCase();
    if (!lower.includes('x') && !lower.includes('y') && !lower.includes('solve') && !lower.includes('find') && !lower.includes('value')) {
      return null;
    }

    const equations = prompt.match(/\w+\s*[+*/-]\s*\w+\s*=\s*\d+/g);
    if (!equations || equations.length < 2) return null;

    if (prompt.includes('x^3') && prompt.includes('y^3') && prompt.includes('x+y')) {
      const numbers = prompt.match(/\d+/g);
      if (!numbers || numbers.length < 2) return null;
      
      const sum = parseInt(numbers[0]);
      const sumCubes = parseInt(numbers[1]);
      
      const xy = (sum * sum - sumCubes / sum) / 3;
      
      if (Number.isInteger(xy) && xy > 0) {
        const options = prompt.match(/[A-D]\.\s*(\d+)/g);
        if (options) {
          for (const opt of options) {
            const val = parseInt(opt.match(/\d+/)[0]);
            if (val === xy) {
              return `Sagot: ${opt}\n\nPaliwanag:\nGiven:\n- x + y = ${sum}\n- x³ + y³ = ${sumCubes}\n\nGamitin ang formula:\n- x³ + y³ = (x + y)[(x + y)² - 3xy]\n\nSubstitute:\n- ${sumCubes} = ${sum}[${sum * sum} - 3xy]\n- ${sumCubes} = ${sum * sum * sum} - ${sum * 3}xy\n- ${sum * 3}xy = ${sum * sum * sum - sumCubes}\n- xy = ${xy}\n\nKaya ang sagot ay ${opt}.`;
            }
          }
        }
        
        return `Sagot: ${xy}\n\nPaliwanag:\nGiven:\n- x + y = ${sum}\n- x³ + y³ = ${sumCubes}\n\nGamitin ang formula:\n- x³ + y³ = (x + y)[(x + y)² - 3xy]\n\nSubstitute:\n- ${sumCubes} = ${sum}[${sum * sum} - 3xy]\n- ${sumCubes} = ${sum * sum * sum} - ${sum * 3}xy\n- ${sum * 3}xy = ${sum * sum * sum - sumCubes}\n- xy = ${xy}\n\nKaya ang value ng xy ay ${xy}.`;
      }
    }
  ...
  // ==================== COMPARISON LOGIC SOLVER (100% DYNAMIC) ====================
  solveComparisonLogic(prompt) {
    const lower = prompt.toLowerCase();
    
    // Check if it's a comparison logic question
    const comparisonKeywords = [
      'anong meron sa isa', 'what has one', 'ano ang meron sa isa',
      'anong meron', 'wala sa tatlo', 'meron sa isa',
      'naiiba', 'different', 'pinakaiba',
      'anong wala', 'what is missing', 'what\'s different'
    ];
    
    if (!comparisonKeywords.some(k => lower.includes(k))) {
      return null;
    }

    // Extract items dynamically
    const items = prompt.match(/[A-Z][a-z]+/g);
    if (!items || items.length < 3) return null;

    // Find unique item dynamically
    const result = this.findUniqueItem(items);
    if (result) {
      return `Sagot: ${result.item}\n\nPaliwanag:\n${result.reason}`;
    }

    return null;
  },

  findUniqueItem(items) {
    // Analyze each item's characteristics
    const analysis = items.map(item => ({
      name: item,
      length: item.length,
      firstLetter: item[0],
      lastLetter: item[item.length - 1],
      vowels: (item.match(/[AEIOU]/gi) || []).length,
      consonants: (item.match(/[BCDFGHJKLMNPQRSTVWXYZ]/gi) || []).length,
      letters: item.split(''),
      category: this.detectCategoryDynamic(item)
    }));

    // Check unique first letter
    const firstLetters = analysis.map(a => a.firstLetter);
    const uniqueFirst = firstLetters.find((letter, idx) => 
      firstLetters.indexOf(letter) === firstLetters.lastIndexOf(letter)
    );
    if (uniqueFirst) {
      const item = analysis.find(a => a.firstLetter === uniqueFirst);
      return {
        item: item.name,
        reason: `Ang ${item.name} ang tanging may letrang "${uniqueFirst}" sa simula.`
      };
    }

    // Check unique last letter
    const lastLetters = analysis.map(a => a.lastLetter);
    const uniqueLast = lastLetters.find((letter, idx) => 
      lastLetters.indexOf(letter) === lastLetters.lastIndexOf(letter)
    );
    if (uniqueLast) {
      const item = analysis.find(a => a.lastLetter === uniqueLast);
      return {
        item: item.name,
        reason: `Ang ${item.name} ang tanging may letrang "${uniqueLast}" sa dulo.`
      };
    }

    // Check unique length
    const lengths = analysis.map(a => a.length);
    const uniqueLength = lengths.find((len, idx) => 
      lengths.indexOf(len) === lengths.lastIndexOf(len)
    );
    if (uniqueLength) {
      const item = analysis.find(a => a.length === uniqueLength);
      return {
        item: item.name,
        reason: `Ang ${item.name} ang may ${uniqueLength} na letra, samantalang ang iba ay may ibang bilang.`
      };
    }

    // Check unique vowel count
    const vowels = analysis.map(a => a.vowels);
    const uniqueVowels = vowels.find((v, idx) => 
      vowels.indexOf(v) === vowels.lastIndexOf(v)
    );
    if (uniqueVowels) {
      const item = analysis.find(a => a.vowels === uniqueVowels);
      return {
        item: item.name,
        reason: `Ang ${item.name} ang may ${uniqueVowels} na patinig (vowels).`
      };
    }

    // Check unique consonant count
    const consonants = analysis.map(a => a.consonants);
    const uniqueConsonant = consonants.find((c, idx) => 
      consonants.indexOf(c) === consonants.lastIndexOf(c)
    );
    if (uniqueConsonant) {
      const item = analysis.find(a => a.consonants === uniqueConsonant);
      return {
        item: item.name,
        reason: `Ang ${item.name} ang may ${uniqueConsonant} na katinig (consonants).`
      };
    }

    // Check unique category
    const categories = analysis.map(a => a.category);
    const uniqueCategory = categories.find((cat, idx) => 
      categories.indexOf(cat) === categories.lastIndexOf(cat)
    );
    if (uniqueCategory) {
      const item = analysis.find(a => a.category === uniqueCategory);
      return {
        item: item.name,
        reason: `Ang ${item.name} ang tanging ${uniqueCategory} sa grupo.`
      };
    }

    // Find most different item based on multiple factors
    let scores = items.map((item, idx) => {
      let score = 0;
      const current = analysis[idx];
      
      for (let j = 0; j < analysis.length; j++) {
        if (j === idx) continue;
        const other = analysis[j];
        
        if (current.length !== other.length) score++;
        if (current.firstLetter !== other.firstLetter) score++;
        if (current.vowels !== other.vowels) score++;
        if (current.consonants !== other.consonants) score++;
        if (current.category !== other.category) score++;
        if (current.lastLetter !== other.lastLetter) score++;
      }
    ...
    if (winner.score > 0) {
      return {
        item: winner.item,
        reason: `Ang ${winner.item} ang may pinakamaraming pagkakaiba sa iba pang mga salita sa grupo.`
      };
    }
    ...
    return {
      item: items[0],
      reason: `Ang ${items[0]} ang naiiba sa grupo.`
    };
  },

  detectCategoryDynamic(item) {
    const lower = item.toLowerCase();
    
    // Common word patterns
    if (lower.endsWith('an') || lower.endsWith('on') || lower.endsWith('in')) {
      return 'lalagyan';
    }
    if (lower.endsWith('or') || lower.endsWith('er') || lower.endsWith('ar')) {
      return 'kagamitan';
    }
  ...
  // ==================== LOGIC PUZZLE SOLVER ====================
  solveLogicPuzzle(prompt) {
    const lower = prompt.toLowerCase();
    if (!lower.includes('sequence') && !lower.includes('alphabet') && !lower.includes('letter')) {
      return null;
    }

    const letters = prompt.match(/[A-Z]\s*[A-Z]\s*[A-Z]\s*[A-Z]/g);
    if (!letters) return null;
    
    const sequence = letters[0].split(/\s+/);
    if (sequence.length < 2) return null;
    
    const alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
    let startIndex = alphabet.indexOf(sequence[0]);
    if (startIndex === -1) return null;
    
    for (let i = 0; i < sequence.length; i++) {
      if (sequence[i] !== alphabet[startIndex + i]) {
        return null;
      }
    }
    
    const nextLetter = alphabet[startIndex + sequence.length];
    if (!nextLetter) return null;
    
    const restrictionMatch = prompt.match(/without writing the letter\s*([A-Z])/i) || 
                            prompt.match(/bawal isulat ang\s*([A-Z])/i);
    
    if (restrictionMatch) {
      const forbidden = restrictionMatch[1].toUpperCase();
      
      if (nextLetter === forbidden) {
        const lastLetter = sequence[sequence.length - 1];
        const lastIndex = alphabet.indexOf(lastLetter);
        const forbiddenIndex = alphabet.indexOf(forbidden);
        
        for (let i = forbiddenIndex + 1; i < alphabet.length; i++) {
          const candidate = alphabet[i];
          const middleIndex = Math.floor((lastIndex + i) / 2);
          if (alphabet[middleIndex] === forbidden) {
            return this.formatLogicAnswer(sequence, nextLetter, candidate, forbidden);
          }
        }
        
        for (let i = forbiddenIndex - 1; i >= 0; i--) {
          const candidate = alphabet[i];
          const middleIndex = Math.floor((lastIndex + i) / 2);
          if (alphabet[middleIndex] === forbidden) {
            return this.formatLogicAnswer(sequence, nextLetter, candidate, forbidden);
        ...
        const alternative = alphabet[startIndex + sequence.length + 1];
        if (alternative) {
          return this.formatLogicAnswer(sequence, nextLetter, alternative, forbidden);
        }
      }
    }
    
    return this.formatLogicAnswer(sequence, nextLetter);
  },

  formatLogicAnswer(sequence, nextLetter, alternative = null, forbidden = null) {
    let response = '';
    
    if (forbidden && alternative) {
      response = `Sagot: ${alternative}\n\n`;
      response += `Paliwanag:\n`;
      response += `Ang sequence ay: ${sequence.join(' ')} _\n`;
      response += `Ang dapat na susunod na letra ay ${nextLetter}.\n`;
      response += `Pero bawal isulat ang "${forbidden}".\n\n`;
      response += `Kaya ang sagot ay ${alternative}.\n`;
      
      const alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
      const lastLetter = sequence[sequence.length - 1];
      const lastIndex = alphabet.indexOf(lastLetter);
      const altIndex = alphabet.indexOf(alternative);
      const middleIndex = Math.floor((lastIndex + altIndex) / 2);
      const middleLetter = alphabet[middleIndex];
      
      response += `Dahil ang ${lastLetter} (${lastIndex + 1}) at ${alternative} (${altIndex + 1}) ay may ${middleLetter} (${middleIndex + 1}) sa gitna.\n\n`;
      response += `Kaya sa pamamagitan ng paglalagay ng ${alternative}, ang implicit na sequence ay ${sequence.join(' ')} ${nextLetter}.`;
    } else {
      response = `Sagot: ${nextLetter}\n\n`;
      response += `Paliwanag:\n`;
      response += `Ang sequence ay: ${sequence.join(' ')} _\n`;
      response += `Ang pattern ay alphabet order.\n`;
      response += `Kaya ang susunod na elemento ay ${nextLetter}.`;
    }
  ...
  solveMoneyProblem(prompt) {
    const lower = prompt.toLowerCase();
    
    const moneyKeywords = ['sukli', 'change', 'pera', 'money', 'bayad', 'payment', 'bibilhin', 'gastos', 'presyo', 'pesos', 'piso', 'coins', 'barya'];
    if (!moneyKeywords.some(k => lower.includes(k))) return null;
    ...
    const mayMatch = prompt.match(/may\s*(\d+)/i);
    if (mayMatch) amount = parseInt(mayMatch[1]);
    
    const bayadMatch = prompt.match(/bayaran\s*(\d+)/i) || prompt.match(/bayad\s*(\d+)/i);
    if (bayadMatch) payment = parseInt(bayadMatch[1]);
    
    const meronMatch = prompt.match(/meron akong\s*(\d+)/i) || prompt.match(/naa koy\s*(\d+)/i);
    if (meronMatch && !amount) amount = parseInt(meronMatch[1]);
    ...
    if (payment < amount) {
      return `Kulang ang pera mo ng ${amount - payment} pesos.\n\nKailangan mo ng ${amount} pesos, pero ${payment} pesos lang ang meron ka.\nKulang ka ng ${amount - payment} pesos.`;
    }
    
    return this.formatMoneyAnswer(amount, payment);
  },

  formatMoneyAnswer(amount, payment) {
    const change = payment - amount;
    
    let response = `Sukli mo: ${change} pesos\n\n`;
    response += `Mga Praktikal na Paraan:\n\n`;
    
    response += `- Magbayad ng eksaktong ${amount} pesos - WALANG SUKLI (Pinaka-praktikal)\n`;
    
    const exactCombo = this.getExactPaymentCombo(amount);
    if (exactCombo) {
      response += `- Magbayad ng ${exactCombo} - WALANG SUKLI\n`;
    }
    ...
    if (nextBill && nextBill !== amount) {
      const change2 = nextBill - amount;
      response += `- Magbayad ng ${nextBill} pesos - SUKLI ${change2} pesos\n`;
    }
    
    if (payment > amount && payment !== nextBill) {
      response += `- Magbayad ng ${payment} pesos - SUKLI ${change} pesos (ito ang ginawa mo)\n`;
    }
    
    if (change > 0 && change <= 100) {
      const coins = this.getCoinCombination(change);
      if (coins) {
        response += `\nPwedeng gamitin na barya: ${coins}`;
      }
    }
    
    if (change > 0) {
      response += `\n\nPinaka-praktikal: Magbayad ng eksaktong ${amount} pesos - walang sukli.`;
    }
    
    return response;
  },

  getExactPaymentCombo(amount) {
    const denominations = [1000, 500, 200, 100, 50, 20, 10, 5, 1];
    ...
    if (combo.length === 0 || remaining > 0) return null;
    
    const counts = {};
    combo.forEach(c => counts[c] = (counts[c] || 0) + 1);
    
    return Object.entries(counts)
      .map(([coin, count]) => {
        const name = coin >= 20 ? `${coin} pesos` : `${coin} piso`;
        return count > 1 ? `${count} piraso ng ${name}` : `${name}`;
      })
      .join(' + ');
  },
  ...
  getCoinCombination(amount) {
    ...
    return Object.entries(counts)
      .map(([coin, count]) => `${count} piraso ng ${coin} piso`)
      .join(' + ');
  },
  ...
  buildFinalPrompt(prompt, previousResponse, previousPrompt, isReply, wantsDetailed) {
    ...
    const isCasual = this.isCasualConversation(prompt);
    const isSimpleQuestion = this.isSimpleQuestion(prompt);
    const isDefinitionRequest = prompt.toLowerCase().includes('ano ibig sabihin') || 
                                prompt.toLowerCase().includes('meaning') || 
                                prompt.toLowerCase().includes('define') ||
                                prompt.toLowerCase().includes('definition') ||
                                prompt.toLowerCase().includes('kahulugan') ||
                                prompt.toLowerCase().includes('ibig sabihin');

    // ===== CASUAL CONVERSATION (SHORT, NATURAL) =====
    if (isCasual && !isDefinitionRequest) {
      final = `The user is having a casual conversation.
      
RULES:
- Respond naturally and conversationally like a friend
- Keep response SHORT (1-3 sentences only)
- Be friendly, warm, and engaging
- DO NOT give definitions or translations unless specifically asked
- Use the same language as the user
- Be practical and accurate but keep it simple
- If the user just said a casual phrase, respond casually too

User said: ${prompt}`;

      if (previousResponse) {
        final += `\n\nPrevious: User said "${previousPrompt}", you responded with "${previousResponse}". Continue the conversation naturally.`;
      }
    ...
    if (isSimpleQuestion && !isDefinitionRequest) {
      final = `The user asked a simple question.
...
User question: ${prompt}`;
    ...
    // ===== COMPLEX QUESTIONS (DETAILED, PRACTICAL) =====
    if (previousResponse) {
    ...
    if (this.isTranslationRequest(prompt)) {
      ...
      final += `User wants to translate to ${lang}. Provide only the translation, no extra text.\n\n`;
    }

    // Check if it's a definition request
    if (isDefinitionRequest) {
      final += `User is asking for a definition or meaning. Provide a clear, concise explanation.\n\n`;
    }

    final += 'User question: ' + prompt + '\n\n';

    // ===== PRACTICAL INSTRUCTIONS BASED ON QUESTION TYPE =====
    if (this.isMoneyProblem(prompt)) {
      final += 'IMPORTANT: This is a MONEY problem.\n';
      final += '- Calculate exact change correctly\n';
      final += '- Give practical payment options\n';
      final += '- Suggest the best option\n\n';
    }

    if (this.isLogicPuzzle(prompt)) {
      final += 'IMPORTANT: This is a LOGIC puzzle.\n';
      final += '- Analyze the pattern step by step\n';
      final += '- Find the logical answer\n';
      final += '- Explain the reasoning clearly\n\n';
    }

    if (this.isComparisonLogic(prompt)) {
      final += 'IMPORTANT: This is a COMPARISON question.\n';
      final += '- Compare all items\n';
      final += '- Find what makes one different\n';
      final += '- Explain the reasoning clearly\n\n';
    }

    if (wantsDetailed) {
      final += 'Provide a DETAILED response with explanation.\n';
    } else {
      final += 'Provide a CONCISE but PRACTICAL response.\n';
    }

    final += '\nRULES:\n';
    final += '- Be accurate and practical\n';
    final += '- Use plain text only\n';
    final += '- Keep it conversational and natural\n';
    final += '- Don\'t over-explain simple things\n';
    final += '- Respond in the same language as the user\n';
    final += '- For money problems: ALWAYS show change + options\n';
    final += '- For logic puzzles: ALWAYS show reasoning\n';
    final += '- For casual conversations: Keep it short and friendly\n';

    return final;
  },

  // ===== DETECTION METHODS =====
  isMoneyProblem(prompt) {
    const lower = prompt.toLowerCase();
    const keywords = ['sukli', 'change', 'pera', 'money', 'bayad', 'payment', 'pesos', 'piso', 'coins', 'barya'];
    return keywords.some(k => lower.includes(k)) && /\d+/.test(prompt);
  },

  isLogicPuzzle(prompt) {
    const lower = prompt.toLowerCase();
    return (lower.includes('sequence') || lower.includes('alphabet') || lower.includes('letter')) && 
           /[A-Z]\s*[A-Z]\s*[A-Z]\s*[A-Z]/.test(prompt);
  },

  isComparisonLogic(prompt) {
    const lower = prompt.toLowerCase();
    const phrases = ['anong meron sa isa', 'what has one', 'anong meron', 'wala sa tatlo', 'naiiba', 'different'];
    return phrases.some(p => lower.includes(p)) && prompt.match(/[A-Z][a-z]+/g)?.length >= 3;
  },

  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      if (this.isShortenRequest(prompt)) {
        ...
        await sendMessage(senderId, { text: 'Creating a concise but comprehensive summary... Please wait.' }, token);
        
        const lang = this.getLanguageName(this.detectLanguage(prompt));
        const summaryPrompt = `Create a concise but comprehensive summary of the following text. The summary should be:

1. UNDERSTANDABLE - Use clear language that is easy to read
2. PRECISE - Include all key information, facts, and figures
3. ACCURATE - Do not change or invent any information
4. WELL-STRUCTURED - Use proper paragraph structure with smooth transitions
5. COMPLETE - Cover all important points without omitting critical details

The summary should be about 1/3 of the original length, but still comprehensive enough to understand the full context.
...
Respond in ${lang}.\n\nTEXT:\n${textToShorten}`;
  ...
  buildModificationPrompt(type, text, language) {
    ...
    switch(type) {
      case 'short':
      case 'concise':
        instruction = `Create a concise but comprehensive summary of the following text. The summary should be:
...
The summary should be about 1/3 of the original length, but still comprehensive enough to understand the full context.`;
      ...
      case 'paragraph':
        instruction = `Convert this text into a well-structured paragraph. Make it flow smoothly from one idea to the next. Use proper transitions and ensure coherence.`;
        break;
        
      case 'fluent':
        instruction = `Rewrite this text to make it more fluent and coherent. Fix any awkward phrasing, improve sentence flow, and ensure smooth transitions between ideas.`;
        break;
        
      case 'formal':
        instruction = `Rewrite this text in a formal tone. Use professional language, avoid contractions, and maintain a respectful and polished style suitable for academic or professional settings.`;
        break;
        
      case 'casual':
        instruction = `Rewrite this text in a casual, conversational tone. Make it sound like how a real person would speak in everyday conversation. Use natural language and contractions.`;
        break;
        
      case 'academic':
        instruction = `Rewrite this text in an academic tone. Use formal language, proper terminology, and maintain an objective and scholarly style suitable for academic writing.`;
        break;
        
      case 'professional':
        instruction = `Rewrite this text in a professional tone. Use business-appropriate language, be clear and concise, and maintain a polished and competent style.`;
        break;
        
      case 'simple':
        instruction = `Rewrite this text in simple, easy-to-understand language. Use plain English, avoid jargon, and make it accessible to a general audience.`;
        break;
        
      case 'detailed':
        instruction = `Expand and elaborate on this text. Add more details, examples, and explanations to make it more comprehensive and thorough.`;
        break;
        
      case 'polish':
        instruction = `Polish and refine this text. Fix any grammatical errors, improve word choice, and enhance the overall quality of the writing.`;
        break;
        
      case 'improve':
        instruction = `Improve the overall quality of this text. Enhance clarity, coherence, and readability. Make it more engaging and effective.`;
        break;
        
      case 'essay':
        instruction = `Convert this text into a well-structured essay. Include an introduction, body paragraphs with supporting details, and a conclusion.`;
        break;
        
      case 'article':
        instruction = `Convert this text into a well-written article. Use an engaging title, clear sections, and a professional journalistic style.`;
        break;
        
      case 'report':
        instruction = `Convert this text into a formal report. Use proper report structure with sections like Introduction, Findings, Analysis, and Conclusion.`;
        break;
        
      case 'letter':
        instruction = `Convert this text into a proper letter format. Include appropriate salutation, body, and closing.`;
        break;
        
      case 'email':
        instruction = `Convert this text into a professional email format. Include subject line, appropriate salutation, clear body, and proper closing.`;
        break;
        
      case 'story':
        instruction = `Convert this text into a compelling story format. Use narrative techniques, descriptive language, and engaging storytelling.`;
        break;
        
      default:
        instruction = `Rewrite this text according to the user's request. Make it better, clearer, and more effective while keeping all important information.`;
    }

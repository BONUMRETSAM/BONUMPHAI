...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      const isEducational = this.isEducationalRequest(prompt);
      const isMathProblem = this.isMathProblem(prompt);
      const isSerious = this.isSeriousQuestion(prompt);
      needsFormal = isEducational || isMathProblem || isSerious || questionType === 'serious';

      if (this.isGreeting(prompt)) {
        await this.handleGreeting(senderId, prompt, token);
        return;
      }
      ...
      if (!isReply && prompt) {
        ...
        if (history && history.lastResponse) {
          ...
          const isFollowUp = this.isFollowUpRequest(lowerPrompt) || 
                            ...
                            this.hasRelatedWords(prompt, history.lastPrompt);
      ...
      if (!prompt && !isReply && !imageUrl) {
        await sendMessage(senderId, {
          text: 'Uy hello! Ako si Teacher Arlene. Ano maitutulong ko sayo? \n\nPwede mo kong tanungin kahit ano:\n- Chat lang\n- Magpadala ng picture, aanalyze ko\n- Magresearch sa Google Scholar\n- Gumawa ng image\n- Maghanap ng kanta\n- Lyrics ng kanta\n- Real-time updates\n- Translate\n- Summary\n- Assignments/Homework\n\nMga commands:\nai [tanong]\nMagpadala ng picture\ngenerate [search term] [number]\ngscholar [search query]\nplay [song title]\nlyrics [song title] by [artist]'
        }, token);
        return;
      }
      ...
      if (this.isOwnerQuestion(prompt)) {
        await sendMessage(senderId, {
          text: 'Ako ginawa ni GeoDevz69. Check mo to for more info:\nhttps://www.facebook.com/geotechph.net'
        }, token);
        return;
      }
      ...
      if (imageUrl) {
        ...
        const finalPrompt = this.buildFinalPrompt(prompt, previousResponse, previousPrompt, isReply, wantsDetailed, needsFormal, questionType);
        const response = await this.callAPI(finalPrompt, senderId);
        aiResponse = this.cleanResponse(response || 'No response from API.');
      }

      if (!imageUrl && !isReply && !wantsDetailed && !needsFormal) {
        aiResponse = this.shortenResponse(aiResponse);
      }

      if (needsFormal) {
        aiResponse = this.makeFormal(aiResponse);
      } else {
        aiResponse = this.makeConversational(aiResponse);
      }
  ...
  detectQuestionType(prompt) {
    ...
    const seriousKeywords = [
      'investigator', 'investigation', 'offences', 'offense', 'crime', 'criminal',
      'legal', 'law', 'justice', 'police', 'evidence', 'witness', 'testimony',
      'court', 'trial', 'prosecution', 'defense', 'prosecutor', 'defendant',
      'accused', 'suspect', 'arrest', 'detective', 'forensic', 'interrogation',
      'research', 'study', 'analysis', 'evaluation', 'critical', 'comprehensive',
      'assignment', 'homework', 'quiz', 'exam', 'test', 'activity', 'worksheet',
      'module', 'lesson', 'exercise', 'practice', 'problem set', 'task', 'project',
      'research paper', 'essay', 'report', 'case study', 'lab report',
      'explain', 'solve', 'compute', 'calculate', 'determine', 'find', 'prove',
      'analyze', 'evaluate', 'summarize', 'discuss', 'methodology', 'hypothesis',
      'scientific', 'academic', 'scholarly', 'peer reviewed', 'conclusion',
      'recommendation', 'findings', 'results', 'data analysis', 'experiment',
      'why is that the', 'why do investigators', 'why does the', 'need to know'
    ];
    
    const isSerious = seriousKeywords.some(k => lower.includes(k));
    
    const casualKeywords = ['hello', 'hi', 'hey', 'kamusta', 'musta', 'haha', 'lol', 'okay', 'sige', 'cge', 'musta na', 'kamusta na'];
    const isCasual = casualKeywords.some(k => lower.includes(k)) && lower.split(' ').length <= 3;
    
    if (isSerious) return 'serious';
    if (isCasual) return 'casual';
    if (lower.match(/^(why|what|how|when|where|who|which|is|are|do|does|did|can|could|would|will|shall|should|may|might|must|ano|bakit|paano|kailan|saan|sino|alin)/i) && lower.split(' ').length > 2) return 'question';
    
    return 'general';
  },

  // ============ IMPROVED BUILD FINAL PROMPT ============

  buildFinalPrompt(prompt, previousResponse, previousPrompt, isReply, wantsDetailed, needsFormal, questionType) {
    let final = '';

    // ===== STRONG SYSTEM INSTRUCTION =====
    final += 'You are Teacher Arlene, a helpful and knowledgeable AI assistant. ';
    final += 'Your ONLY task is to answer the user\'s question DIRECTLY and ACCURATELY. ';
    final += 'DO NOT talk about movies, TV shows, streaming services, entertainment, or any unrelated topics. ';
    final += 'DO NOT mention Peacock, Netflix, Disney, or any streaming platform. ';
    final += 'STAY FOCUSED on the user\'s question. ';
    final += 'If you do not know the answer, say "I don\'t know" clearly. ';
    final += 'DO NOT make up information or hallucinate.\n\n';

    // ===== CONTEXT =====
    if (previousResponse) {
      const shortContext = previousResponse.substring(0, 150) + (previousResponse.length > 150 ? '...' : '');
      final += `Previous conversation context:\n`;
      final += `User previously asked: ${previousPrompt || 'previous topic'}\n`;
      final += `You responded: ${shortContext}\n\n`;
      final += `The user is now asking a follow-up question. Continue the conversation naturally.\n\n`;
    }

    // ===== QUESTION TYPE INSTRUCTION =====
    if (questionType === 'serious') {
      final += 'IMPORTANT: This is a SERIOUS/EDUCATIONAL question. Provide a formal, well-structured, and comprehensive answer. ';
      final += 'Use proper grammar, complete sentences, and be thorough. ';
      final += 'Include clear explanations and examples if helpful.\n\n';
    } else if (questionType === 'question') {
      final += 'This is a direct question. Provide a clear, accurate, and direct answer. ';
      final += 'Be concise but complete. Give the most relevant information first.\n\n';
    } else if (questionType === 'casual') {
      final += 'This is a casual conversation. Respond in a friendly, natural, and engaging way. ';
      final += 'Sound like a real person talking to a friend.\n\n';
    } else {
      final += 'Provide a helpful and relevant response to the user.\n\n';
    }

    // ===== DETAIL LEVEL =====
    if (wantsDetailed) {
      final += 'USER WANTS DETAILED ANSWER: Provide a comprehensive, thorough explanation with examples and complete information.\n\n';
    } else if (questionType !== 'serious' && questionType !== 'casual') {
      final += 'USER WANTS CONCISE ANSWER: Be direct, short, and to the point. Maximum 2-3 sentences or 1-2 paragraphs.\n\n';
    }
    ...
    final += '- Answer the question directly. No tangents.\n';
    final += '- Stay on topic. No unrelated content.\n';
    final += '- Be accurate and truthful.\n';
    final += '- Use plain text only. No markdown or symbols.\n';
    final += '- If unsure, say so clearly.\n';
    final += '- Do not ask questions back. Just provide the answer.\n';
    
    if (needsFormal || questionType === 'serious') {
      final += '- Use formal, academic, and professional tone.\n';
      final += '- Use complete sentences and proper grammar.\n';
    } else {
      final += '- Be friendly, engaging, and conversational.\n';
    }
  ...
  isEducationalRequest(prompt) {
    if (!prompt) return false;
    const lower = prompt.toLowerCase();
    const keywords = [
      'assignment', 'homework', 'quiz', 'exam', 'test', 'activity',
      'worksheet', 'module', 'lesson', 'exercise', 'practice',
      'problem set', 'task', 'project', 'research paper',
      'essay', 'report', 'case study', 'lab report',
      'takdang aralin', 'gawain', 'pagsasanay', 'pagsusulit',
      'answer', 'solution', 'explain', 'solve', 'compute',
      'calculate', 'determine', 'find', 'show', 'prove',
      'analyze', 'evaluate', 'summarize', 'discuss'
    ];
  ...
  isMathProblem(prompt) {
    ...
    const hasMathWords = ['solve', 'compute', 'calculate', 'what is', 'how much', 'how many', 'find', 'determine', 'math', 'algebra', 'geometry', 'trigonometry', 'calculus', 'statistics', 'probability', 'equation', 'formula', 'solve for', 'x =', 'y =']
      .some(k => lower.includes(k));
    
    return hasMath || hasMathWords;
  },

  isSeriousQuestion(prompt) {
    if (!prompt) return false;
    const lower = prompt.toLowerCase();
    const seriousKeywords = [
      'research', 'study', 'analysis', 'evaluation', 'critical',
      'comprehensive', 'thorough', 'detailed', 'explain thoroughly',
      'in depth', 'step by step', 'formal', 'proper', 'correct',
      'accurate', 'scientific', 'academic', 'scholarly',
      'peer reviewed', 'methodology', 'hypothesis', 'conclusion',
      'recommendation', 'findings', 'results', 'investigator',
      'investigation', 'offences', 'crime', 'legal', 'law', 'justice',
      'police', 'evidence', 'witness', 'testimony', 'court', 'trial'
    ];
  ...
  makeFormal(text) {
    if (!text) return 'I cannot provide a response. Please try again.';
    
    let result = text;
    
    result = result.replace(/^(Ah okay, |Gets ko, |Ah ganun ba, |Oo naman, |Sige, |Eto na, |Wait lang, |Teka, |Ahh, |Hmm, |Alam mo, |Kasi, |So, |Parang, |Uy, |Hoy, |Oi, |Ayy, |Naku, |Hay, |Sus, |Ano, |Eh, |Ha, )/gi, '');
    ...
    for (const [key, value] of Object.entries(formalReplacements)) {
  ...
  makeConversational(text) {
    if (!text) return 'Hmm, wala akong maisip na sagot. Try mo ulit?';
    
    let result = text;
    
    result = result.replace(/^(I'?m?\s*a?\s*AI.*?model.*?\.\s*)/i, '');
    result = result.replace(/^(As an AI assistant,?)\s*/i, '');
    result = result.replace(/^(Here is my (response|analysis|answer)):?\s*/i, '');
    result = result.replace(/^(Let me (answer|explain|help|assist)):?\s*/i, '');
    result = result.replace(/^(Based on my analysis):?\s*/i, '');
    
    const randomStarters = [
      'Ah okay, ', 'Gets ko, ', 'Ah ganun ba, ', 'Oo naman, ', 
      'Sige, ', 'Eto na, ', 'Wait lang, ', 'Teka, ', 'Ahh, ',
      'Hmm, ', 'Alam mo, ', 'Kasi, ', 'So, ', 'Parang, '
    ];
    
    if (!result.match(/^(Ah|Gets|Oo|Sige|Wait|Teka|Hmm|Alam|Kasi|So|Parang|Ayy|Naku|Hay|Uy|Hoy|Oi)/i)) {
    ...
    const casualReplacements = {
      ...
      'Unable to': 'di ko',
      'Unable': 'di ko',
      'Please try again': 'try mo ulit',
      'later': 'mamaya',
      'currently': 'ngayon',
      'currently not': 'hindi pa',
      'not available': 'wala pa',
      'available': 'meron',
      'unavailable': 'wala',
      'successfully': 'ayos',
      'complete': 'tapos na',
      'completed': 'tapos na',
      'processing': 'pinoprocess ko na',
      'loading': 'naglo-load',
      'wait a moment': 'sandali lang',
      'moment': 'sandali',
      'instant': 'agad',
      'immediately': 'agad-agad',
      'quickly': 'mabilis',
      'slowly': 'dahan-dahan',
      'definitely': 'sigurado',
      'absolutely': 'talaga',
      'certainly': 'syempre',
      'obviously': 'obvious naman',
      'clearly': 'klaro naman',
      'understand': 'gets ko',
      'understood': 'gets',
      'got it': 'gets ko na',
      ...
      'unsure': 'di sure',
      'unknown': 'di ko alam',
      'confidential': 'private yan',
      'private': 'personal yan',
      'public': 'public naman',
      ...
      'extremely': 'sobrang',
      'incredibly': 'sobrang',
      'unbelievably': 'sobrang',
      'amazingly': 'ang galing',
      'surprisingly': 'nakakagulat',
      'unfortunately': 'sayang',
      'fortunately': 'buti naman',
      'luckily': 'buti naman',
      'sadly': 'nakakalungkot',
      'happily': 'masaya',
      'gladly': 'masaya',
    ...
    for (const [key, value] of Object.entries(casualReplacements)) {
      const regex = new RegExp('\\b' + key + '\\b', 'gi');
      result = result.replace(regex, value);
    }
    
    const fillers = ['ah', 'eh', 'ay', 'naku', 'hay', 'sus', 'ano', 'kasi', 'eh', 'ha'];
    ...
    if (result.includes('?')) {
      const reactions = ['Ah okay, ', 'Hmm, ', 'Alam mo, ', 'Sige, ', 'So, '];
      if (!result.match(/^(Ah|Hmm|Alam|Sige|So)/i)) {
        result = reactions[Math.floor(Math.random() * reactions.length)] + result;
      }
    }
    
    result = result.replace(/\bvery\b/gi, 'sobrang');
    result = result.replace(/\breally\b/gi, 'talaga');
    result = result.replace(/\bquite\b/gi, 'medyo');
    result = result.replace(/\bsomewhat\b/gi, 'medyo');
    result = result.replace(/\bslightly\b/gi, 'medyo');
    result = result.replace(/\bcompletely\b/gi, 'talagang');
    result = result.replace(/\btotally\b/gi, 'talagang');
    result = result.replace(/\babsolutely\b/gi, 'talagang');
  ...
  hasRelatedWords(prompt, previousPrompt) {
    ...
    return prevWords.some(w => currentWords.some(cw => cw.includes(w) || w.includes(cw)));
  },
  ...
  isGreeting(prompt) {
    ...
    const greetings = ['hello', 'hi', 'hey', 'good morning', 'good afternoon', 'good evening', 'kamusta', 'musta', 'magandang umaga', 'magandang hapon', 'magandang gabi', 'hi there', 'hey there', 'hello there', 'sup', 'yo', 'musta na', 'kamusta na', 'helo', 'helow', 'hallow', 'hola', 'oi', 'oy'];
    return greetings.some(g => lower === g || lower.startsWith(g + ' ') || lower.startsWith(g + '!'));
  },

  async handleGreeting(senderId, prompt, token) {
    try {
      let date = null;
      let time = null;
      ...
      try {
        ...
        if (response.data && response.data.datetime) {
          date = new Date(response.data.datetime);
        ...
        console.log('[Time] Primary API failed, using fallback');
      }
      ...
      let timeOfDay = 'umaga';
      if (hour >= 12 && hour < 18) timeOfDay = 'hapon';
      else if (hour >= 18) timeOfDay = 'gabi';
      
      const greetings = [
        `Uy, good ${timeOfDay}! ${time} na sa Pinas. Anong meron?`,
        `Hello! Good ${timeOfDay} dyan. ${time} na pala. Kumusta ka?`,
        `Oi! Good ${timeOfDay}. ${time} na sa atin. Ano magandang chika?`,
        `Hoy! Good ${timeOfDay} dyan. ${time} na. Musta na buhay?`
      ];
      
      const greeting = greetings[Math.floor(Math.random() * greetings.length)];
      await sendMessage(senderId, { text: greeting }, token);
      
    } catch (error) {
      console.error('[Greeting] Error:', error.message);
      await sendMessage(senderId, { text: 'Uy hello! Kumusta ka na?' }, token);
    }
  },
  ...
  async handleRealtimeQuestion(senderId, prompt, token) {
    ...
    await sendMessage(senderId, { 
      text: 'Ayy sorry, di ko makuha yung info ngayon. Try mo ulit mamaya ha?' 
    }, token);
  },
  ...
  async handleTimeRequest(senderId, prompt, token) {
    try {
      ...
      try {
        ...
        console.log('[Time] WorldTimeAPI failed, trying fallback');
      }
      
      if (!date) {
        try {
          const response = await axios.get('https://timeapi.io/api/Time/current/zone?timeZone=Asia/Manila', {
            timeout: 5000
          });
          if (response.data) {
          ...
          console.log('[Time] TimeAPI failed, using local');
        }
      }
      ...
      const messages = [
        `Eto na, ${day}, ${month} ${date.getDate()}, ${date.getFullYear()} na. ${hour12}:${minute.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')} ${ampm} na sa Pinas.`,
        `Ngayon ay ${day}, ${month} ${date.getDate()}, ${date.getFullYear()}. Oras na ${hour12}:${minute.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')} ${ampm} (PHT).`,
        `Oh, ${day} ngayon, ${month} ${date.getDate()}, ${date.getFullYear()}. ${hour12}:${minute.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')} ${ampm} na.`,
        `Ayy, ${day} na pala ngayon! ${month} ${date.getDate()}, ${date.getFullYear()}. ${hour12}:${minute.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')} ${ampm} na dito sa Pinas.`
      ];
      ...
      try {
        const now = new Date();
        const utc = now.getTime() + (now.getTimezoneOffset() * 60000);
        const date = new Date(utc + (8 * 3600000));
        const options = {
          timeZone: 'Asia/Manila',
          year: 'numeric',
          month: 'long',
        ...
        const fallbackTime = date.toLocaleString('en-PH', options);
        const message = `Eto yung time sa system ko: ${fallbackTime} (PHT).`;
        await this.sendChunks(senderId, message, token);
      } catch (fallbackError) {
        await sendMessage(senderId, { 
          text: 'Ayy sorry, di ko makuha yung time ngayon. Try mo ulit mamaya ha?' 
        }, token);
      }
    }
  },
  ...
  isLyricsRequest(prompt) {
    ...
    const commands = ['lyrics ', 'lyric ', 'letra ', 'kanta ni '];
    if (commands.some(cmd => lower.startsWith(cmd))) {
      return true;
    }
    
    const lyricsKeywords = ['lyrics of', 'lyrics ng', 'letra ng', 'song lyrics', 'full lyrics', 'complete lyrics'];
    return lyricsKeywords.some(k => lower.includes(k));
  },
  ...
  async handleLyricsSearch(senderId, prompt, token) {
    ...
    try {
      ...
      if (!data.status || !data.data) {
        await sendMessage(senderId, { 
          text: `Wala akong mahanap na lyrics para sa "${title}".\n\nTry mo:\n- I-check yung spelling\n- Add artist name\n- Gamitin format: lyrics [title] by [artist]` 
        }, token);
        return;
      }
      ...
      await sendMessage(senderId, { 
        text: `Ayy sorry, nagka-error sa pagkuha ng lyrics para sa "${title}". Try mo ulit mamaya ha?` 
      }, token);
    }
  },
  ...
  isMusicRequest(prompt) {
    ...
    const commands = ['play ', 'music ', 'song ', 'listen ', 'pakinggan ', 'patugtog '];
    if (commands.some(cmd => lower.startsWith(cmd))) {
      return true;
    }
    
    const musicKeywords = ['kanta', 'tugtog', 'soundtrack', 'playlist', 'album', 'single', 'remix', 'cover'];
    const hasMusicKeyword = musicKeywords.some(k => lower.includes(k));
    const isShortPrompt = lower.split(' ').length <= 5;
    
    return hasMusicKeyword && isShortPrompt;
  },
  ...
  async handleMusicSearch(senderId, prompt, token) {
    ...
    try {
      ...
      if (!data || !data.results || data.results.length === 0) {
        await sendMessage(senderId, { text: `Wala akong mahanap na kanta para sa "${searchTerm}".` }, token);
        return;
      }
      ...
      await sendMessage(senderId, { text: `Ayy sorry, nagka-error sa paghahanap ng "${searchTerm}". Try mo ulit mamaya.` }, token);
    }
  },
  ...
  async handleImageGeneration(senderId, prompt, token) {
    ...
    try {
      ...
      if (allImages.length === 0) {
        await sendMessage(senderId, { text: `Wala akong mahanap na images para sa "${searchTerm}".` }, token);
        return;
      }
      ...
      if (resultImages.length === 0) {
        await sendMessage(senderId, { text: `Wala akong mahanap na valid images para sa "${searchTerm}".` }, token);
        return;
      }
      ...
      await sendMessage(senderId, { text: `Eto na, ${resultImages.length} image(s) para sa "${searchTerm}"` }, token);

    } catch (error) {
      console.log('[Generate] Error:', error.message);
      await sendMessage(senderId, { text: `Ayy sorry, nagka-error sa pagkuha ng images para sa "${searchTerm}". Try mo ulit mamaya.` }, token);
    }
  },
  ...
  async handleScholarSearch(senderId, prompt, token) {
    ...
    try {
      ...
      if (results.length === 0) {
        await sendMessage(senderId, { text: `Wala akong mahanap na results para sa "${query}".` }, token);
        return;
      }
      ...
      let errorMessage = 'Ayy sorry, di ko mahanap yung hinahanap mo. ';
      if (error.response?.status === 429) errorMessage += 'Masyado maraming request. Wait ka lang konti.';
      else if (error.response?.status === 403) errorMessage += 'API key ko ata expired na.';
      else errorMessage += 'Try mo ulit mamaya ha?';
  ...
  async callGeminiAPI(prompt, imageUrl) {
    try {
      ...
      return processed || 'Ayy sorry, di ko ma-analyze yung image. Try mo ulit.';
      ...
      return this.cleanResponse(response || 'Ayy sorry, di ko ma-analyze yung image. Try mo ulit.');
    }
  },
...
- Provide FORMAL, ACADEMIC, and DETAILED responses for educational content.
...
USER QUESTION: ${userPrompt || 'Analyze this image and provide a comprehensive response.'}`;
  ...
  isFollowUpRequest(prompt) {
    ...
    if (lower.split(' ').length <= 3) {
      const shortFollowUps = ['bakit', 'why', 'paano', 'how', 'ano', 'what', 'saan', 'where', 'kailan', 'when', 'sino', 'who', 'talaga', 'really', 'gets', 'nagets', 'okay', 'ok', 'sige', 'cge', 'tama', 'correct', 'so', 'eh', 'ay', 'ha', 'hmm', 'oh', 'ah'];
      if (shortFollowUps.some(w => lower.includes(w))) return true;
    }
  ...
  async handleUserInfo(senderId, prompt, token) {
    try {
      ...
      if (prompt.toLowerCase().includes('name') || prompt.toLowerCase().includes('pangalan')) {
        response = userInfo.name ? `Ang pangalan mo ay ${userInfo.name}.` : 'Ayy sorry, di ko yan ma-share. Private yan eh.';
      }
      if (prompt.toLowerCase().includes('birthday') || prompt.toLowerCase().includes('kelan')) {
        response += userInfo.birthday ? `\nAng birthday mo ay ${userInfo.birthday}.` : '\nAyy sorry, di ko yan ma-share. Private yan eh.';
      }
      if (!response) {
        const publicInfo = [];
        ...
        response = publicInfo.length > 0
          ? `Eto yung public info mo:\n${publicInfo.join('\n')}`
          : 'Ayy sorry, di ko yan ma-share. Private yan eh.';
      }
      await sendMessage(senderId, { text: response }, token);
    } catch (error) {
      console.error('[User Info] Failed:', error.message);
      await sendMessage(senderId, { text: 'Ayy sorry, nagka-error sa pagkuha ng info mo.' }, token);
    }
  },
  ...
  cleanResponse(text) {
    ...
    cleaned = cleaned.replace(/^(I'?m?\s*a?\s*AI.*?model.*?\.\s*)/i, '');
    cleaned = cleaned.replace(/^(As an AI assistant,?)\s*/i, '');
    cleaned = cleaned.replace(/^(Here is my (response|analysis|answer)):?\s*/i, '');
    cleaned = cleaned.replace(/^(Let me (answer|explain|help|assist)):?\s*/i, '');
    cleaned = cleaned.replace(/^(I can help you with that):?\s*/i, '');
    return cleaned.trim() || 'No response.';
  },

  getErrorMessage(error) {
    if (error.code === 'ECONNABORTED') return 'Ayy, nag-timeout. Try mo ulit.';
    if (error.response?.status === 429) return 'Masyado maraming request. Wait ka lang konti.';
    if (error.response?.status === 403) return 'Ayy, may problema sa API key ko.';
    if (error.response?.status >= 500) return 'May problema sa server. Try mo ulit mamaya.';
    return 'Ayy sorry, nagka-error. Try mo ulit.';
  },

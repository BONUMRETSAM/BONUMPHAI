...
module.exports = {
  ...
  getTopicSpecificInstructions(topic, language) {
    ...
    const instructions = {
      arithmetic: {
        english: 'Focus ONLY on basic arithmetic operations. Show each operation step clearly.',
        tagalog: 'Tumutok LAMANG sa basic arithmetic operations. Ipakita ang bawat operation nang malinaw.',
        bisaya: 'Tumutok LAMANG sa basic arithmetic operations. Ipakita ang matag operation nga klaro.'
      },
      algebra: {
        english: 'Focus ONLY on algebraic concepts. Show each algebraic manipulation step.',
        tagalog: 'Tumutok LAMANG sa algebraic concepts. Ipakita ang bawat algebraic manipulation step.',
        bisaya: 'Tumutok LAMANG sa algebraic concepts. Ipakita ang matag algebraic manipulation step.'
      },
      geometry: {
        english: 'Focus ONLY on geometric concepts. Identify the shape first. Include units.',
        tagalog: 'Tumutok LAMANG sa geometric concepts. Tukuyin muna ang shape. Isama ang units.',
        bisaya: 'Tumutok LAMANG sa geometric concepts. Ilha una ang shape. Iapil ang units.'
      },
      statistics: {
        english: 'Focus ONLY on statistical concepts. Identify the data type first.',
        tagalog: 'Tumutok LAMANG sa statistical concepts. Tukuyin muna ang data type.',
        bisaya: 'Tumutok LAMANG sa statistical concepts. Ilha una ang data type.'
      },
      sequences: {
        english: 'Focus ONLY on sequences and series. Identify the sequence type first.',
        tagalog: 'Tumutok LAMANG sa sequences at series. Tukuyin muna ang sequence type.',
        bisaya: 'Tumutok LAMANG sa sequences ug series. Ilha una ang sequence type.'
      },
      general: {
        english: 'Focus on the specific mathematical concept being asked. Show each step clearly.',
        tagalog: 'Tumutok sa specific mathematical concept na tinatanong. Ipakita ang bawat step nang malinaw.',
        bisaya: 'Tumutok sa specific mathematical concept nga gipangutana. Ipakita ang matag step nga klaro.'
      }
    };
  ...
  buildMathSolutionPrompt(prompt, language) {
    ...
    if (isTagalog) {
      ...
      final += `- WALANG special symbols o mathematical notation\n\n`;
      ...
      final += `- NO special symbols or mathematical notation\n\n`;
  ...
  buildGeminiPrompt(userPrompt, language = 'english') {
    ...
    if (language === 'tagalog' || language === 'bisaya' || language === 'cebuano') {
      ...
      prompt += `Para sa math problems: Ipakita ang step-by-step na solusyon.\n`;
      prompt += `Para sa activity sheets: Basahin at sagutin ang bawat tanong.\n`;
      prompt += `Para sa memes: Ipaliwanag ang biro (1-2 pangungusap).\n`;
      prompt += `Para sa general images: Ilarawan kung ano ang nakikita.\n\n`;
      ...
      prompt += `For math problems: Show step-by-step solution.\n`;
      prompt += `For activity sheets: Read and answer each question.\n`;
      prompt += `For memes: Explain the joke briefly.\n`;
      prompt += `For general images: Describe what you see.\n\n`;
  ...
  isFollowUpRequest(prompt) {
    const keywords = [
      ...
      'simplify', 'clarify', 'example', 'sample', 'translate',
  ...
  isRealtimeQuestion(prompt) {
    const keywords = ['oras', 'time', 'petsa', 'date', 'balita', 'news', 'panahon', 'weather'];
    return keywords.some(k => prompt.toLowerCase().includes(k));
  },

  isExactTimeRequest(prompt) {
    const lower = prompt.toLowerCase();
    const keywords = ['oras', 'time', 'petsa', 'date', 'anong oras', 'what time', 'current time', 'current date'];
  ...
  async handleRealtimeQuestion(senderId, prompt, token) {
    ...
    try {
      ...
      const data = response.data;
      if (data && data.answer) {
        await this.sendChunks(senderId, this.cleanResponse(data.answer), token);
        return;
      }
    } catch (error) {
      console.error('[RealTime] Error:', error.message);
    }
    const errorMessage = detectedLanguage === 'tagalog' ? 'Hindi makuha ang real-time na impormasyon.' : 'Unable to fetch real-time information.';
  ...
  async handleTimeRequest(senderId, prompt, token) {
    ...
    try {
      ...
      if (detectedLanguage === 'tagalog') {
        message = `Real-Time sa Pilipinas\n\nPetsa: ${date.toLocaleString('fil-PH', { timeZone: 'Asia/Manila' })}\nTimezone: Asia/Manila (UTC+8)`;
      } else if (detectedLanguage === 'bisaya' || detectedLanguage === 'cebuano') {
        message = `Real-Time sa Pilipinas\n\nPetsa: ${date.toLocaleString('en-PH', { timeZone: 'Asia/Manila' })}\nTimezone: Asia/Manila (UTC+8)`;
      } else {
        message = `Real-Time in the Philippines\n\nDate: ${date.toLocaleString('en-PH', { timeZone: 'Asia/Manila' })}\nTimezone: Asia/Manila (UTC+8)`;
      }
      await this.sendChunks(senderId, message, token);
    } catch (error) {
      const now = new Date();
  ...
  isTranslationRequest(prompt) {
    const keywords = ['translate', 'isalin', 'salin', 'translation'];
    return keywords.some(k => prompt.toLowerCase().includes(k));
  },
  ...
  async translateResponse(text, targetLanguage) {
    try {
      const translatePrompt = `Translate this text to ${targetLanguage}. Only provide the translation: ${text}`;

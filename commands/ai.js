...
module.exports = {
  ...
  detectContentType(prompt) {
    ...
    const speechKeywords = [
      'speech', 'talumpati', 'pasasalamat', 'pagdadalamhati', 'eulogy',
      'pagpapasalamat', 'paggunita', 'pagluluksa', 'libing', 'lamay', 'lubong',
      'pag-alala', 'pagpupugay', 'pagkilala', 'paggalang', 'pamamaalam',
      'thank you speech', 'message of gratitude', 'condolence message',
      'sympathy message', 'funeral speech', 'eulogy speech', 'pagpapaalam'
    ];
    
    // Letter-related keywords
    const letterKeywords = [
      'letter', 'liham', 'sulat', 'correspondence', 'note',
      'open letter', 'bukas na liham', 'love letter', 'liham ng pag-ibig',
      'thank you letter', 'pasasalamat na liham', 'condolence letter',
      'liham pasasalamat', 'liham pagdadalamhati'
    ];
    
    let hasSpeech = speechKeywords.some(k => lower.includes(k));
    let hasLetter = letterKeywords.some(k => lower.includes(k));
    
    if (hasSpeech && hasLetter) {
      if (lower.includes('liham') || lower.includes('sulat') || lower.includes('letter')) {
        return 'letter';
      }
      return 'speech';
    }
    ...
    // Check for action verbs that imply content creation
    const createKeywords = ['gumawa', 'sumulat', 'magsulat', 'magbigay', 'create', 'write', 'make', 'give', 'magsalita', 'magtalumpati'];
    const contentKeywords = ['pasasalamat', 'pagdadalamhati', 'mensahe', 'message', 'thank you', 'condolence', 'sympathy', 'pagpapasalamat', 'pag-alala', 'paggunita', 'eulogy'];
    
    let hasCreate = createKeywords.some(k => lower.includes(k));
    let hasContent = contentKeywords.some(k => lower.includes(k));
  ...
  // ================================================================
  // 🆕 FIXED: BUILD SPEECH PROMPT (ALWAYS LONG)
  // ================================================================
  buildSpeechPrompt(prompt, wantsDetailed, language = 'english') {
    const langName = this.getLanguageName(language);
    let final = '';
    
    const lower = prompt.toLowerCase();
    
    const isPowerful = lower.includes('powerful') || lower.includes('makapangyarihan') || lower.includes('strong') || lower.includes('emotional');
    const isThankful = lower.includes('thankful') || lower.includes('pasasalamat') || lower.includes('gratitude') || lower.includes('thanks') || lower.includes('pagpapasalamat');
    const isFuneral = lower.includes('funeral') || lower.includes('libing') || lower.includes('namatay') || lower.includes('death') || lower.includes('pumanaw') || lower.includes('nawala') || lower.includes('pagkawala') || lower.includes('pagdadalamhati') || lower.includes('mourning') || lower.includes('eulogy') || lower.includes('lamay') || lower.includes('lubong') || lower.includes('pagluluksa');
    
    const isExplicitlyShort = lower.includes('short') || lower.includes('maikli') || lower.includes('brief');
    
    let speechType = 'GENERAL';
    if (isThankful) speechType = 'THANK YOU';
    if (isFuneral) speechType = 'FUNERAL / EULOGY / PAGDADALAMHATI';
    if (isPowerful) speechType = 'POWERFUL ' + speechType;
    
    final += `You are a master speechwriter. Create a COMPLETE, LONG, FLUENT, and ${isPowerful ? 'POWERFUL and EMOTIONAL' : 'HEARTFELT and SINCERE'} ${isThankful ? 'THANK YOU' : ''} ${isFuneral ? 'FUNERAL / EULOGY / PAGDADALAMHATI' : ''} SPEECH in ${langName.toUpperCase()}.\n\n`;
    
    let topic = prompt;
    const removeWords = ['create', 'make', 'write', 'speech', 'talumpati', 'thankful', 'pasasalamat', 'powerful', 'long', 'mahaba', 'detailed', 'about', 'tungkol sa', 'para sa', 'for', 'full', 'complete', 'please', 'paki', 'pakiusap', 'as', 'a', 'an', 'the'];
    for (const word of removeWords) {
      topic = topic.replace(new RegExp(word, 'gi'), '');
    }
    topic = topic.trim() || 'gratitude and appreciation';
    
    final += `SPEECH TOPIC/PURPOSE: ${topic}\n\n`;
    
    if (isThankful) {
      final += `This is a THANK YOU SPEECH. It should express deep gratitude and appreciation.\n`;
    }
    if (isFuneral) {
      final += `This is a FUNERAL / EULOGY / PAGDADALAMHATI SPEECH. It should honor the deceased, express grief, and comfort the family.\n`;
    }
    
    if (isExplicitlyShort) {
      final += `📝 LENGTH: SHORT (2-3 paragraphs) - User explicitly asked for short.\n`;
    } else {
      final += `📝 LENGTH: COMPLETE and LONG (8-12 paragraphs).\n`;
      final += `- This must be a FULLY DEVELOPED speech, NOT a short response.\n`;
      final += `- Include: Opening greeting, body with multiple sections, and a powerful closing.\n`;
      final += `- Add emotional depth, personal stories, and heartfelt messages.\n`;
      final += `- The speech should feel NATURAL, SPOKEN, and FLUENT.\n`;
      final += `- DO NOT CUT IT SHORT. This is a COMPLETE speech.\n`;
    }
    final += `\n`;
    
    if (isPowerful) {
      final += `⚡ STYLE: POWERFUL and EMOTIONAL.\n`;
      final += `- Use strong, impactful language.\n`;
      final += `- Include rhetorical devices like repetition, parallelism, and emotional appeal.\n`;
      final += `- Make it inspiring and moving.\n`;
    } else {
      final += `💝 STYLE: HEARTFELT, SINCERE, and FLUENT.\n`;
      final += `- Use warm, genuine language.\n`;
      final += `- Be personal and authentic.\n`;
      final += `- Make it sound like a real person speaking from the heart.\n`;
    }
    
    final += `
🌐 LANGUAGE: The user may want a MIX of languages (Tagalog, Bisaya, English). 
   - You can use a combination of these languages naturally.
   - The primary language should be ${langName.toUpperCase()} but you can switch to other languages for emotional impact.
   - Example: "Daghang salamat sa inyong tanan. Your kindness we will never forget."

📌 SPEECH STRUCTURE (Complete & Detailed):
1. OPENING (1-2 paragraphs): 
   - Warm greeting to all attendees
   - State the purpose of the speech
   - Acknowledge the occasion

2. BODY (4-6 paragraphs):
   a. EXPRESS GRATITUDE: Thank specific groups (family, friends, community, etc.)
   b. SHARE MEMORIES: Personal stories or meaningful moments
   c. ACKNOWLEDGE SUPPORT: Recognize the help and love received
   d. REFLECT: Share lessons, insights, or the significance of the occasion
   ${isFuneral ? 'e. HONOR THE DECEASED: Celebrate their life, legacy, and impact\n   f. OFFER COMFORT: Words of encouragement to the grieving family' : ''}

3. CLOSING (1-2 paragraphs):
   - End with a powerful message, final thanks, or meaningful quote
   - Blessing or prayer for the audience

🎯 TONE: ${isPowerful ? 'Inspirational, moving, and impactful' : 'Warm, sincere, and heartfelt'}.

🚨 CRITICAL INSTRUCTIONS:
- Respond in ${langName.toUpperCase()} language (with optional mix of Tagalog/Bisaya/English).
- Use proper speech format with PARAGRAPHS.
- Make it sound NATURAL and SPOKEN, NOT like a formal essay.
- Include EMOTIONAL DEPTH and PERSONAL CONNECTION.
- Do NOT just give a one-sentence definition of "speech".
- Do NOT ask questions back.
- This is a COMPLETE, LONG speech.
- ${isExplicitlyShort ? 'Keep it SHORT (2-3 paragraphs).' : 'MAKE IT 8-12 PARAGRAPHS LONG. DO NOT CUT IT SHORT.'}
- Be FLUENT and NATURAL.

NOW CREATE THE COMPLETE, LONG, FLUENT ${isPowerful ? 'POWERFUL' : 'HEARTFELT'} ${isThankful ? 'THANK YOU' : ''} ${isFuneral ? 'FUNERAL/EULOGY/PAGDADALAMHATI' : ''} SPEECH IN ${langName.toUpperCase()} (with optional Tagalog/Bisaya mix):
START WITH A PROPER GREETING AND MAKE IT COMPLETE AND LONG.`;
  
  return final;
  },

  // ================================================================
  // 🆕 FIXED: BUILD LETTER PROMPT
  // ================================================================
  buildLetterPrompt(prompt, wantsDetailed, language = 'english') {
    const langName = this.getLanguageName(language);
    let final = '';
    
    const lower = prompt.toLowerCase();
    const isThankful = lower.includes('thankful') || lower.includes('pasasalamat') || lower.includes('gratitude');
    const isLove = lower.includes('love') || lower.includes('pag-ibig');
    const isOpen = lower.includes('open letter') || lower.includes('bukas na liham');
    const isFuneral = lower.includes('funeral') || lower.includes('libing') || lower.includes('namatay') || lower.includes('death') || lower.includes('pumanaw') || lower.includes('nawala') || lower.includes('pagkawala') || lower.includes('pagdadalamhati');
    
    final += `You are a letter writer. Create a COMPLETE, LONG ${isLove ? 'LOVE' : ''} ${isThankful ? 'THANK YOU' : ''} ${isOpen ? 'OPEN' : ''} ${isFuneral ? 'CONDOLENCE / SYMPATHY' : ''} LETTER in ${langName.toUpperCase()}.\n\n`;
    
    let topic = prompt;
    const removeWords = ['create', 'make', 'write', 'letter', 'liham', 'thankful', 'pasasalamat', 'love', 'open', 'long', 'mahaba', 'detailed', 'about', 'tungkol sa', 'para sa', 'for', 'full', 'complete', 'please', 'paki'];
    for (const word of removeWords) {
      topic = topic.replace(new RegExp(word, 'gi'), '');
    }
    topic = topic.trim() || 'gratitude';
    
    final += `LETTER PURPOSE: ${topic}\n\n`;
    
    final += `📝 LENGTH: COMPLETE and LONG (5-7 paragraphs).\n`;
    final += `- Include proper letter format with date, greeting, body, closing, and signature.\n`;
    final += `- Make it personal, heartfelt, and complete.\n`;
    final += `- Do NOT cut it short.\n\n`;
...
📌 LETTER FORMAT:
1. DATE: Include date at the top.
2. GREETING: Dear [Recipient],
3. BODY (4-5 paragraphs): 
   - Express the main message (gratitude, love, appreciation, condolences, etc.)
   - Include specific reasons, memories, or details
   - Be personal and sincere
4. CLOSING: Sincerely, With love, Yours truly, etc.
5. SIGNATURE: Your name

🎯 TONE: ${isLove ? 'Romantic and heartfelt' : isThankful ? 'Grateful and warm' : isFuneral ? 'Sympathetic and comforting' : 'Sincere and personal'}.

🚨 IMPORTANT:
- Respond in ${langName.toUpperCase()} language (with optional mix of Tagalog/Bisaya/English).
- Use proper letter format.
- Make it personal and authentic.
- Do NOT just give a one-sentence definition.
- MAKE IT LONG AND COMPLETE with 5-7 paragraphs.

NOW CREATE THE COMPLETE, LONG LETTER IN ${langName.toUpperCase()}:`;
...
USER QUESTION: ${userPrompt || 'Analyze this image'}`;
  ...
  // ================================================================
  // 🆕 FIXED: MODIFICATION REQUEST with "long" support
  // ================================================================
  isModificationRequest(prompt) {
    ...
    const patterns = [
      ...
      'pls', 'plz', 'pleas',
      // 🔥 NEW: Long/Extend patterns
      'make it long', 'make it longer', 'extend it', 'pahabain', 'hahabaan',
      'gusto ko mahaba', 'gusto ko long', 'i want long', 'i want longer',
      'mas mahaba', 'longer version', 'full version', 'complete version',
      'buo', 'kumpleto', 'dagdagan mo', 'more content', 'i-extend',
      'gusto ko mahaba', 'pahabain mo', 'dagdagan mo pa'
    ];
  ...
  // ================================================================
  // 🆕 FIXED: FOLLOW-UP DETECTION with "long" support
  // ================================================================
  isFollowUpRequest(prompt) {
    ...
    const keywords = [
      ...
      'pingi', 'ping', 'peng', 'pengi', 'pking',
      'pleas', 'pls', 'plz',
      // 🔥 NEW: Long/Extend keywords
      'long', 'mahaba', 'haba', 'pahabain', 'extend', 'mas mahaba',
      'longer', 'pahaba', 'hahabaan', 'dagdagan', 'more content',
      'complete', 'full', 'buo', 'kumpleto', 'mas buo',
      'i-extend', 'i-full', 'i-complete', 'gusto ko long'
    ];
  ...
  isNewTopic(prompt, previousPrompt, originalPrompt) {
    ...
    const followUpIndicators = [
      ...
      // 🔥 NEW: Long indicators
      'long', 'mahaba', 'haba', 'pahabain', 'extend', 'buo', 'kumpleto'
    ];
  ...
  // ================================================================
  // 🆕 FIXED: FINAL PROMPT BUILDER with "long" and "extend" support
  // ================================================================
  buildFinalPrompt(prompt, previousResponse, previousPrompt, isReply, wantsDetailed, language = 'english') {
    ...
    if (contentType === 'speech') {
      return this.buildSpeechPrompt(prompt, wantsDetailed, language);
    }
    
    if (contentType === 'letter') {
      return this.buildLetterPrompt(prompt, wantsDetailed, language);
    }
    ...
    if (previousResponse) {
      ...
      // 🔥🔥🔥 FIX: Handle "long" / "extend" / "pahabain" requests
      if (lower.includes('long') || lower.includes('mahaba') || lower.includes('pahabain') || 
          lower.includes('extend') || lower.includes('longer') || lower.includes('hahabaan') ||
          lower.includes('mas mahaba') || lower.includes('dagdagan') || lower.includes('more content') ||
          lower.includes('gusto ko long') || lower.includes('i want long') || lower.includes('i want longer') ||
          lower.includes('buo') || lower.includes('kumpleto') || lower.includes('complete') ||
          lower.includes('pahabain mo') || lower.includes('dagdagan mo pa') || lower.includes('gusto ko mahaba')) {
        
        final += `⚠️⚠️⚠️ CRITICAL INSTRUCTION ⚠️⚠️⚠️\n`;
        final += `The user wants the PREVIOUS RESPONSE to be LONGER and MORE COMPLETE.\n`;
        final += `- DO NOT ask what they want to be long.\n`;
        final += `- DO NOT ask for clarification.\n`;
        final += `- DO NOT change the topic.\n`;
        final += `- DO NOT start a new topic.\n`;
        final += `- EXTEND the PREVIOUS RESPONSE above.\n`;
        final += `- Add MORE DETAILS, MORE PARAGRAPHS, and MORE CONTENT.\n`;
        final += `- EXPAND on what you already said.\n`;
        final += `- Make it 2-3 times longer than the previous response.\n\n`;
        final += `PREVIOUS TOPIC: "${previousPrompt}"\n`;
        final += `PREVIOUS RESPONSE: "${previousResponse}"\n\n`;
        final += `NOW, EXTEND THE PREVIOUS RESPONSE TO MAKE IT LONGER AND MORE COMPLETE.\n`;
        final += `DO NOT ASK QUESTIONS. JUST PROVIDE THE EXTENDED VERSION.\n\n`;
      }
      
      // 🔥🔥🔥 CRITICAL FIX FOR "ELABORATE" / "MORE EXPLANATION"
      else if (lower.includes('elaborate') || lower.includes('paki elaborate') || 
      ...
      // 🚨 FINAL REMINDER - Always at the end
      ...
      final += `- Just provide the response directly.\n`;
      final += `- STAY ON TOPIC.\n\n`;

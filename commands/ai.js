...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // Check for specific commands first with more strict matching
      if (this.isExactLyricsCommand(prompt)) {
        await this.handleLyricsSearch(senderId, prompt, token);
        return;
      }

      if (this.isExactMusicCommand(prompt)) {
        await this.handleMusicSearch(senderId, prompt, token);
        return;
      }

      // Check for generate/image command with proper prefix
      if (this.isExactGenerateCommand(prompt)) {
        await this.handleImageGeneration(senderId, prompt, token);
        return;
      }

      // Check for scholar command with proper prefix
      if (this.isExactScholarCommand(prompt)) {
        await this.handleScholarSearch(senderId, prompt, token);
        return;
      }

      // Check for realtime questions - but only if not part of other contexts
      if (this.isExactRealtimeQuestion(prompt)) {
      ...
      // Handle return to topic with more strict matching
      if (!isReply && prompt) {
        const history = conversationHistory[senderId];
        if (history && history.lastResponse) {
          const lowerPrompt = prompt.toLowerCase();
          
          // Only check for return to topic if the user explicitly mentions it
          const returnToTopic = this.isExplicitReturnToTopic(prompt);
          if (returnToTopic) {
  ...
  // NEW: Strict command detection functions
  isExactLyricsCommand(prompt) {
    const lower = prompt.toLowerCase().trim();
    // Only trigger if the command starts with 'lyrics' or 'letra' or is clearly a lyrics request
    return lower.startsWith('lyrics') || 
           lower.startsWith('letra') || 
           lower.startsWith('song lyrics') ||
           lower.match(/^(lyrics|letra|song lyrics)\s+/i) !== null;
  },

  isExactMusicCommand(prompt) {
    const lower = prompt.toLowerCase().trim();
    // Only trigger if the command starts with 'play' or 'music' and is a clear music request
    if (!lower.startsWith('play') && !lower.startsWith('music') && !lower.startsWith('pakinggan')) {
      return false;
    }
    // Check if the remaining text is a song title (not a question)
    const afterCommand = prompt.slice(lower.startsWith('play') ? 4 : lower.startsWith('music') ? 5 : 8).trim();
    if (!afterCommand) return false;
    // Don't trigger for questions or academic terms
    const questionWords = ['what', 'why', 'how', 'when', 'where', 'who', 'which', 'ano', 'bakit', 'paano', 'kailan', 'saan', 'sino', 'alin'];
    if (questionWords.some(w => afterCommand.toLowerCase().includes(w))) return false;
    // Don't trigger for academic/research terms
    const academicWords = ['research', 'thesis', 'study', 'paper', 'article', 'journal', 'literature', 'citation', 'academic', 'scholar', 'reference'];
    if (academicWords.some(w => afterCommand.toLowerCase().includes(w))) return false;
    return true;
  },

  isExactGenerateCommand(prompt) {
    const lower = prompt.toLowerCase().trim();
    // Only trigger if it starts with 'generate', 'image', 'img', or 'show' and is clearly an image request
    if (!lower.startsWith('generate') && !lower.startsWith('image') && !lower.startsWith('img') && !lower.startsWith('show')) {
      return false;
    }
    const afterCommand = prompt.slice(lower.startsWith('generate') ? 8 : lower.startsWith('image') ? 5 : lower.startsWith('img') ? 3 : 4).trim();
    if (!afterCommand) return false;
  ...
  isExactScholarCommand(prompt) {
    const lower = prompt.toLowerCase().trim();
    // Only trigger if it starts with 'gscholar', 'scholar', 'googlescholar', or 'research' as a command
    return lower.startsWith('gscholar') || 
           lower.startsWith('googlescholar') || 
           lower.startsWith('scholar') ||
           lower.match(/^research\s+/i) !== null;
  },

  isExactRealtimeQuestion(prompt) {
    const lower = prompt.toLowerCase();
    
    // Time requests - only if explicitly asking for time
    const timeKeywords = ['anong oras', 'what time', 'what is the time', 'anong petsa', 'what date', 'what is the date', 'current time', 'current date', 'anong oras na', 'what time is it'];
    if (timeKeywords.some(k => lower.includes(k))) {
      return true;
    }

    // News/current events - only if explicitly asking about news
    const newsKeywords = ['ano balita', 'what\'s the news', 'what is the news', 'news today', 'balita ngayon', 'latest news', 'may nangyari', 'what happened'];
    if (newsKeywords.some(k => lower.includes(k))) {
      return true;
    }

    // Weather - only if explicitly asking about weather
    const weatherKeywords = ['weather today', 'panahon ngayon', 'anong panahon', 'ulan ngayon', 'will it rain', 'temperature today'];
    if (weatherKeywords.some(k => lower.includes(k))) {
      return true;
    }

    // Price updates - only if explicitly asking about prices
    const priceKeywords = ['magkano', 'how much', 'price of', 'presyo ng', 'gastos', 'cost of'];
  ...
  isExplicitReturnToTopic(prompt) {
    const lower = prompt.toLowerCase().trim();
    // More strict: must explicitly mention returning to topic
    const patterns = ['balik tayo sa topic', 'balik sa topic', 'balikan natin', 'going back to the topic', 'back to the topic', 'return to the topic', 'tungkol naman sa'];
    return patterns.some(p => lower.includes(p));
  },

  isReturnToTopicRequest(prompt) {
    // Keep for backward compatibility but use stricter version
    return this.isExplicitReturnToTopic(prompt);
  },
  ...
  isRealtimeQuestion(prompt) {
    // Keep for backward compatibility but use stricter version
    return this.isExactRealtimeQuestion(prompt);
  },
  ...
  isExactTimeRequest(prompt) {
    const lower = prompt.toLowerCase();
    const keywords = ['anong oras', 'what time', 'what is the time', 'anong petsa', 'what date', 'what is the date', 'current time', 'current date', 'anong oras na', 'what time is it'];
  ...
  isLyricsRequest(prompt) {
    // Keep but use stricter version via isExactLyricsCommand
    return this.isExactLyricsCommand(prompt);
  },
  ...
  isGenerateCommand(prompt) {
    // Keep but use stricter version via isExactGenerateCommand
    return this.isExactGenerateCommand(prompt);
  },
  ...
  isScholarCommand(prompt) {
    // Keep but use stricter version via isExactScholarCommand
    return this.isExactScholarCommand(prompt);
  },

  isResearchQuery(prompt) {
    const lower = prompt.toLowerCase();
    // Only trigger if it starts with research or is an explicit research command
    if (lower.startsWith('research') || lower.startsWith('gscholar') || lower.startsWith('scholar')) {
      return true;
    }
    // Don't trigger if the word is just mentioned in a question
    const questionWords = ['what', 'why', 'how', 'when', 'where', 'who', 'which', 'ano', 'bakit', 'paano', 'kailan', 'saan', 'sino', 'alin'];
  ...
  isMusicRequest(prompt) {
    // Keep but use stricter version via isExactMusicCommand
    return this.isExactMusicCommand(prompt);
  },

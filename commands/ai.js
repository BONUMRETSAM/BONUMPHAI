...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // Check if it's an assignment/homework/educational request
      const isEducational = this.isEducationalRequest(prompt);
      const isMathProblem = this.isMathProblem(prompt);
      const isInvestigation = this.isInvestigationQuestion(prompt);
      needsFormal = isEducational || isMathProblem || this.isSeriousQuestion(prompt) || isInvestigation;
      ...
      if (imageUrl) {
        ...
        const finalPrompt = this.buildFinalPrompt(prompt, previousResponse, previousPrompt, isReply, wantsDetailed, needsFormal);
  ...
  isInvestigationQuestion(prompt) {
    if (!prompt) return false;
    const lower = prompt.toLowerCase();
    const keywords = [
      'investigator', 'investigation', 'investigate', 'investigating',
      'offences', 'offense', 'crime', 'criminal', 'legal', 'law', 
      'justice', 'police', 'evidence', 'witness', 'testimony',
      'court', 'trial', 'prosecution', 'defense', 'prosecutor',
      'defendant', 'accused', 'suspect', 'arrest', 'detective',
      'forensic', 'interrogation', 'questioning', 'inquiry',
      'why is that the', 'why do investigators', 'why does the',
      'need to know the', 'must know the', 'should know the'
    ];
  ...
  isEducationalRequest(prompt) {
    ...
    const keywords = [
      ...
      'ano ang sagot', 'paano i-solve', 'help me with',
      'need help', 'please answer', 'answer this',
      'grade', 'score', 'points', 'percentage',
      'investigator', 'investigation', 'offences', 'crime',
      'legal', 'law', 'justice', 'police', 'evidence',
      'why is that', 'why do', 'why does', 'why did'
    ];
    return keywords.some(k => lower.includes(k));
  },
  ...
  isSeriousQuestion(prompt) {
    ...
    const seriousKeywords = [
      'research', 'study', 'analysis', 'evaluation',
      'critical', 'comprehensive', 'thorough', 'detailed',
      'explain thoroughly', 'in depth', 'step by step',
      'formal', 'proper', 'correct', 'accurate',
      'scientific', 'academic', 'scholarly', 'peer reviewed',
      'methodology', 'hypothesis', 'conclusion',
      'recommendation', 'findings', 'results'
    ];
  ...
  async handleGreeting(senderId, prompt, token) {
    try {
      ...
      // Fallback: Use local time with correct timezone
      if (!time) {
        const now = new Date();
        date = now;
  ...
  async handleTimeRequest(senderId, prompt, token) {
    try {
      ...
      // Fallback 2: Local time with correct timezone
      if (!date) {
        const now = new Date();
  ...
  isMusicRequest(prompt) {
    ...
    // OR explicitly mention music/song with specific pattern
...
USER QUESTION: ${userPrompt || 'Analyze this image and provide a comprehensive response.'}`;
  ...
  buildFinalPrompt(prompt, previousResponse, previousPrompt, isReply, wantsDetailed, needsFormal) {
    let final = '';

    // Check for investigation question
    const isInvestigation = this.isInvestigationQuestion(prompt);
    if (isInvestigation) {
      needsFormal = true;
    }
    ...
    if (previousResponse) {
      ...
      final += 'Previous context (summarized):\n';
      final += `User asked about: ${previousPrompt || 'previous topic'}\n`;
      final += `AI said: ${shortContext}\n\n`;
      ...
      if (this.isContextualQuestion(lower, previousPrompt)) {
        ...
        final += 'Acknowledge the previous context and respond directly.\n\n';
      }
    ...
    // Special handling for investigation questions
    if (isInvestigation) {
      final += 'IMPORTANT: This is a serious question about investigation and legal matters.\n';
      final += 'Provide a clear, accurate, and educational response.\n';
      final += 'Explain the importance of knowing offences in investigations.\n';
      final += 'Use formal and professional language.\n\n';
    }

    if (needsFormal || wantsDetailed) {
      final += 'USER NEEDS FORMAL/ACADEMIC RESPONSE: Provide a comprehensive, thorough, and detailed explanation.\n';
      final += 'Use formal language, proper structure, and complete information.\n';
      final += 'Include examples, context, and step-by-step details when applicable.\n';
      final += 'This is for educational or academic purposes.\n\n';
    } else {
    ...
    if (needsFormal || isInvestigation) {
      final += '- Maintain a formal, academic, and professional tone.\n';
      final += '- Use proper grammar and complete sentences.\n';
      final += '- Provide thorough and well-structured explanation.\n';
    } else {

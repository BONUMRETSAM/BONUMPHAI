...
module.exports = {
    ...
    try {
      ...
      if (this.isReturnToTopicRequest(prompt)) {
        ...
        if (history && topic && history.topicHistory && history.topicHistory[topic]) {
          ...
          // If no specific topic found, use last conversation
      ...
      // Save topic for return-to-topic feature
  ...
  extractTopicKey(prompt) {
    // Extract a short key from the prompt for topic tracking
  ...
  isReturnToTopicRequest(prompt) {
    ...
    // More specific patterns for return to topic
    const patterns = [
      'balik tayo sa',
      'balikan natin',
      'going back to',
      ...
      'balik tayo sa topic',
      'balikan ang topic',
      'balik sa pinag-usapan',
      'balik sa pinagusapan',
      'continue about',
      'continue with',
      'tuloy natin ang',
      'ituloy ang',
    ...
    // Check for "balik tayo" followed by topic
  ...
  extractTopicFromReturn(prompt) {
    ...
    // If no specific topic, return null
    return null;
  },
  ...
  isLyricsRequest(prompt) {
    const lower = prompt.toLowerCase();
    const keywords = ['lyrics', 'lyric', 'letra', 'song lyrics', 'lyrics of', 'lyrics ng', 'letra ng', 'lyrics for', 'lyrics to', 'full lyrics', 'complete lyrics', 'lyrics and chords', 'chords and lyrics'];
    return keywords.some(k => lower.includes(k));
  },

  async handleLyricsSearch(senderId, prompt, token) {
    let searchTerm = prompt;
    const removeKeywords = ['lyrics', 'lyric', 'letra', 'song lyrics', 'lyrics of', 'lyrics ng', 'letra ng', 'lyrics for', 'lyrics to', 'full lyrics', 'complete lyrics', 'lyrics and chords', 'chords and lyrics'];
  ...
  isGenerateCommand(prompt) {
    ...
    // Only match if the prompt STARTS with the command
    return commands.some(cmd => lower.startsWith(cmd + ' ') || lower === cmd);
  },

  isImageRequest(prompt) {
    const lower = prompt.toLowerCase();
    // More specific patterns - require actual image request intent
    const patterns = [
      'show me image',
      'show me picture',
      'show me photo',
  ...
  isResearchQuery(prompt) {
    const lower = prompt.toLowerCase();
    // More specific patterns - require actual research intent
    const patterns = [
      'find research',
      'find study',
      'find studies',

...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // ===== CONVERSATIONAL ADJUSTMENT (HINDI SIRA ANG SAGOT) =====
      aiResponse = this.makeConversational(aiResponse, prompt);
  ...
  // ===== CONVERSATIONAL MAKER - HINDI SINISIRA ANG SAGOT =====
  makeConversational(response, prompt) {
    if (!response) return response;
    
    const lowerPrompt = prompt.toLowerCase();
    const isTagalog = this.isTagalog(prompt);
    const isAcknowledgment = this.isAcknowledgment(prompt);
    const isShort = prompt.split(' ').length <= 4;
    const isQuestion = this.isQuestion(prompt);
    
    // KUNG ACKNOWLEDGMENT LANG (ahhh yun pala, wala na, okay, sige)
    if (isAcknowledgment) {
      if (isTagalog) {
    ...
    // KUNG SOBRANG IKLI AT CASUAL (hindi question)
    if (isShort && !isQuestion) {
      // I-shorten ang response para direct
    ...
    // KUNG TAGALOG ANG TANONG AT HINDI FORMAL
    if (isTagalog && !this.isFormalQuestion(prompt) && !isQuestion) {
      // Tanggalin ang mga formal intros
      let cleaned = response;
      cleaned = cleaned.replace(/^I understand your question\.\s*/i, '');
      cleaned = cleaned.replace(/^Let me explain\.\s*/i, '');
      cleaned = cleaned.replace(/^Here is my answer\.\s*/i, '');
      cleaned = cleaned.replace(/^Based on my knowledge\.\s*/i, '');
      cleaned = cleaned.trim();
      
      // Kung mahaba at maikli ang tanong, paikliin
  ...
  isTagalog(prompt) {
    const tagalogWords = [
      ...
      'hahaha', 'hehehe', 'shet', 'gagi', 'lala', 'hala',
      'ayoko', 'sige', 'cge', 'okay', 'ok', 'ge',
      'gets', 'nagets', 'naintindihan', 'gots',
  ...
  isAcknowledgment(prompt) {
    ...
    const ackPhrases = [
      ...
      'sige', 'cge', 'ge', 'salamat', 'thanks',
      'wala na', 'tapos na', 'done', 'finished',
    ...
    if (wordCount <= 4 && ackPhrases.some(p => lower.includes(p))) {
      return true;
    }
    const fullPhrases = [
      'yun pala yun', 'yon pala yon', 'yun na yun',
      'ahhh yun pala', 'ah yun pala', 'ganun pala',
      'gets ko na', 'nagets ko na', 'naintindihan ko na',
      'okay na', 'sige na', 'cge na', 'wala na',
      'ahhh gets', 'ah okay', 'oh okay', 'aw okay',
      'so yun', 'so yon', 'so ganun', 'mao diay'
    ];
    return fullPhrases.some(p => lower.includes(p));
  },
  ...
  isFormalQuestion(prompt) {
    ...
    const formalKeywords = [
      ...
      'statistics', 'data', 'scientific', 'academic',
      'scholarly', 'literature', 'citation', 'reference',
      'bibliography', 'journal', 'article', 'thesis',
      'dissertation', 'abstract', 'conclusion', 'findings',
      'results', 'discussion', 'introduction', 'history',
      'historical', 'philosophy', 'psychological', 'sociological',
      'anthropology', 'linguistics', 'economics', 'political',
      'government', 'constitution', 'legal', 'law', 'justice',
    ...
    return formalKeywords.some(k => lower.includes(k));
  },
  ...
  isQuestion(prompt) {
    ...
    const questionWords = ['ano', 'what', 'bakit', 'why', 'paano', 'how', 'kailan', 'when', 'saan', 'where', 'sino', 'who', 'alin', 'which', 'may tanong', 'question', 'ask'];

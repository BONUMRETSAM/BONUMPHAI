...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // Check if it's a logic puzzle that can be solved directly
      ...
      // Check if it's a money problem that can be solved directly
  ...
  solveLogicPuzzleDirectly(prompt) {
    // Extract any sequence from the prompt
    ...
    // Check for restrictions (e.g., "without writing the letter E")
    const restriction = this.extractRestriction(prompt);
    
    // If there's a restriction and the next element is forbidden
    ...
    // If no restriction or restriction doesn't apply
  ...
  extractRestriction(prompt) {
    // Look for "without writing the letter X" pattern
    const match = prompt.match(/without writing the letter\s*([A-Z])/i);
    if (match) {
      return match[1].toUpperCase();
    }
    
    // Look for "bawal isulat ang X" pattern
  ...
  findAlternative(sequence, pattern, forbidden) {
    if (pattern.type === 'alphabet') {
      ...
      // Try to find a letter that implies the forbidden letter
      ...
      // Fallback: just skip the forbidden letter
  ...
  formatLogicAnswer(sequence, nextElement, alternative = null, restriction = null) {
    ...
    if (restriction && alternative) {
      ...
      response += `Paliwanag:\n`;
      response += `Ang sequence ay: ${sequence.join(' ')} _\n`;
      response += `Ang pattern ay ${this.describePattern(sequence)}.\n`;
  ...
  describePattern(sequence) {
    ...
    if (start !== -1) {
      ...
      if (isAlpha) return 'alphabet order (sunod-sunod na letra)';
    }
    ...
    if (numbers.every(n => !isNaN(n))) {
      ...
      if (isArithmetic) {
        return `+${diff} pattern (dagdag ng ${diff} bawat hakbang)`;
      }
    }
  ...
  solveMoneyProblemDirectly(prompt) {
    ...
    if (lower.includes('sukli') || lower.includes('change') || lower.includes('pera') || 
        lower.includes('money') || lower.includes('bayad') || lower.includes('payment') ||
        lower.includes('bibilhin') || lower.includes('gastos') || lower.includes('presyo')) {
      
      // Extract numbers
      const numbers = prompt.match(/\d+/g);
      if (numbers && numbers.length >= 2) {
        // Determine which is amount and which is payment
        ...
        // Check for "bayad" pattern
        const bayadMatch = prompt.match(/bayad\s*(\d+)/i) || prompt.match(/bayaran\s*(\d+)/i);
        if (bayadMatch) {
          payment = parseInt(bayadMatch[1]);
          amount = numbers.find(n => parseInt(n) !== payment);
        }
        
        // Check for "may" pattern (may X pesos)
        const mayMatch = prompt.match(/may\s*(\d+)/i);
        if (mayMatch && !amount) {
          amount = parseInt(mayMatch[1]);
          payment = numbers.find(n => parseInt(n) !== amount);
        }
        
        // Fallback: first number is amount, second is payment
        if (!amount || !payment) {
          amount = parseInt(numbers[0]);
          payment = parseInt(numbers[1]);
        }
  ...
  formatMoneyAnswer(amount, payment) {
    ...
    if (change < 0) {
      return `Kulang ang pera mo ng ${Math.abs(change)} pesos.\n\nKailangan mo ng ${amount} pesos, pero ${payment} pesos lang ang meron ka.\nKulang ka ng ${Math.abs(change)} pesos.`;
    }
    
    let response = `Sukli mo: ${change} pesos\n\n`;
    response += `📌 Mga Praktikal na Paraan:\n\n`;
    
    // Option 1: Exact payment
    response += `✅ Magbayad ng eksaktong ${amount} pesos - WALANG SUKLI\n`;
    
    // Option 2: Next higher bill
  ...
  getBestPaymentOption(amount, change) {
    // Find the most practical payment option
    const bills = [1000, 500, 200, 100, 50, 20];
    ...
    for (const bill of bills) {
      if (bill >= amount) {
        const change2 = bill - amount;
        return `Magbayad ng ${bill} pesos - sukli ${change2} pesos (isang pirasong pera lang ang ibabayad mo).`;
      }
    }
    
    // If current payment is reasonable
    if (change <= 100) {

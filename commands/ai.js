...
module.exports = {
  name: ['ai', 'opera', 'ask', 'gemini', 'vision', 'gscholar', 'scholar', 'googlescholar', 'research', 'generate', 'image', 'img', 'show', 'math', 'solve', 'mathhelp', 'mathtutor', 'algebra', 'calculus', 'geometry', 'statistics', 'trigonometry', 'matrix', 'complex', 'arithmetic', 'probability', 'derivative', 'integral', 'equation'],
  description: 'Multi-modal AI with Math Solver, Image Analysis, Scholar, Image Generation, Music, and Lyrics',
  usage: 'ai [message] or math [problem] or send/reply to image',
  version: '5.0.0',
  author: 'GeoDevz69',
  category: 'AI',
  cooldown: 3,
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      const isMathQuery = this.isMathQuery(prompt);
      
      if (isMathQuery && !this.isGeneralAIQuery(prompt)) {
        await this.handleMathQuery(senderId, prompt, token);
        return;
      }
      ...
      if (!prompt && !isReply && !imageUrl) {
        await sendMessage(senderId, {
          text: 'Hello. I am Teacher Arlene - Multi-Modal AI with Math Solver.\n\nCapabilities:\nText conversations\nImage analysis\nAcademic research\nImage generation\nMusic search\nLyrics search\nReal-time questions\nTranslation\nSummarization\nMath solving (99.9% accurate)\n\nCommands:\nai [question]\nmath [problem]\nmath samples [topic]\nSend image to analyze\ngenerate [search term] [number]\ngscholar [search query]\nplay [song title]\nlyrics [song title] by [artist]\n\nI can speak Tagalog, Bisaya, English, and other languages.'
        }, token);
        return;
      }
  ...
  isMathQuery(prompt) {
    ...
    const generalAI = ['hello', 'hi', 'kamusta', 'musta', 'how are you', 'what is ai', 'who are you', 'salamat', 'thank you', 'thanks'];
    if (generalAI.some(k => lower.includes(k))) return false;
    ...
    const mathKeywords = [
      ...
      'samples', 'examples', 'sample', 'example', 'determinant', 'inverse',
      'complex', 'imaginary', 'det', 'matrix', 'matrices'
    ];
  ...
  isGeneralAIQuery(prompt) {
    const lower = prompt.toLowerCase();
    const general = ['hello', 'hi', 'hey', 'kamusta', 'musta', 'how are you', 
                     'what is ai', 'who are you', 'good morning', 'good afternoon',
                     'good evening', 'salamat', 'thank you', 'thanks'];
    return general.some(k => lower.includes(k));
  },

  async handleMathQuery(senderId, prompt, token) {
    try {
      if (prompt.toLowerCase().includes('samples') || 
          prompt.toLowerCase().includes('examples') ||
          prompt.toLowerCase().includes('sample')) {
        
        const topic = this.extractMathTopic(prompt);
        const result = await this.getMathSamples(topic);
        const cleanResult = this.cleanMathResponse(result);
        await this.sendChunks(senderId, cleanResult, token);
        return;
      }

      const result = await this.solveWithMathJS(prompt);
      const cleanResult = this.cleanMathResponse(result);
      await this.sendChunks(senderId, cleanResult, token);
      ...
      try {
        const aiResult = await this.solveWithAI(prompt);
        const cleanResult = this.cleanMathResponse(aiResult);
        await this.sendChunks(senderId, cleanResult, token);
      } catch (aiError) {
        await sendMessage(senderId, {
          text: 'Error solving math problem. Please try again with a clearer question.'
        }, token);
      }
    }
  },

  extractMathTopic(prompt) {
    const lower = prompt.toLowerCase();
    const topics = {
      'algebra': ['algebra', 'equation', 'linear', 'quadratic', 'polynomial'],
      'calculus': ['calculus', 'derivative', 'integral', 'limit'],
      'geometry': ['geometry', 'triangle', 'circle', 'area', 'volume'],
      'trigonometry': ['trig', 'sin', 'cos', 'tan'],
  ...
  async getMathSamples(topic) {
    const prompt = `Generate 5 ${topic} problems with complete step-by-step solutions. Format each problem as: Problem: [statement] Solution: [steps] Answer: [final answer]. Use plain text only.`;
    return await this.callAPI(prompt);
  },

  async solveWithMathJS(prompt) {
    const lower = prompt.toLowerCase();
    let result = '';

    try {
      if (this.isArithmetic(prompt)) {
        result = await this.solveArithmetic(prompt);
        return result;
      }

      if (lower.includes('solve') || lower.includes('equation') || 
          lower.includes('find x') || lower.includes('=')) {
        result = await this.solveAlgebra(prompt);
        return result;
      }

      if (lower.includes('derivative') || lower.includes('differentiate')) {
        result = await this.solveDerivative(prompt);
        return result;
      }

      if (lower.includes('integral') || lower.includes('integrate')) {
        result = await this.solveIntegral(prompt);
        return result;
      }

      if (lower.includes('factor')) {
        result = await this.solveFactor(prompt);
        return result;
      }

      if (lower.includes('expand')) {
        result = await this.solveExpand(prompt);
        return result;
      }

      if (lower.includes('area')) {
        result = await this.solveArea(prompt);
        return result;
      }

      if (lower.includes('volume')) {
        result = await this.solveVolume(prompt);
        return result;
      }

      if (lower.includes('sin') || lower.includes('cos') || lower.includes('tan')) {
        result = await this.solveTrig(prompt);
        return result;
      }

      if (lower.includes('mean') || lower.includes('median') || lower.includes('mode') ||
          lower.includes('standard deviation') || lower.includes('variance')) {
        result = await this.solveStats(prompt);
        return result;
      }

      if (lower.includes('matrix') || lower.includes('determinant') || 
          lower.includes('det') || lower.includes('inverse')) {
        result = await this.solveMatrix(prompt);
        return result;
      }

      if (lower.includes('complex') || lower.includes('imaginary') || 
          prompt.includes('i') && (prompt.includes('+') || prompt.includes('-'))) {
        result = await this.solveComplex(prompt);
        return result;
      }

      if (lower.includes('percent') || lower.includes('percentage') || lower.includes('%')) {
        result = await this.solvePercentage(prompt);
        return result;
      }

      if (prompt.includes('/')) {
        result = await this.solveFraction(prompt);
        return result;
      }

      if (prompt.includes('^') || lower.includes('power') || lower.includes('square')) {
        result = await this.solvePower(prompt);
        return result;
      }

      if (lower.includes('sqrt') || lower.includes('root') || lower.includes('√')) {
        result = await this.solveRoot(prompt);
        return result;
      }

      if (this.isWordProblem(prompt)) {
        result = await this.solveWithAI(prompt);
        return result;
      }
      ...
      console.error('[MathJS] Error:', error.message);
      return await this.solveWithAI(prompt);
    }
  },

  isArithmetic(prompt) {
    const clean = prompt.replace(/[a-zA-Z]/g, '').trim();
    return /[\d\+\-\*\/\(\)]/.test(clean) && !prompt.includes('=');
  },

  solveArithmetic(prompt) {
    try {
      const clean = prompt.replace(/[^0-9+\-*/(). ]/g, '');
      const result = math.evaluate(clean);
      return `Problem: ${prompt}\nSolution: ${clean} = ${result}\nAnswer: ${result}`;
    } catch (error) {
      throw error;
    }
  },
    ...
    try {
      ...
      if (eqMatch) {
        ...
        try {
          ...
          const solved = math.solve(expr);
          return `Problem: ${prompt}\nSolution:\n${left} = ${right}\nAnswer: x = ${solved}`;
        } catch {
          throw new Error('Cannot solve algebra');
        }
      }
  ...
  solveDerivative(prompt) {
    try {
      const match = prompt.match(/derivative\s+of\s+(.+)/i) || 
                    prompt.match(/differentiate\s+(.+)/i);
      if (match) {
        const expr = match[1].trim();
        const derivative = math.derivative(expr, 'x');
        return `Problem: Find derivative of ${expr}\nSolution: d/dx(${expr}) = ${derivative}\nAnswer: ${derivative}`;
      }
      throw new Error('No derivative expression found');
    } catch (error) {
      throw error;
    }
  },

  solveIntegral(prompt) {
    try {
      const match = prompt.match(/integral\s+of\s+(.+)/i) ||
                    prompt.match(/integrate\s+(.+)/i);
      if (match) {
        const expr = match[1].trim();
        const integral = math.integrate(expr, 'x');
        return `Problem: Find integral of ${expr}\nSolution: ∫${expr}dx = ${integral} + C\nAnswer: ${integral} + C`;
      }
      throw new Error('No integral expression found');
    } catch (error) {
      throw error;
    }
  },
  ...
  solveFactor(prompt) {
    try {
      const match = prompt.match(/factor\s+(.+)/i);
      if (match) {
        const expr = match[1].trim();
        const factored = math.simplify(expr);
        return `Problem: Factor ${expr}\nSolution: ${expr} = ${factored}\nAnswer: ${factored}`;
      }
      throw new Error('No factor expression found');
    } catch (error) {
      throw error;
    }
  },
  ...
  solveExpand(prompt) {
    try {
      const match = prompt.match(/expand\s+(.+)/i);
      if (match) {
        const expr = match[1].trim();
        const expanded = math.simplify(expr);
        return `Problem: Expand ${expr}\nSolution: ${expr} = ${expanded}\nAnswer: ${expanded}`;
      }
      throw new Error('No expand expression found');
    } catch (error) {
      throw error;
    }
  },
  ...
  solveArea(prompt) {
    try {
      ...
      const circleMatch = lower.match(/circle\s+radius\s+(\d+\.?\d*)/);
      if (circleMatch) {
        const r = parseFloat(circleMatch[1]);
        const area = math.pi * r * r;
        return `Problem: Area of circle with radius ${r}\nFormula: A = πr²\nSolution: A = π(${r})² = ${area}\nAnswer: ${area.toFixed(4)} square units`;
      }

      const rectMatch = lower.match(/rectangle\s+(\d+\.?\d*)\s*(?:by|x)\s*(\d+\.?\d*)/);
      if (rectMatch) {
        const l = parseFloat(rectMatch[1]);
        const w = parseFloat(rectMatch[2]);
        const area = l * w;
        return `Problem: Area of rectangle ${l} x ${w}\nFormula: A = L × W\nSolution: A = ${l} × ${w} = ${area}\nAnswer: ${area} square units`;
      }

      const triMatch = lower.match(/triangle\s+base\s+(\d+\.?\d*)\s+height\s+(\d+\.?\d*)/);
      if (triMatch) {
        const b = parseFloat(triMatch[1]);
        const h = parseFloat(triMatch[2]);
        const area = 0.5 * b * h;
        return `Problem: Area of triangle with base ${b} and height ${h}\nFormula: A = ½ × b × h\nSolution: A = ½ × ${b} × ${h} = ${area}\nAnswer: ${area} square units`;
      }

      throw new Error('No area formula matched');
    } catch (error) {
      throw error;
    }
  },
  ...
  solveVolume(prompt) {
    try {
      ...
      const cylMatch = lower.match(/cylinder\s+radius\s+(\d+\.?\d*)\s+height\s+(\d+\.?\d*)/);
      if (cylMatch) {
        const r = parseFloat(cylMatch[1]);
        const h = parseFloat(cylMatch[2]);
        const volume = math.pi * r * r * h;
        return `Problem: Volume of cylinder radius ${r}, height ${h}\nFormula: V = πr²h\nSolution: V = π(${r})²(${h}) = ${volume}\nAnswer: ${volume.toFixed(4)} cubic units`;
      }

      const sphMatch = lower.match(/sphere\s+radius\s+(\d+\.?\d*)/);
      if (sphMatch) {
        const r = parseFloat(sphMatch[1]);
        const volume = (4/3) * math.pi * r * r * r;
        return `Problem: Volume of sphere radius ${r}\nFormula: V = ⁴⁄₃πr³\nSolution: V = ⁴⁄₃π(${r})³ = ${volume}\nAnswer: ${volume.toFixed(4)} cubic units`;
      }

      throw new Error('No volume formula matched');
    } catch (error) {
      throw error;
    }
  },
  ...
  solveTrig(prompt) {
    try {
      ...
      const sinMatch = lower.match(/sin\s*\(?\s*(\d+\.?\d*)\s*(?:deg|degrees)?\s*\)?/);
      if (sinMatch) {
        const angle = parseFloat(sinMatch[1]);
        const rad = angle * math.pi / 180;
        const result = math.sin(rad);
        return `Problem: sin(${angle}°)\nSolution: sin(${angle}°) = ${result}\nAnswer: ${result.toFixed(6)}`;
      }

      const cosMatch = lower.match(/cos\s*\(?\s*(\d+\.?\d*)\s*(?:deg|degrees)?\s*\)?/);
      if (cosMatch) {
        const angle = parseFloat(cosMatch[1]);
        const rad = angle * math.pi / 180;
        const result = math.cos(rad);
        return `Problem: cos(${angle}°)\nSolution: cos(${angle}°) = ${result}\nAnswer: ${result.toFixed(6)}`;
      }

      const tanMatch = lower.match(/tan\s*\(?\s*(\d+\.?\d*)\s*(?:deg|degrees)?\s*\)?/);
      if (tanMatch) {
        const angle = parseFloat(tanMatch[1]);
        const rad = angle * math.pi / 180;
        const result = math.tan(rad);
        return `Problem: tan(${angle}°)\nSolution: tan(${angle}°) = ${result}\nAnswer: ${result.toFixed(6)}`;
      }

      throw new Error('No trig function matched');
    } catch (error) {
      throw error;
    }
  },
  ...
  solveStats(prompt) {
    try {
      ...
      const meanMatch = lower.match(/mean\s+of\s+([\d,\s]+)/);
      if (meanMatch) {
        const numbers = meanMatch[1].split(',').map(n => parseFloat(n.trim()));
        const mean = math.mean(numbers);
        return `Problem: Mean of ${numbers.join(', ')}\nSolution: μ = (${numbers.join(' + ')}) / ${numbers.length}\nμ = ${mean}\nAnswer: ${mean}`;
      }

      const medianMatch = lower.match(/median\s+of\s+([\d,\s]+)/);
      if (medianMatch) {
        const numbers = medianMatch[1].split(',').map(n => parseFloat(n.trim()));
        const sorted = [...numbers].sort((a,b) => a-b);
        const median = math.median(numbers);
        return `Problem: Median of ${numbers.join(', ')}\nSolution: Sort: ${sorted.join(', ')}\nMedian = ${median}\nAnswer: ${median}`;
      }

      throw new Error('No stats function matched');
    } catch (error) {
      throw error;
    }
  },
  ...
  solveMatrix(prompt) {
    try {
      const rows = prompt.match(/\[([\d,\s]+)\]/g);
      if (rows) {
        const matrix = rows.map(row => 
          row.replace(/[\[\]]/g, '').split(',').map(n => parseFloat(n.trim()))
        );
        
        const lower = prompt.toLowerCase();
        if (lower.includes('determinant') || lower.includes('det')) {
          const det = math.det(matrix);
          return `Problem: Determinant of ${matrix}\nSolution: det = ${det}\nAnswer: ${det}`;
        }
        
        if (lower.includes('inverse')) {
          const inv = math.inv(matrix);
          return `Problem: Inverse of ${matrix}\nSolution: A⁻¹ = ${inv}\nAnswer: ${inv}`;
        }
      }
      throw new Error('No matrix operation matched');
    } catch (error) {
      throw error;
    }
  },
  ...
  solveComplex(prompt) {
    try {
      const complexMatch = prompt.match(/([\d.]+)([+-])([\d.]+)i/);
      if (complexMatch) {
        const a = parseFloat(complexMatch[1]);
        const sign = complexMatch[2];
        const b = parseFloat(complexMatch[3]) * (sign === '-' ? -1 : 1);
        const c = math.complex(a, b);
        return `Problem: ${prompt}\nSolution: ${c}\nAnswer: ${c}`;
      }
      throw new Error('No complex number matched');
    } catch (error) {
      throw error;
    }
  },

  solvePercentage(prompt) {
    try {
      const match = prompt.match(/(\d+\.?\d*)\s*%\s+of\s+(\d+\.?\d*)/);
      if (match) {
        const percent = parseFloat(match[1]);
        const number = parseFloat(match[2]);
        const result = (percent / 100) * number;
        return `Problem: ${percent}% of ${number}\nSolution: (${percent}/100) × ${number} = ${result}\nAnswer: ${result}`;
      }
      throw new Error('No percentage matched');
    } catch (error) {
      throw error;
    }
  },

  solveFraction(prompt) {
    try {
      const clean = prompt.replace(/[^0-9+\-*/()./ ]/g, '');
      const result = math.evaluate(clean);
      return `Problem: ${prompt}\nSolution: ${clean} = ${result}\nAnswer: ${result}`;
  ...
  solvePower(prompt) {
    try {
      ...
      if (match) {
        ...
        return `Problem: ${base}^${exp}\nSolution: ${base} raised to ${exp} = ${result}\nAnswer: ${result}`;
      }
      throw new Error('No power expression matched');
    } catch (error) {
      throw error;
    }
  },
  ...
  solveRoot(prompt) {
    try {
      const match = prompt.match(/sqrt\s*\(?\s*(\d+\.?\d*)\s*\)?/);
      if (match) {
        const number = parseFloat(match[1]);
        const result = math.sqrt(number);
        return `Problem: √${number}\nSolution: √${number} = ${result}\nAnswer: ${result}`;
      }
      throw new Error('No root expression matched');
    } catch (error) {
      throw error;
    }
  },

  isWordProblem(prompt) {
    const wordIndicators = [
      'john', 'mary', 'age', 'work', 'mixture', 'investment', 
      'distance', 'rate', 'time', 'coin', 'discount', 'interest'
    ];
    const lower = prompt.toLowerCase();
    return wordIndicators.some(w => lower.includes(w)) && prompt.length > 20;
  },

  async solveWithAI(prompt) {
    const aiPrompt = `Solve this math problem step by step. Be 100% accurate:
    ${prompt}
    
    Format:
    Problem: [restate]
    Given: [list given info if applicable]
    Solution: [step by step]
    Answer: [final answer]
    
    Use plain text only.`;
    
    return await this.callAPI(aiPrompt);
  },

  cleanMathResponse(text) {
    if (!text) return 'No solution generated.';
    let cleaned = text;
    cleaned = cleaned.replace(/\*\*/g, '').replace(/\*/g, '').replace(/`/g, '');
    cleaned = cleaned.replace(/```/g, '').replace(/#{1,6}\s*/g, '');
    cleaned = cleaned.replace(/---+/g, '').replace(/__/g, '').replace(/_/g, '');
    cleaned = cleaned.replace(/~~/g, '').replace(/\[([^\]]+)\]\([^)]+\)/g, '$1');
    cleaned = cleaned.replace(/[\u{1F000}-\u{1FFFF}]/gu, '');
    cleaned = cleaned.replace(/[\u{2600}-\u{27BF}]/gu, '');
    cleaned = cleaned.replace(/^I'?m?\s+a?\s*AI.*?model.*?\n\n?/i, '');
    cleaned = cleaned.replace(/^As an AI.*?\n\n?/i, '');
    cleaned = cleaned.replace(/\n{3,}/g, '\n\n');
    cleaned = cleaned.replace(/[ \t]+/g, ' ');
    cleaned = cleaned.trim();
    if (!cleaned.toLowerCase().includes('problem:') && !cleaned.toLowerCase().includes('solution:')) {
      cleaned = 'Problem: ' + cleaned;
    }
    return cleaned || 'No solution generated.';
  },
  ...
  async callGeminiAPI(prompt, imageUrl, detectedLanguage) {
    try {
      const detectPrompt = `Analyze this image and determine what language the text in the image is written in. Common languages: Tagalog, Filipino, English, Bisaya, Cebuano, Spanish, etc. Respond with ONLY the language name in English.`;
      ...
      return this.cleanResponse(response || 'Unable to analyze image. Please try again.');
    }
  },

  buildGeminiPrompt(userPrompt, language) {
    const langName = this.getLanguageName(language);
    let prompt;
    if (language === 'tagalog' || language === 'bisaya' || language === 'cebuano') {
...
USER QUESTION: ${userPrompt || 'Analyze this image'}`;
  ...
  buildFinalPrompt(prompt, previousResponse, previousPrompt, isReply, wantsDetailed, language) {
    const langName = this.getLanguageName(language);
    let final = '';
    ...
    if (previousResponse) {
      ...
      if (lower.includes('elaborate') || lower.includes('paki elaborate') || 
        ...
        final += `CRITICAL: ELABORATE on the PREVIOUS RESPONSE.\n`;
        ...
        final += `NOW, ELABORATE on the SAME TOPIC.\n\n`;
        ...
        final += `- Provide SPECIFIC examples.\n\n`;
                 ...
                 lower.includes('brief') || lower.includes('short')) {
        final += `CRITICAL: Provide a SUMMARY of the PREVIOUS RESPONSE.\n`;
        final += `- Provide only KEY POINTS.\n`;
        final += `- Be CONCISE and DIRECT.\n\n`;
      } else if (lower.includes('simplify') || lower.includes('simple') || 
                 lower.includes('pasimplehin')) {
        final += `CRITICAL: Provide a SIMPLER explanation.\n`;
        final += `- Use SIMPLE words and layman terms.\n`;
        final += `- Keep the SAME meaning.\n\n`;
      } else if (this.isTranslationRequest(prompt)) {
        const targetLang = this.detectTargetLanguage(prompt);
        final += `CRITICAL: Translate the PREVIOUS RESPONSE to ${targetLang}.\n`;
      ...
      final += `- STAY ON TOPIC.\n`;
      final += `- DO NOT explain what words mean.\n`;
    ...
    final += `\nRespond in ${langName.toUpperCase()}. Use plain text only. No markdown.`;
    return final;
  },
  ...
  async handleLyricsSearch(senderId, prompt, token) {
    ...
    try {
      ...
      if (!data.status || !data.data) {
        await sendMessage(senderId, { text: `No lyrics found for "${title}".\n\nTry:\n- Check spelling\n- Add artist name\n- Format: lyrics [title] by [artist]` }, token);
        return;
      }
      ...
      await sendMessage(senderId, { text: `Error fetching lyrics for "${title}". Please try again later.` }, token);
    }
  },
  ...
  async handleImageGeneration(senderId, prompt, token) {
    ...
    try {
      ...
      await sendMessage(senderId, { text: `Found ${resultImages.length} images for "${searchTerm}"` }, token);
    } catch (error) {
  ...
  async handleScholarSearch(senderId, prompt, token) {
    ...
    try {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        message += `APA 7th Edition:\n${apaCitation}\n\nMLA 9th Edition:\n${mlaCitation}\n\nVerified: Checked and accessible\n${new Date().toLocaleString('fil-PH', { timeZone: 'Asia/Manila' })}`;
      ...
      let errorMessage = 'Google Scholar search failed. ';
      if (error.response?.status === 429) errorMessage += 'Rate limit reached. Please wait.';
      else if (error.response?.status === 403) errorMessage += 'Invalid or expired API key.';
  ...
  async handleMusicSearch(senderId, prompt, token) {
    ...
    try {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        const created = track.created_at ? new Date(track.created_at).toLocaleDateString('fil-PH') : 'Unknown Date';
        let audioUrl = '';
  ...
  async handleUserInfo(senderId, prompt, token) {
    try {
      ...
      await sendMessage(senderId, { text: 'Error fetching user information.' }, token);
    }
  },
  ...
  getErrorMessage(error, detectedLanguage) {

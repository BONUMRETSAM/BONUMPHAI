...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // ============================================================
      // FIXED RESUME HANDLING - NO API CALL, USES HARDCODED DATA
      // ============================================================
      if (this.isResumeRequest(prompt)) {
        const history = conversationHistory[senderId];
        const lower = prompt.toLowerCase();
        
        // Detect job type from prompt
        const jobKeywords = ['teacher', 'agriculture', 'nurse', 'engineer', 'accountant', 
                           ...
                           'encoder', 'data entry', 'clerk', 'assistant', 'supervisor',
                           'team lead', 'laborer', 'worker'];
        
        const hasJobSpecified = jobKeywords.some(k => lower.includes(k));
        
        // ALWAYS USE HARDCODED DATA - DIRECT RESPONSE, NO API CALL
        let aiResponse;
        
        if (hasJobSpecified) {
          // Generate complete resume based on job type using hardcoded data
          aiResponse = this.buildResumeByJob(prompt, detectedLanguage);
        } else {
          // Default to cashier resume if no specific job mentioned
          aiResponse = this.buildResumeByJob('cashier resume', detectedLanguage);
        }
        ...
        // Send response directly - NO API CALL
        await this.sendChunks(senderId, aiResponse, token);
        return;
      }
      // ============================================================
      // END FIXED RESUME HANDLING
      // ============================================================
...
- STAY ON TOPIC.`;
  ...
  // ============================================================
  // COMPLETE RESUME BUILDER - EXACT FORMAT FROM SCREENSHOT
  // NO EXTRA LINES, NO ( ) PLACEHOLDERS, COMPLETE INFO
  // ============================================================
  ...
  buildResumeByJob(prompt, detectedLanguage) {
    ...
    const jobMap = {
      'cashier': ['cashier', 'retail', 'taga-cash', 'supermarket', 'landmark', 'sm', 'jollibee'],
      'teacher': ['teacher', 'guro', 'educator', 'professor', 'instructor', 'deped', 'school'],
      'nurse': ['nurse', 'nursing', 'hospital', 'clinic', 'medical', 'health'],
      'engineer': ['engineer', 'engineering', 'civil', 'mechanical', 'electrical', 'architecture'],
      'accountant': ['accountant', 'accounting', 'finance', 'bookkeeper', 'audit'],
      'callcenter': ['call center', 'bpo', 'customer service', 'teleperformance', 'concentrix'],
      'manager': ['manager', 'supervisor', 'team lead', 'head', 'director', 'operations'],
      'secretary': ['secretary', 'admin', 'administrative', 'receptionist', 'office staff'],
      'cook': ['cook', 'chef', 'kitchen', 'culinary', 'restaurant'],
      'driver': ['driver', 'truck', 'delivery', 'transporter', 'van'],
      'security': ['security', 'guard', 'safety officer', 'bantay', 'protection'],
      'janitor': ['janitor', 'cleaner', 'maintenance', 'housekeeping', 'utility'],
      'laborer': ['laborer', 'construction', 'worker', 'helper', 'factory', 'warehouse'],
      'it': ['it', 'programmer', 'developer', 'software', 'web developer', 'coding'],
      'sales': ['sales', 'marketing', 'seller', 'promoter', 'distributor'],
      'hr': ['hr', 'human resources', 'recruiter', 'personnel'],
      'barista': ['barista', 'coffee', 'cafe', 'starbucks', 'brew'],
      'receptionist': ['receptionist', 'front desk', 'frontline', 'guest']
    };
    
    for (const [type, keywords] of Object.entries(jobMap)) {
    ...
    const resumes = {
      ...
      cashier: {
        ...
        experiences: [
          {
            title: 'Cashier',
            company: 'SM Hypermarket – SM Tunasan',
            years: '',
            responsibilities: [
          ...
          },
          {
            ...
            company: 'Jollibee Shell SLT Alabang',
            years: '',
            responsibilities: [
        ...
        education: [
          { level: 'Bachelor of Science in Criminology', school: 'Pamantasan ng Lungsod ng Muntinlupa', year: '2018' },
          { level: 'Muntinlupa National High School (MNHS)', school: 'Muntinlupa City, Philippines', year: '2010 – 2014' },
          { level: 'Muntinlupa Elementary School (MES)', school: 'Muntinlupa City, Philippines', year: '2004 – 2010' }
        ],
        skills: [
        ...
        references: [
          { name: 'Angelika Mae Lopez', position: 'HR Assistant', contact: '09911451130' },
          { name: 'Czerina Mae Espenesin', position: 'Accounting Assistant | Mabex Manufacturing Inc.', contact: '09928196248' },
          { name: 'Menchie Donayre', position: 'Graphic Designer', contact: '09563892622' }
        ]
      },
      
      // ======================== TEACHER ========================
      teacher: {
        summary: `Dedicated and passionate Teacher with a Bachelor's degree in Education and over 5 years of experience in delivering engaging and effective instruction to diverse learners. Proven ability to create a positive and inclusive classroom environment that fosters academic growth and personal development. Skilled in curriculum planning, classroom management, and student assessment. Committed to inspiring students to reach their full potential and achieve academic success.`,
        personal: {
          gender: 'Female',
          nationality: 'Filipino',
          ...
          religion: 'Roman Catholic',
          status: 'Married',
          languages: 'English, Tagalog, and Ilocano'
        },
        experiences: [
          {
            title: 'Elementary Teacher',
            company: 'Muntinlupa Elementary School – Muntinlupa City',
            years: '2020 – Present',
            responsibilities: [
              'Developed and implemented engaging lesson plans aligned with DepEd curriculum standards to meet diverse learning needs.',
              'Created a positive and inclusive classroom environment that fosters academic growth and personal development.',
              'Assessed student progress through tests, assignments, and projects, providing constructive feedback to support learning.',
              'Collaborated with parents and colleagues to support student development and address academic concerns.',
              'Participated in faculty meetings, seminars, and professional development programs to enhance teaching skills.'
            ]
          },
          {
            ...
            years: '2018 – 2020',
            responsibilities: [
              'Assisted lead teacher in preparing instructional materials and activities to support effective lesson delivery.',
              'Provided one-on-one and small group support to students, addressing individual learning needs.',
              'Monitored student behavior and maintained classroom order to create a conducive learning environment.',
              'Evaluated student work and provided constructive feedback to support academic progress.'
            ]
          }
        ],
        education: [
          { level: 'Bachelor of Elementary Education', school: 'Pamantasan ng Lungsod ng Muntinlupa', year: '2018' },
        ...
        references: [
          { name: 'Dr. Maria Theresa C. Santos', position: 'School Principal II | Muntinlupa Elementary School', contact: '09987654321' },
          { name: 'Mrs. Rosa Marie B. Cruz', position: 'Master Teacher III | St. Mary\'s Academy', contact: '09176543210' },
          { name: 'Mr. Jose Paolo G. Garcia', position: 'Education Program Supervisor | DepEd Muntinlupa', contact: '09085432109' }
        ]
      },
      
      // ======================== NURSE ========================
      nurse: {
        summary: `Compassionate and dedicated Registered Nurse with over 4 years of experience in providing high-quality patient care in hospital and community settings. Skilled in patient assessment, medication administration, and emergency response. Committed to promoting health and wellness through patient education and evidence-based practice. Seeking to contribute clinical expertise and compassionate care to a reputable healthcare institution.`,
        personal: {
          gender: 'Female',
          nationality: 'Filipino',
          ...
          religion: 'Roman Catholic',
          status: 'Single',
          languages: 'English, Tagalog, and Bisaya'
        },
        experiences: [
          {
            title: 'Staff Nurse',
            company: 'Muntinlupa Medical Center – Muntinlupa City',
            years: '2021 – Present',
            responsibilities: [
              'Provided comprehensive nursing care to patients in the medical-surgical ward, managing 10-12 patients per shift.',
              'Administered medications and treatments according to physician orders and established protocols.',
              'Conducted thorough patient assessments and documented vital signs, interventions, and outcomes accurately.',
              'Educated patients and families on disease management, treatment plans, and discharge instructions.',
              'Collaborated with interdisciplinary healthcare team to develop and implement individualized care plans.'
            ]
          },
          {
            title: 'Community Health Nurse',
            company: 'Barangay Health Center – Muntinlupa City',
            years: '2019 – 2020',
            responsibilities: [
              'Provided health education and promotion services to community members through seminars and outreach programs.',
              'Conducted maternal and child health assessments and provided prenatal and postnatal care.',
              'Administered immunizations and participated in health campaigns to prevent disease outbreaks.',
              'Maintained accurate patient records and coordinated referrals to hospitals for specialized care.'
            ]
          }
        ],
        education: [
          { level: 'Bachelor of Science in Nursing', school: 'Pamantasan ng Lungsod ng Muntinlupa', year: '2019' },
          { level: 'Senior High School', school: 'Muntinlupa National High School – Muntinlupa City', year: '2015 – 2017' },
          { level: 'Muntinlupa National High School (MNHS)', school: 'Muntinlupa City, Philippines', year: '2011 – 2015' },
          { level: 'Muntinlupa Elementary School (MES)', school: 'Muntinlupa City, Philippines', year: '2005 – 2011' }
        ],
        skills: [
        ...
        references: [
          { name: 'Dr. Anna Marie C. Reyes', position: 'Chief Nurse | Muntinlupa Medical Center', contact: '09187654321' },
          { name: 'Mrs. Maria Lourdes A. Lopez', position: 'Head Nurse | Muntinlupa Medical Center', contact: '09176543210' },
          { name: 'Dr. Juan Miguel B. Dela Cruz', position: 'Medical Director | Muntinlupa Medical Center', contact: '09085432109' }
        ]
      },
      
      // ======================== ENGINEER ========================
      engineer: {
        summary: `Licensed Civil Engineer with over 3 years of experience in construction project management, structural design, and site supervision. Proven track record of delivering projects on time and within budget while maintaining quality and safety standards. Skilled in AutoCAD, STAAD Pro, and project management software. Committed to sustainable and innovative engineering solutions that benefit communities and stakeholders.`,
        personal: {
          gender: 'Male',
          nationality: 'Filipino',
        ...
        experiences: [
          {
            ...
            company: 'D.M. Consunji Inc. (DMCI) – Makati City',
            years: '2021 – Present',
            responsibilities: [
              'Managed construction of mid-rise residential buildings with total project value of ₱500 million.',
              'Prepared and reviewed structural designs, blueprints, and construction drawings using AutoCAD and STAAD Pro.',
              'Conducted site inspections and quality control checks to ensure compliance with specifications and safety standards.',
              'Coordinated with architects, contractors, and stakeholders to ensure smooth project execution.',
              'Monitored project progress, prepared progress reports, and managed project documentation.'
            ]
          },
          {
            title: 'Junior Engineer',
            company: 'Mega Builders Inc. – Alabang, Muntinlupa City',
            years: '2019 – 2021',
            responsibilities: [
              'Assisted in the construction of commercial buildings and residential subdivisions.',
              'Supervised field activities and ensured proper implementation of project plans.',
              'Prepared quantity estimates and material take-offs for project costing.',
              'Conducted regular site inspections and prepared daily progress reports.',
              'Collaborated with senior engineers in preparing project schedules and resource allocation.'
            ]
          }
        ],
        education: [
          { level: 'Bachelor of Science in Civil Engineering', school: 'University of the Philippines – Diliman', year: '2019' },
          { level: 'Senior High School', school: 'Muntinlupa National High School – Muntinlupa City', year: '2012 – 2014' },
          { level: 'Muntinlupa National High School (MNHS)', school: 'Muntinlupa City, Philippines', year: '2008 – 2012' },
          { level: 'Muntinlupa Elementary School (MES)', school: 'Muntinlupa City, Philippines', year: '2002 – 2008' }
        ],
        skills: [
        ...
        references: [
          { name: 'Engr. Rafael M. Cruz', position: 'Project Manager | D.M. Consunji Inc.', contact: '09171234567' },
          { name: 'Engr. Maria A. Santos', position: 'Senior Engineer | Mega Builders Inc.', contact: '09282345678' },
          { name: 'Arch. Juan P. Garcia', position: 'Architect | Garcia & Associates', contact: '09093456789' }
        ]
      },
      
      // ======================== ACCOUNTANT ========================
      accountant: {
        summary: `Certified Public Accountant (CPA) with over 4 years of experience in financial accounting, auditing, and tax compliance. Proven ability to manage financial records, prepare accurate financial statements, and ensure regulatory compliance. Skilled in QuickBooks, SAP, and Microsoft Excel. Detail-oriented and analytical with a strong commitment to accuracy and integrity in financial reporting.`,
        personal: {
          gender: 'Female',
          nationality: 'Filipino',
        ...
        experiences: [
          {
            ...
            company: 'SGV & Co. – Makati City',
            years: '2021 – Present',
            responsibilities: [
              'Prepared and reviewed financial statements, including balance sheets, income statements, and cash flow reports.',
              'Conducted external audits for 15+ corporate clients across various industries.',
              'Assisted in tax planning and compliance, ensuring timely filing of BIR returns.',
              'Performed financial analysis and identified areas for cost reduction and efficiency improvement.',
              'Prepared audit reports and presented findings to clients and management.'
            ]
          },
          {
            title: 'Accounting Assistant',
            company: 'Mabex Manufacturing Inc. – Muntinlupa City',
            years: '2019 – 2020',
            responsibilities: [
              'Processed accounts payable and receivable, ensuring timely payment and collection of invoices.',
              'Maintained accurate general ledger entries and reconciled bank statements.',
              'Assisted in preparation of budgets and financial forecasts.',
              'Managed inventory costing and reported on inventory valuation.',
              'Supported the accounting team in daily operations and special projects.'
            ]
          }
        ],
        education: [
          { level: 'Bachelor of Science in Accountancy', school: 'Pamantasan ng Lungsod ng Muntinlupa', year: '2019' },
          { level: 'Senior High School', school: 'Muntinlupa National High School – Muntinlupa City', year: '2013 – 2015' },
          { level: 'Muntinlupa National High School (MNHS)', school: 'Muntinlupa City, Philippines', year: '2009 – 2013' },
          { level: 'Muntinlupa Elementary School (MES)', school: 'Muntinlupa City, Philippines', year: '2003 – 2009' }
        ],
        skills: [
          'Financial Accounting & Reporting',
          'External & Internal Auditing',
          'Tax Compliance & BIR Regulations',
          'QuickBooks, SAP, and Excel',
          'Analytical Thinking & Problem-Solving',
          'Attention to Detail & Time Management'
        ],
        references: [
          { name: 'Atty. Ramon V. Dela Cruz', position: 'Audit Partner | SGV & Co.', contact: '09178889999' },
          { name: 'Ms. Czerina Mae R. Espenesin', position: 'Accounting Manager | Mabex Manufacturing Inc.', contact: '09928196248' },
          { name: 'Mr. Jose M. Santos', position: 'Senior Accountant | SGV & Co.', contact: '09287776666' }
        ]
      },
      
      // ======================== CALL CENTER ========================
      callcenter: {
        summary: `Highly motivated Customer Service Representative with over 3 years of experience in BPO industry. Proven ability to handle high-volume inbound calls, resolve customer concerns, and provide exceptional service. Skilled in communication, problem-solving, and CRM systems. Adept at de-escalating situations and maintaining professionalism under pressure. Seeking to contribute customer service expertise to a dynamic BPO organization.`,
        personal: {
          gender: 'Female',
          nationality: 'Filipino',
        ...
        experiences: [
          {
            ...
            company: 'Telus International Philippines – BGC, Taguig City',
            years: '2021 – Present',
            responsibilities: [
              'Handled 60-80 inbound calls daily from US-based customers regarding billing inquiries, technical issues, and account concerns.',
              'Resolved customer concerns efficiently, achieving a 95% customer satisfaction rating.',
              'Documented customer interactions and maintained accurate records in the CRM system (Salesforce).',
              'De-escalated irate customers and converted complaints into positive service experiences.',
              'Achieved monthly metrics including 100% quality assurance score and 98% attendance rate.'
            ]
          },
          {
            title: 'Technical Support Representative',
            company: 'Concentrix Philippines – Alabang, Muntinlupa City',
            years: '2019 – 2021',
            responsibilities: [
              'Provided technical support for internet and cable services to US customers via phone and chat.',
              'Troubleshot hardware and software issues, and guided customers through resolution steps.',
              'Escalated complex technical issues to Level 2 support teams.',
              'Consistently met and exceeded key performance indicators (KPIs) for call handling and resolution time.',
              'Received "Rising Star" award for outstanding performance in Q4 2020.'
            ]
          }
        ],
        education: [
          { level: 'Bachelor of Arts in Communication', school: 'Pamantasan ng Lungsod ng Muntinlupa', year: '2019' },
          { level: 'Senior High School', school: 'Muntinlupa National High School – Muntinlupa City', year: '2013 – 2015' },
          { level: 'Muntinlupa National High School (MNHS)', school: 'Muntinlupa City, Philippines', year: '2009 – 2013' },
          { level: 'Muntinlupa Elementary School (MES)', school: 'Muntinlupa City, Philippines', year: '2003 – 2009' }
        ],
        ...
        references: [
          { name: 'Mr. John Patrick R. Santos', position: 'Team Lead | Telus International Philippines', contact: '09171234567' },
          { name: 'Ms. Maria Theresa C. Gomez', position: 'Training Manager | Concentrix Philippines', contact: '09887654321' },
          { name: 'Mr. Carlo A. Reyes', position: 'Operations Manager | Telus International Philippines', contact: '09276543210' }
        ]
      },
      
      // ======================== MANAGER ========================
      manager: {
        summary: `Results-driven and experienced Supervisor with over 6 years of proven leadership in managing cross-functional teams and driving operational excellence. Demonstrated ability to enhance productivity, streamline processes, and foster a culture of accountability and continuous improvement. Skilled in strategic planning, resource allocation, and conflict resolution with a strong commitment to achieving organizational objectives. Seeking to leverage leadership expertise to contribute to the success of a dynamic and forward-thinking organization.`,
        personal: {
          gender: 'Male',
          nationality: 'Filipino',
        ...
        experiences: [
          ...
          {
            title: 'Team Leader',
            company: 'XYZ Company – Pasig City',
            years: '2017 – 2019',
            responsibilities: [
        ...
        education: [
          { level: 'Bachelor of Science in Business Administration', school: 'University of Santo Tomas', year: '2014' },
          { level: 'Senior High School', school: 'Muntinlupa National High School – Muntinlupa City', year: '2010 – 2012' },
          { level: 'Muntinlupa National High School (MNHS)', school: 'Muntinlupa City, Philippines', year: '2006 – 2010' },
          { level: 'Muntinlupa Elementary School (MES)', school: 'Muntinlupa City, Philippines', year: '2000 – 2006' }
        ],
        skills: [
          'Team Leadership & Performance Management',
          'Strategic Planning & Operations Management',
          'Staff Training, Development & Coaching',
          'Conflict Resolution & Problem-Solving',
          'Excellent Communication & Interpersonal Skills',
          'Data Analysis & Reporting'
        ],
        references: [
          { name: 'Mr. Juan Miguel A. Dela Cruz', position: 'General Manager | ABC Corporation', contact: '09171234567' },
          { name: 'Ms. Maria Lourdes C. Santos', position: 'HR Director | ABC Corporation', contact: '09182345678' },
          { name: 'Mr. Jose Rizal M. Garcia', position: 'Senior Operations Manager | XYZ Company', contact: '09273456789' }
        ]
      },
      
      // ======================== SECRETARY ========================
      secretary: {
        summary: `Highly organized and detail-oriented Administrative Assistant / Secretary with over 4 years of experience in providing comprehensive administrative support to executives and teams. Skilled in office management, scheduling, document preparation, and communication. Proficient in Microsoft Office Suite and Google Workspace. Committed to ensuring smooth office operations and contributing to organizational efficiency.`,
        personal: {
          gender: 'Female',
          nationality: 'Filipino',
        ...
        experiences: [
          {
            ...
            company: 'Mega Corporation – Alabang, Muntinlupa City',
            years: '2021 – Present',
            responsibilities: [
              'Managed executive calendar, scheduled meetings, and coordinated travel arrangements for C-suite executives.',
              'Prepared and edited correspondence, reports, presentations, and meeting minutes with high accuracy.',
              'Screened and directed phone calls and emails, ensuring timely and appropriate responses.',
              'Coordinated and organized corporate events, seminars, and team-building activities.',
              'Maintained confidential files and records, ensuring compliance with data privacy policies.'
            ]
          },
          {
            title: 'Administrative Assistant',
            company: 'San Miguel Corporation – Mandaluyong City',
            years: '2019 – 2021',
            responsibilities: [
              'Provided administrative support to the HR department, including document processing and filing.',
              'Assisted in recruitment activities including scheduling interviews and preparing onboarding materials.',
              'Managed office supplies inventory and placed orders to ensure uninterrupted operations.',
              'Handled incoming and outgoing communications and maintained filing system.',
              'Supported finance team with expense reporting and invoice processing.'
            ]
          }
        ],
        education: [
          { level: 'Bachelor of Science in Office Administration', school: 'Pamantasan ng Lungsod ng Muntinlupa', year: '2019' },
          { level: 'Senior High School', school: 'Muntinlupa National High School – Muntinlupa City', year: '2013 – 2015' },
          { level: 'Muntinlupa National High School (MNHS)', school: 'Muntinlupa City, Philippines', year: '2009 – 2013' },
          { level: 'Muntinlupa Elementary School (MES)', school: 'Muntinlupa City, Philippines', year: '2003 – 2009' }
        ],
        skills: [
          ...
          'Travel Arrangement & Event Planning',
          'Records Management & Data Privacy'
        ],
        references: [
          { name: 'Mrs. Maria Theresa A. Santos', position: 'HR Manager | Mega Corporation', contact: '09172345678' },
          { name: 'Mr. Jose Paolo C. Garcia', position: 'Operations Manager | San Miguel Corporation', contact: '09283456789' },
          { name: 'Ms. Anna Marie B. Lopez', position: 'Senior Executive Assistant | Mega Corporation', contact: '09184567890' }
        ]
      },
      
      // ======================== COOK ========================
      cook: {
        summary: `Skilled and passionate Cook with over 4 years of experience in commercial kitchens and food service establishments. Proven ability to prepare high-quality dishes, maintain kitchen hygiene, and collaborate with team members. Knowledgeable in food safety regulations, inventory management, and menu planning. Committed to delivering delicious and consistent food that exceeds customer expectations.`,
        personal: {
          gender: 'Male',
          nationality: 'Filipino',
        ...
        experiences: [
          {
            title: 'Line Cook',
            company: 'The Aristocrat Restaurant – Malate, Manila',
            years: '2021 – Present',
            responsibilities: [
              'Prepared and cooked a variety of dishes according to established recipes and standards.',
              'Maintained cleanliness and organization of the kitchen, ensuring compliance with food safety regulations.',
              'Assisted in menu planning, recipe development, and food costing.',
              'Managed inventory and ordered supplies to ensure adequate stock levels.',
              'Trained and supervised junior cooks and kitchen staff.'
            ]
          },
          {
            title: 'Cook',
            company: 'Max\'s Restaurant – Alabang, Muntinlupa City',
            years: '2019 – 2021',
            responsibilities: [
              'Prepared signature dishes and maintained quality standards in a fast-paced restaurant environment.',
              'Ensured timely food preparation and plating for dine-in, take-out, and delivery orders.',
              'Maintained kitchen cleanliness and followed food safety and sanitation guidelines.',
              'Assisted in receiving and storing deliveries, ensuring quality and freshness of ingredients.'
            ]
          }
        ],
        education: [
          { level: 'Certificate in Culinary Arts', school: 'International School for Culinary Arts', year: '2019' },
          { level: 'Senior High School', school: 'Muntinlupa National High School – Muntinlupa City', year: '2011 – 2013' },
          { level: 'Muntinlupa National High School (MNHS)', school: 'Muntinlupa City, Philippines', year: '2007 – 2011' },
          { level: 'Muntinlupa Elementary School (MES)', school: 'Muntinlupa City, Philippines', year: '2001 – 2007' }
        ],
        skills: [
        ...
        references: [
          { name: 'Chef Ramon A. Santos', position: 'Head Chef | The Aristocrat Restaurant', contact: '09171234567' },
          { name: 'Mr. Juan M. Garcia', position: 'Kitchen Manager | Max\'s Restaurant', contact: '09282345678' },
          { name: 'Ms. Maria L. Lopez', position: 'Restaurant Manager | The Aristocrat Restaurant', contact: '09183456789' }
        ]
      },
      
      // ======================== DRIVER ========================
      driver: {
        summary: `Reliable and experienced Professional Driver with over 5 years of experience in transporting goods and passengers safely and efficiently. Possesses a valid Professional Driver's License with clean driving record. Skilled in vehicle maintenance, route planning, and logistics coordination. Committed to safety, punctuality, and excellent customer service. Seeking to contribute driving expertise to a reputable transportation company.`,
        personal: {
          gender: 'Male',
          nationality: 'Filipino',
        ...
        experiences: [
          {
            ...
            company: 'LBC Express Inc. – Pasay City',
            years: '2021 – Present',
            responsibilities: [
              'Delivered packages and documents to various locations within Metro Manila, ensuring timely and secure delivery.',
              'Planned efficient routes to optimize delivery time and fuel consumption.',
              'Performed routine vehicle inspections and maintenance to ensure safety and reliability.',
              'Maintained accurate delivery records and processed customer signatures and payments.',
              'Achieved 100% on-time delivery rate for 12 consecutive months.'
            ]
          },
          {
            ...
            years: '2019 – 2021',
            responsibilities: [
              'Provided transportation services for executives and employees to meetings, appointments, and events.',
              'Maintained vehicle cleanliness and ensured proper vehicle maintenance and repairs.',
              'Assisted in logistics and errands, including delivery of documents and office supplies.',
              'Ensured compliance with traffic rules and safety regulations at all times.',
              'Maintained accurate mileage and trip logs for fuel and expense reporting.'
            ]
          }
        ],
        education: [
          { level: 'Senior High School', school: 'Muntinlupa National High School – Muntinlupa City', year: '2008' },
          { level: 'Muntinlupa National High School (MNHS)', school: 'Muntinlupa City, Philippines', year: '2004 – 2008' },
          { level: 'Muntinlupa Elementary School (MES)', school: 'Muntinlupa City, Philippines', year: '1998 – 2004' }
        ],
        skills: [
          'Professional Driving (Automatic & Manual)',
          'Route Planning & Navigation',
          'Vehicle Maintenance & Inspection',
          'Logistics & Delivery Coordination',
          'Safety Compliance & Defensive Driving',
          'Customer Service & Communication'
        ],
        references: [
          { name: 'Mr. Jose R. Dela Cruz', position: 'Fleet Manager | LBC Express Inc.', contact: '09171234567' },
          { name: 'Ms. Maria C. Santos', position: 'Operations Manager | Mega Corporation', contact: '09282345678' },
          { name: 'Mr. Juan A. Garcia', position: 'Logistics Supervisor | LBC Express Inc.', contact: '09183456789' }
        ]
      },
      
      // ======================== SECURITY ========================
      security: {
        summary: `Vigilant and dependable Security Guard with over 5 years of experience in protecting properties, assets, and personnel. Skilled in surveillance, access control, and emergency response procedures. Committed to maintaining a safe and secure environment through proactive observation and effective communication. Possesses strong attention to detail and the ability to handle stressful situations calmly. Seeking to apply security expertise in a challenging and dynamic environment.`,
        personal: {
          gender: 'Male',
          nationality: 'Filipino',
        ...
        experiences: [
          {
            ...
            company: 'SecurePro Security Services – Pasay City',
            years: '2021 – Present',
            responsibilities: [
              'Patrolled premises regularly to ensure safety and prevent unauthorized access, maintaining a visible security presence.',
              'Monitored CCTV cameras and maintained detailed security logs to document activities and incidents.',
              'Responded promptly to alarms and emergency situations, taking appropriate action to ensure safety and security.',
              'Assisted visitors and employees with inquiries and ensured compliance with security protocols at all times.',
              'Prepared incident reports and coordinated with law enforcement when necessary to resolve security issues.'
            ]
          },
          {
            title: 'Security Officer',
            company: 'SafeGuard Security Agency – Makati City',
            years: '2019 – 2020',
            responsibilities: [
              'Managed access control systems and verified identification of personnel and visitors to ensure authorized entry.',
              'Conducted routine inspections of facilities to identify security vulnerabilities and recommend improvements.',
              'Enforced company policies and safety regulations to ensure a safe and secure environment for all.',
              'Provided first aid and emergency assistance as needed, demonstrating readiness and competence in crisis situations.'
            ]
          }
        ],
        education: [
          { level: 'Senior High School', school: 'Muntinlupa National High School – Muntinlupa City', year: '2006' },
          { level: 'Muntinlupa National High School (MNHS)', school: 'Muntinlupa City, Philippines', year: '2002 – 2006' },
          { level: 'Muntinlupa Elementary School (MES)', school: 'Muntinlupa City, Philippines', year: '1996 – 2002' }
        ],
        skills: [
        ...
        references: [
          { name: 'Mr. Ramon M. Mendoza', position: 'Chief Security Officer | SecurePro Security Services', contact: '09171234567' },
          { name: 'Mr. Luis T. Torres', position: 'Operations Manager | SafeGuard Security Agency', contact: '09282345678' },
          { name: 'Ms. Carla S. Santos', position: 'Security Supervisor | SecurePro Security Services', contact: '09183456789' }
        ]
      },
      
      // ======================== JANITOR ========================
      janitor: {
        summary: `Hardworking and trustworthy Janitor / Cleaner with over 4 years of experience in maintaining cleanliness and sanitation in commercial, residential, and industrial settings. Proven ability to perform cleaning duties efficiently, handle cleaning equipment safely, and ensure compliance with health and safety standards. Detail-oriented and reliable with a strong work ethic and commitment to providing a clean and safe environment.`,
        personal: {
          gender: 'Male',
          nationality: 'Filipino',
        ...
        experiences: [
          {
            ...
            company: 'CleanMaster Services – Makati City',
            years: '2021 – Present',
            responsibilities: [
              'Performed daily cleaning and sanitation of offices, restrooms, and common areas to maintain high standards of hygiene.',
              'Swept, mopped, and vacuumed floors to ensure cleanliness and safety of all work areas.',
              'Collected and disposed of waste materials properly in accordance with environmental guidelines.',
              'Monitored and replenished cleaning supplies and equipment to ensure uninterrupted operations.',
              'Reported maintenance issues promptly and coordinated with supervisors to ensure timely repairs and resolution.'
            ]
          },
          {
            title: 'Cleaning Staff',
            company: 'Sparkle Clean Corp. – Pasig City',
            years: '2019 – 2021',
            responsibilities: [
              'Cleaned and sanitized surfaces, windows, and fixtures to maintain a hygienic and pleasant environment.',
              'Assisted in setting up and cleaning up for events and meetings, ensuring facilities were prepared and well-maintained.',
              'Followed safety protocols and used cleaning chemicals properly to ensure safety and compliance.',
              'Maintained inventory of cleaning supplies and requested orders when necessary to avoid shortages.'
            ]
          }
        ],
        education: [
          { level: 'Senior High School', school: 'Muntinlupa National High School – Muntinlupa City', year: '2010' },
          { level: 'Muntinlupa National High School (MNHS)', school: 'Muntinlupa City, Philippines', year: '2006 – 2010' },
          { level: 'Muntinlupa Elementary School (MES)', school: 'Muntinlupa City, Philippines', year: '2000 – 2006' }
        ],
        skills: [
        ...
        references: [
          { name: 'Ms. Maria G. Gonzales', position: 'Operations Manager | CleanMaster Services', contact: '09171234567' },
          { name: 'Mr. Jose C. Cruz', position: 'Supervisor | Sparkle Clean Corp.', contact: '09282345678' },
          { name: 'Mrs. Ana R. Reyes', position: 'Property Manager | Green Valley Residences', contact: '09183456789' }
        ]
      },
      
      // ======================== LABORER ========================
      laborer: {
        summary: `Hardworking and dependable Laborer with over 4 years of hands-on experience in construction, warehouse operations, and general labor tasks. Demonstrates exceptional physical stamina, reliability, and a strong commitment to workplace safety. Proficient in operating hand and power tools, following safety protocols, and collaborating effectively with team members to complete projects on time and within budget. Seeking to contribute dedication and skills to a reputable organization that values hard work and safety.`,
        personal: {
          gender: 'Male',
          nationality: 'Filipino',
        ...
        experiences: [
          {
            ...
            company: 'Mega Builders Inc. – Muntinlupa City',
            years: '2021 – Present',
            responsibilities: [
              'Performed a wide range of construction tasks including excavation, loading, material handling, and site preparation to support project timelines.',
              'Assisted in the installation of structural frameworks, piping systems, and electrical components under the guidance of skilled tradespeople.',
              'Operated hand and power tools safely and efficiently, adhering to strict safety protocols and quality standards.',
              'Maintained a clean and hazard-free work site, conducting regular inspections to ensure compliance with occupational safety regulations.',
              'Collaborated effectively with team members to complete projects ahead of schedule while maintaining high-quality workmanship.'
            ]
          },
          {
            ...
            years: '2019 – 2020',
            responsibilities: [
              'Loaded and unloaded goods from delivery trucks, ensuring accurate inventory records and proper handling of materials.',
              'Organized and stored materials in designated warehouse locations to optimize space and facilitate easy retrieval.',
              'Prepared and packaged orders for shipment, verifying accuracy and quality to meet customer specifications.',
              'Conducted regular inventory audits, promptly identifying and resolving discrepancies to maintain accurate stock levels.'
            ]
          }
        ],
        education: [
          { level: 'Senior High School', school: 'Muntinlupa National High School – Muntinlupa City', year: '2009' },
          { level: 'Muntinlupa National High School (MNHS)', school: 'Muntinlupa City, Philippines', year: '2005 – 2009' },
          { level: 'Muntinlupa Elementary School (MES)', school: 'Muntinlupa City, Philippines', year: '1999 – 2005' }
        ],
        skills: [
        ...
        references: [
          { name: 'Mr. Ramon S. Santos', position: 'Site Supervisor | Mega Builders Inc.', contact: '09171234567' },
          { name: 'Mr. Carlos L. Lopez', position: 'Warehouse Manager | Logistics Hub Corp.', contact: '09282345678' },
          { name: 'Mr. Pedro R. Reyes', position: 'Project Manager | Mega Builders Inc.', contact: '09183456789' }
        ]
      },
      
      // ======================== IT ========================
      it: {
        summary: `Skilled and passionate Software Developer with over 3 years of experience in designing, developing, and maintaining web and mobile applications. Proficient in JavaScript, Python, HTML/CSS, React, and Node.js. Strong understanding of software development methodologies, database management, and API integration. Committed to writing clean, efficient code and delivering user-friendly solutions that solve real-world problems. Seeking to contribute technical expertise to an innovative and growth-oriented organization.`,
        personal: {
          gender: 'Male',
          nationality: 'Filipino',
        ...
        experiences: [
          {
            ...
            company: 'Tech Solutions Inc. – BGC, Taguig City',
            years: '2021 – Present',
            responsibilities: [
              'Designed and developed responsive web applications using React.js, Node.js, and PostgreSQL.',
              'Collaborated with cross-functional teams to gather requirements, design solutions, and deliver high-quality products.',
              'Implemented RESTful APIs and integrated third-party services to enhance application functionality.',
              'Conducted code reviews, debugging, and performance optimization to ensure application reliability and performance.',
          ...
          },
          {
            ...
            responsibilities: [
              ...
              'Developed and maintained internal tools and applications to automate business processes.',
              'Assisted in database management and system administration tasks.',
              'Conducted user training sessions to ensure effective use of IT systems and software.'
            ]
          }
        ],
        education: [
          { level: 'Bachelor of Science in Information Technology', school: 'Pamantasan ng Lungsod ng Muntinlupa', year: '2019' },
          { level: 'Senior High School', school: 'Muntinlupa National High School – Muntinlupa City', year: '2013 – 2015' },
          { level: 'Muntinlupa National High School (MNHS)', school: 'Muntinlupa City, Philippines', year: '2009 – 2013' },
          { level: 'Muntinlupa Elementary School (MES)', school: 'Muntinlupa City, Philippines', year: '2003 – 2009' }
        ],
        ...
        references: [
          { name: 'Mr. John David A. Santos', position: 'Senior Developer | Tech Solutions Inc.', contact: '09171234567' },
          { name: 'Ms. Maria C. Villanueva', position: 'IT Manager | Mega Corporation', contact: '09282345678' },
          { name: 'Mr. Jose R. Garcia', position: 'CTO | Tech Solutions Inc.', contact: '09183456789' }
        ]
      },
      
      // ======================== SALES ========================
      sales: {
        summary: `Dynamic and results-oriented Sales Representative with over 4 years of experience in driving sales growth and building strong customer relationships. Proven ability to identify customer needs, deliver compelling product presentations, and close deals effectively. Skilled in territory management, lead generation, and negotiation. Committed to achieving sales targets and contributing to business growth through exceptional customer service.`,
        personal: {
          gender: 'Female',
          nationality: 'Filipino',
        ...
        experiences: [
          {
            ...
            company: 'Globe Telecom Inc. – Makati City',
            years: '2021 – Present',
            responsibilities: [
              'Achieved 120% of sales targets consistently for 12 consecutive months.',
              'Generated new business through cold calling, networking, and referrals.',
              'Conducted product presentations and demonstrations to potential clients.',
              'Built and maintained strong relationships with key accounts and stakeholders.',
              'Prepared sales proposals, contracts, and reports for management.',
              'Provided after-sales support and resolved customer concerns to ensure satisfaction.'
            ]
          },
          {
            ...
            years: '2019 – 2020',
            responsibilities: [
              'Assisted in developing and implementing marketing campaigns and promotional activities.',
              'Conducted market research and analyzed competitor strategies to identify opportunities.',
              'Created marketing materials and managed social media platforms.',
              'Coordinated with advertising agencies and event organizers for brand activation.'
            ]
          }
        ],
        education: [
          { level: 'Bachelor of Science in Business Administration Major in Marketing', school: 'Pamantasan ng Lungsod ng Muntinlupa', year: '2018' },
          { level: 'Senior High School', school: 'Muntinlupa National High School – Muntinlupa City', year: '2012 – 2014' },
          { level: 'Muntinlupa National High School (MNHS)', school: 'Muntinlupa City, Philippines', year: '2008 – 2012' },
          { level: 'Muntinlupa Elementary School (MES)', school: 'Muntinlupa City, Philippines', year: '2002 – 2008' }
        ],
        ...
        references: [
          { name: 'Mr. Juan M. Santos', position: 'Sales Director | Globe Telecom Inc.', contact: '09171234567' },
          { name: 'Ms. Maria L. Garcia', position: 'Marketing Manager | Mega Corporation', contact: '09282345678' },
          { name: 'Mr. Jose R. Cruz', position: 'Regional Sales Manager | Globe Telecom Inc.', contact: '09183456789' }
        ]
      },
      
      // ======================== HR ========================
      hr: {
        summary: `Dedicated Human Resources professional with over 4 years of experience in talent acquisition, employee relations, and HR administration. Proven ability to manage end-to-end recruitment processes, facilitate employee engagement programs, and ensure compliance with labor laws. Skilled in communication, conflict resolution, and organizational development. Committed to fostering a positive workplace culture and supporting employee growth and development.`,
        personal: {
          gender: 'Female',
          nationality: 'Filipino',
        ...
        experiences: [
          {
            title: 'HR Assistant / Recruitment Specialist',
            company: 'Mega Corporation – Alabang, Muntinlupa City',
            years: '2021 – Present',
            responsibilities: [
              'Managed end-to-end recruitment processes for 50+ positions across various departments.',
              'Conducted job postings, resume screening, and initial interviews to identify qualified candidates.',
              'Facilitated new employee orientation and onboarding programs.',
              'Managed employee records and maintained HR database in compliance with data privacy regulations.',
              'Assisted in administering employee benefits, payroll, and performance management processes.',
              'Coordinated employee engagement activities and training programs.'
            ]
          },
          {
            title: 'HR Assistant',
            ...
            years: '2019 – 2021',
            responsibilities: [
              'Assisted in HR operations including recruitment, onboarding, and employee records management.',
              'Processed employee requests and provided HR-related information to employees.',
              'Coordinated company events, wellness programs, and team-building activities.',
              'Supported the HR team in policy implementation and employee relations matters.'
            ]
          }
        ],
        education: [
          { level: 'Bachelor of Science in Psychology', school: 'Pamantasan ng Lungsod ng Muntinlupa', year: '2018' },
          { level: 'Senior High School', school: 'Muntinlupa National High School – Muntinlupa City', year: '2012 – 2014' },
          { level: 'Muntinlupa National High School (MNHS)', school: 'Muntinlupa City, Philippines', year: '2008 – 2012' },
          { level: 'Muntinlupa Elementary School (MES)', school: 'Muntinlupa City, Philippines', year: '2002 – 2008' }
        ],
        ...
        references: [
          { name: 'Ms. Ma. Isabel C. Cruz', position: 'HR Manager | Mega Corporation', contact: '09171234567' },
          { name: 'Mr. Jose Paolo R. Garcia', position: 'HR Director | ABC Corporation', contact: '09282345678' },
          { name: 'Atty. Anna Marie S. Lopez', position: 'Legal Counsel | Mega Corporation', contact: '09183456789' }
        ]
      },
      
      // ======================== BARISTA ========================
      barista: {
        summary: `Passionate and skilled Barista with over 3 years of experience in specialty coffee shops. Proficient in espresso preparation, latte art, and customer service. Knowledgeable in coffee bean origins, brewing methods, and beverage preparation. Committed to delivering exceptional coffee experiences and creating a welcoming atmosphere. Seeking to contribute passion for coffee and customer service skills to a thriving café environment.`,
        personal: {
          gender: 'Female',
          nationality: 'Filipino',
        ...
        experiences: [
          {
            title: 'Barista',
            company: 'Starbucks Philippines – Alabang, Muntinlupa City',
            years: '2021 – Present',
            responsibilities: [
              'Prepared and served a variety of hot and cold beverages, including espresso, latte, and specialty drinks.',
              'Created latte art and customized beverages according to customer preferences.',
              'Greeted customers warmly and provided excellent customer service.',
              'Maintained cleanliness and organization of workstations and café areas.',
              'Assisted in inventory management and restocking of supplies.',
              'Trained new baristas on beverage preparation and customer service standards.'
            ]
          },
          {
            title: 'Barista / Cashier',
            company: 'Bo\'s Coffee – Alabang Town Center, Muntinlupa City',
            years: '2019 – 2020',
            responsibilities: [
              'Prepared and served coffee and non-coffee beverages with consistent quality.',
              'Operated POS system and processed customer payments accurately.',
              'Maintained a clean and organized work environment.',
              'Provided product recommendations and answered customer inquiries.'
            ]
          }
        ],
        education: [
          { level: 'Bachelor of Science in Hotel and Restaurant Management', school: 'Pamantasan ng Lungsod ng Muntinlupa', year: '2019' },
          { level: 'Senior High School', school: 'Muntinlupa National High School – Muntinlupa City', year: '2013 – 2015' },
          { level: 'Muntinlupa National High School (MNHS)', school: 'Muntinlupa City, Philippines', year: '2009 – 2013' },
          { level: 'Muntinlupa Elementary School (MES)', school: 'Muntinlupa City, Philippines', year: '2003 – 2009' }
        ],
        skills: [
          'Espresso Preparation & Brewing Methods',
          'Latte Art & Beverage Customization',
          'Customer Service & Communication',
        ...
        references: [
          { name: 'Mr. Juan M. Santos', position: 'Store Manager | Starbucks Philippines', contact: '09171234567' },
          { name: 'Ms. Maria C. Cruz', position: 'Assistant Store Manager | Bo\'s Coffee', contact: '09282345678' },
          { name: 'Mr. Jose R. Garcia', position: 'Regional Manager | Starbucks Philippines', contact: '09183456789' }
        ]
      },
      
      // ======================== RECEPTIONIST ========================
      receptionist: {
        summary: `Professional and welcoming Receptionist with over 3 years of experience in providing exceptional front desk service in corporate and hospitality settings. Skilled in managing phone calls, scheduling appointments, and greeting visitors with professionalism. Proficient in Microsoft Office, calendar management, and administrative support. Committed to creating a positive first impression and ensuring smooth office operations.`,
        personal: {
          gender: 'Female',
          nationality: 'Filipino',
        ...
        experiences: [
          {
            ...
            company: 'Marriott Hotel Manila – Pasay City',
            years: '2021 – Present',
            responsibilities: [
              'Greeted and welcomed guests with warmth and professionalism, ensuring a positive first impression.',
              'Managed check-in and check-out processes, processed payments, and handled guest inquiries.',
              'Coordinated with housekeeping and other departments to ensure guest satisfaction.',
              'Responded promptly to guest needs and resolved complaints to ensure a pleasant stay.',
              'Managed phone calls, reservations, and guest communications efficiently.'
            ]
          },
          {
            ...
            years: '2019 – 2021',
            responsibilities: [
              'Answered and directed phone calls to appropriate personnel and departments.',
              'Greeted visitors and provided assistance and directions as needed.',
              'Managed office mail, packages, and deliveries.',
        ...
        education: [
          { level: 'Bachelor of Science in Office Administration', school: 'Pamantasan ng Lungsod ng Muntinlupa', year: '2018' },
          { level: 'Senior High School', school: 'Muntinlupa National High School – Muntinlupa City', year: '2012 – 2014' },
          { level: 'Muntinlupa National High School (MNHS)', school: 'Muntinlupa City, Philippines', year: '2008 – 2012' },
          { level: 'Muntinlupa Elementary School (MES)', school: 'Muntinlupa City, Philippines', year: '2002 – 2008' }
        ],
        ...
        references: [
          { name: 'Ms. Maria L. Santos', position: 'Front Office Manager | Marriott Hotel Manila', contact: '09171234567' },
          { name: 'Mr. Jose R. Cruz', position: 'HR Manager | Mega Corporation', contact: '09282345678' },
          { name: 'Ms. Anna C. Reyes', position: 'Hotel Manager | Marriott Hotel Manila', contact: '09183456789' }
        ]
      }
    };
    
    // GET RESUME DATA OR USE DEFAULT (CASHIER)
    const resumeData = resumes[jobType] || resumes.cashier;
    
    // BUILD RESUME - EXACT FORMAT FROM SCREENSHOT
    // NO EXTRA LINES, NO ( ) PLACEHOLDERS, COMPLETE INFO
    let final = '';
    
    final += `PROFESSIONAL SUMMARY\n\n`;
    final += `${resumeData.summary}\n\n`;
    final += `PERSONAL PROFILE\n\n`;
    final += `- Gender: ${resumeData.personal.gender}\n`;
    final += `- Nationality: ${resumeData.personal.nationality}\n`;
    final += `- Date of Birth: ${resumeData.personal.birthdate}\n`;
    final += `- Religion: ${resumeData.personal.religion}\n`;
    final += `- Civil Status: ${resumeData.personal.status}\n`;
    final += `- Language Spoken: ${resumeData.personal.languages}\n\n`;
    final += `WORK EXPERIENCES\n\n`;
    
    for (const exp of resumeData.experiences) {
      final += `**${exp.title}**\n`;
      final += `${exp.company}\n`;
      if (exp.years) final += `${exp.years}\n`;
  ...
  buildResumePrompt(prompt, detectedLanguage) {
    ...
    final += `USER REQUEST: "${prompt}"\n\n`;
    final += `Create a COMPLETE RESUME template matching this exact format:\n\n`;
    final += `PROFESSIONAL SUMMARY\n\n`;
  ...
  buildResumeResponse(prompt, detectedLanguage) {
    ...
    final += `You are an AI assistant that creates COMPLETE and PROFESSIONAL RESUMES.\n\n`;
    final += `USER DETAILS: "${prompt}"\n\n`;
    final += `Create a COMPLETE RESUME using these details. Match this exact format:\n\n`;
    final += `PROFESSIONAL SUMMARY\n\n`;
    final += `(A strong professional summary based on user details)\n\n`;
    final += `PERSONAL PROFILE\n\n`;
    final += `- Gender: (Gender)\n`;
    final += `- Nationality: Filipino\n`;
    final += `- Date of Birth: (Birthdate)\n`;
  ...
  buildResumeImprovePrompt(prompt, previousResume, detectedLanguage) {
    ...
    final += `PROFESSIONAL SUMMARY\n\n`;
    final += `(Improved professional summary - 3 to 5 sentences)\n\n`;
    final += `PERSONAL PROFILE\n\n`;
    final += `- Gender: (Gender)\n`;
    final += `- Nationality: Filipino\n`;
    final += `- Date of Birth: (Birthdate)\n`;

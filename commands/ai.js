...
module.exports = {
  ...
  detectContentType(prompt) {
    ...
    // Speech requests
    if (lower.includes('speech') || lower.includes('talumpati') || 
        lower.includes('thank you speech') || lower.includes('pasasalamat na talumpati') ||
        lower.includes('speech tungkol') || lower.includes('speech about') ||
        lower.includes('speech para sa') || lower.includes('speech for')) {
      return 'speech';
    }
    
    // Letter requests
    if (lower.includes('letter') || lower.includes('liham') || 
        lower.includes('thank you letter') || lower.includes('pasasalamat na liham') ||
        lower.includes('open letter') || lower.includes('bukas na liham') ||
        lower.includes('love letter') || lower.includes('liham ng pag-ibig')) {
      return 'letter';
    }
    
    // Essay requests
    if (lower.includes('essay') || lower.includes('sanaysay') || 
        lower.includes('komposisyon') || lower.includes('composition')) {
      return 'essay';
    }
    
    // Poem requests
    if (lower.includes('poem') || lower.includes('tula') || 
        lower.includes('verse') || lower.includes('talata') ||
        lower.includes('poetry')) {
      return 'poem';
    }
    
    // Story requests
    if (lower.includes('story') || lower.includes('kwento') || 
        lower.includes('narrative') || lower.includes('salaysay') ||
        lower.includes('short story') || lower.includes('maikling kwento')) {
      return 'story';
    }
  ...
  buildSpeechPrompt(prompt, wantsDetailed, language = 'english') {
    ...
    const lower = prompt.toLowerCase();
    const isLong = lower.includes('long') || lower.includes('mahaba') || lower.includes('detailed') || lower.includes('complete');
    const isPowerful = lower.includes('powerful') || lower.includes('makapangyarihan') || lower.includes('strong') || lower.includes('emotional');
    const isThankful = lower.includes('thankful') || lower.includes('pasasalamat') || lower.includes('gratitude') || lower.includes('thanks');
    const isFuneral = lower.includes('funeral') || lower.includes('libing') || lower.includes('namatay') || lower.includes('death') || lower.includes('pumanaw') || lower.includes('nawala') || lower.includes('pagkawala');
    
    let speechType = 'GENERAL';
    if (isThankful) speechType = 'THANK YOU';
    if (isFuneral) speechType = 'FUNERAL / EULOGY';
    if (isPowerful) speechType = 'POWERFUL ' + speechType;
    if (isLong) speechType = 'LONG ' + speechType;
    
    final += `You are a speechwriter. Create a ${isPowerful ? 'POWERFUL and EMOTIONAL' : 'HEARTFELT'} ${isThankful ? 'THANK YOU' : ''} ${isFuneral ? 'FUNERAL / EULOGY' : ''} SPEECH in ${langName.toUpperCase()}.\n\n`;
    
    // Extract the topic/purpose
    let topic = prompt;
    const removeWords = ['create', 'make', 'write', 'speech', 'talumpati', 'thankful', 'pasasalamat', 'powerful', 'long', 'mahaba', 'detailed', 'about', 'tungkol sa', 'para sa', 'for', 'full', 'complete', 'please', 'paki', 'pakiusap'];
    ...
    if (isFuneral) {
      final += `This is a FUNERAL / EULOGY SPEECH. It should honor the deceased, express grief, and comfort the family.\n`;
    }
    
    if (isLong) {
      final += `📝 LENGTH: LONG and DETAILED (4-6 paragraphs or more).\n`;
      final += `- Include: Opening, body (with specific reasons for gratitude/remembrance), and closing.\n`;
      final += `- Add emotional depth and personal touches.\n`;
    } else {
      final += `📝 LENGTH: MODERATE (2-3 paragraphs).\n`;
    }
    ...
    if (isPowerful) {
      ...
      final += `💝 STYLE: HEARTFELT and SINCERE.\n`;
    ...
    final += `
📌 SPEECH STRUCTURE:
1. OPENING: Greet the audience and state the purpose of the speech.
2. BODY: 
   - Express gratitude to specific groups (family, friends, community, etc.)
   - Share memories or meaningful moments
   - Acknowledge the support and love received
   - Reflect on the significance of the occasion
   ${isFuneral ? '- Honor the life and legacy of the deceased\n   - Share cherished memories\n   - Offer comfort to the grieving family' : ''}
3. CLOSING: End with a powerful message, final thanks, or a meaningful quote.
...
- This is a complete speech, not a generic explanation.

NOW CREATE THE ${isPowerful ? 'POWERFUL' : 'HEARTFELT'} ${isThankful ? 'THANK YOU' : ''} ${isFuneral ? 'FUNERAL/EULOGY' : ''} SPEECH IN ${langName.toUpperCase()}:`;
  ...
  buildLetterPrompt(prompt, wantsDetailed, language = 'english') {
    ...
    const lower = prompt.toLowerCase();
    const isLong = lower.includes('long') || lower.includes('mahaba') || lower.includes('detailed');
    ...
    final += `You are a letter writer. Create a ${isLove ? 'LOVE' : ''} ${isThankful ? 'THANK YOU' : ''} ${isOpen ? 'OPEN' : ''} LETTER in ${langName.toUpperCase()}.\n\n`;
    ...
    if (isLong) {
      final += `📝 LENGTH: LONG and DETAILED (3-4 paragraphs).\n`;
    } else {
      final += `📝 LENGTH: MODERATE (2-3 paragraphs).\n`;
    }
...
3. BODY: 
   - Express the main message (gratitude, love, appreciation, etc.)
...
🎯 TONE: ${isLove ? 'Romantic and heartfelt' : isThankful ? 'Grateful and warm' : 'Sincere and personal'}.
...
NOW CREATE THE LETTER IN ${langName.toUpperCase()}:`;
...
USER QUESTION: ${userPrompt || 'Analyze this image'}`;
  ...
  // ================================================================
  // 🆕 IMPROVED: FINAL PROMPT BUILDER with content type detection
  // ================================================================

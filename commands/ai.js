...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // Check for specific commands first - this prevents false positives
      ...
      // SCHOLAR SEARCH - Make sure this is checked before general research
      if (this.isScholarCommand(prompt)) {
        await this.handleScholarSearch(senderId, prompt, token);
        return;
      }

      // Check for PowerPoint request BEFORE other text processing
      if (this.isPowerPointRequest(prompt)) {
        await this.handlePowerPointRequest(senderId, prompt, token, detectedLanguage);
        return;
      }

      // Check for Report request BEFORE other text processing
      if (this.isReportRequest(prompt)) {
        await this.handleReportRequest(senderId, prompt, token, detectedLanguage);
        return;
      }

      // Check for Resume request
      if (this.isResumeRequest(prompt)) {
        await this.handleResumeRequest(senderId, prompt, token, detectedLanguage);
        return;
      }

      // Check for Letter request
      if (this.isLetterRequest(prompt)) {
        await this.handleLetterRequest(senderId, prompt, token, detectedLanguage);
        return;
      }
  ...
  // New handler methods for specific requests
  
  async handlePowerPointRequest(senderId, prompt, token, detectedLanguage) {
    try {
      ...
      console.error('[PowerPoint] Error:', error.message);
      const errorMsg = detectedLanguage === 'tagalog' ? 'Error sa paggawa ng PowerPoint. Subukan muli.' :
                      detectedLanguage === 'bisaya' ? 'Error sa paghimo sa PowerPoint. Sulayi pag-usab.' :
                      'Error creating PowerPoint. Please try again.';
      await sendMessage(senderId, { text: errorMsg }, token);
    }
  },

  async handleReportRequest(senderId, prompt, token, detectedLanguage) {
    try {
      ...
      const errorMsg = detectedLanguage === 'tagalog' ? 'Error sa paggawa ng Report. Subukan muli.' :
                      detectedLanguage === 'bisaya' ? 'Error sa paghimo sa Report. Sulayi pag-usab.' :
                      'Error creating Report. Please try again.';
      await sendMessage(senderId, { text: errorMsg }, token);
    }
  },

  async handleResumeRequest(senderId, prompt, token, detectedLanguage) {
    try {
      const history = conversationHistory[senderId];
      let resumePrompt;
      ...
      const errorMsg = detectedLanguage === 'tagalog' ? 'Error sa paggawa ng Resume. Subukan muli.' :
                      detectedLanguage === 'bisaya' ? 'Error sa paghimo sa Resume. Sulayi pag-usab.' :
                      'Error creating Resume. Please try again.';
      await sendMessage(senderId, { text: errorMsg }, token);
    }
  },

  async handleLetterRequest(senderId, prompt, token, detectedLanguage) {
    try {
      let letterPrompt;
      ...
      const errorMsg = detectedLanguage === 'tagalog' ? 'Error sa paggawa ng sulat. Subukan muli.' :
                      detectedLanguage === 'bisaya' ? 'Error sa paghimo sa sulat. Sulayi pag-usab.' :
                      'Error creating letter. Please try again.';
      await sendMessage(senderId, { text: errorMsg }, token);
    }
  },

  // [Keep all the existing methods unchanged - correctTypos, detectLanguage, etc.]
  // I'll include them below but they remain the same as your original code
  ...
  isReportRequest(prompt) {
    ...
    // Exclude PowerPoint and other document types
    const exclude = ['powerpoint', 'ppt', 'slides', 'presentation', 'resume', 'cv', 'curriculum vitae'];
  ...
  buildReportPrompt(prompt, detectedLanguage) {
    ...
    if (detectedLanguage === 'tagalog' || detectedLanguage === 'bisaya' || detectedLanguage === 'cebuano') {
      ...
      final += 'Ang ulat ay dapat nasa ' + langName.toUpperCase() + ' na wika.\n\n';
      ...
      final += 'The report should be in ' + langName.toUpperCase() + ' language.\n\n';
      ...
      final += '- THE REPORT MUST BE IN ' + langName.toUpperCase() + ' LANGUAGE\n';
  ...
  isPowerPointRequest(prompt) {
    ...
    // Exclude report requests to avoid conflicts
    const exclude = ['report', 'reports', 'report about', 'ulat', 'gumawa ng report'];
    if (exclude.some(e => lower.includes(e) && !lower.includes('presentation'))) return false;
    return keywords.some(k => lower.includes(k));
  },
  ...
  isScholarCommand(prompt) {
    ...
    const lower = prompt.toLowerCase().trim();
    // Make sure it starts with the command, not just contains it
    return commands.some(cmd => lower.startsWith(cmd + ' ') || lower === cmd);
  },
  ...
  isLetterRequest(prompt) {
    const lower = prompt.toLowerCase();
    // More specific check - avoid catching "report" or "powerpoint"
    const keywords = [
      ...
      'letter for work', 'work application',
      'speech', 'graduation speech', 'wedding speech', 'eulogy', 'campaign speech',
      'talumpati', 'pagsasalita', 'speech for', 'speech about', 
      ...
      'love letter for', 'love letter to',
      'formal love letter', 'deep love letter',
    ...
    // Exclude PowerPoint and report to avoid conflicts
    const exclude = ['powerpoint', 'ppt', 'slides', 'presentation', 'report', 'reports', 'gumawa ng report', 'gumawa ng ulat'];
    if (exclude.some(e => lower.includes(e))) return false;
    return keywords.some(k => lower.includes(k));
  },
  ...
  isResumeRequest(prompt) {
    const lower = prompt.toLowerCase();
    // More specific check for resume
    const keywords = [
      ...
      'tapusin ang resume', 'strong objective', 'better objective',
      'mas magandang objective', 'gumawa ng objective',
      'best skills', 'best achievements', 'best references',
      'i-complete ang resume', 'enhance my resume', 'ayusin ang resume'
    ];
    // Exclude report, ppt, letter to avoid conflicts
    const exclude = ['report', 'reports', 'gumawa ng report', 'powerpoint', 'ppt', 'slides', 'presentation', 'letter', 'liham', 'sulat'];
    if (exclude.some(e => lower.includes(e))) return false;
    return keywords.some(k => lower.includes(k));
  },

  buildResumeByJob(prompt, detectedLanguage) {
    // [Keep the original buildResumeByJob code here - it's already in your original]
    // I'll include it but it's the same as before
    const lower = prompt.toLowerCase();
      ...
      jobExperiences = [
        {
          ...
          years: '2021 – 2025',
          responsibilities: [
            'Developed and implemented engaging lesson plans aligned with curriculum standards to meet diverse learning needs',
      ...
      jobExperiences = [
        {
          ...
          years: '2021 – 2025',
          responsibilities: [
            'Processed cash, credit, and digital payments accurately and efficiently, ensuring a smooth checkout experience for customers',
            'Greeted customers warmly and assisted with inquiries and purchases, providing excellent customer service',
            'Conducted opening and closing cash counts daily to ensure precise financial tracking and accountability',
            'Maintained a clean and organized checkout area to create a positive shopping environment',
            'Assisted customers with returns, exchanges, and resolved complaints professionally to ensure satisfaction'
          ]
        },
        {
          ...
          responsibilities: [
            ...
            'Assisted in inventory management and restocking of supplies to support seamless operations'
          ]
        }
      ];
      jobSkills = [
    ...
    final += 'IMPORTANT - PAPER AND FORMATTING DETAILS:\n';
    final += '- Bond Paper: Short (8.5 x 11 inches)\n';
    final += '- Font: Arial 11pt (body), Arial 14pt Bold (headers)\n';
    ...
    for (let i = 0; i < jobExperiences.length; i++) {
      const exp = jobExperiences[i];
      final += exp.title + '\n';
      final += exp.company + '\n';
      final += exp.years + '\n';
      for (let j = 0; j < exp.responsibilities.length; j++) {
        final += '- ' + exp.responsibilities[j] + '\n';
      }
      final += '\n';
    }
    ...
    for (let i = 0; i < jobEducation.length; i++) {
      const edu = jobEducation[i];
      final += edu.level + '\n';
      final += edu.school + '\n';
      final += edu.year + '\n\n';
    }
    ...
    for (let i = 0; i < jobReferences.length; i++) {
      const ref = jobReferences[i];
      final += ref.name + '\n';

...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // FIX 1: CHECK IF IT'S A QUESTION BEFORE TRIGGERING COMMANDS
      const isQuestion = this.isQuestion(prompt);
      const isCasual = this.isCasualConversation(prompt);

      // FIX 2: SCHOLAR - ONLY IF ASKING FOR ARTICLES, NOT ASKING ABOUT RESEARCH
  ...
  // ===== NEW FUNCTIONS FOR CONTEXTUAL DETECTION =====
  ...
  isQuestion(prompt) {
    ...
    const questionWords = [
      'ano', 'what', 'bakit', 'why', 'paano', 'how', 'kailan', 'when', 
      'saan', 'where', 'sino', 'who', 'alin', 'which', 'may tanong', 
      'question', 'ask', 'pwede', 'can', 'could', 'paki', 'please'
    ];
  ...
  isCasualConversation(prompt) {
    const lower = prompt.toLowerCase();
    const casualWords = [
      'hello', 'hi', 'hey', 'kamusta', 'musta', 'okay', 'ok', 'sige', 
      'cge', 'ge', 'yes', 'no', 'haha', 'hehe', 'lol', 'wow', 'shet',
      'gagi', 'grabe', 'sus', 'hay', 'ay', 'oh', 'ah', 'eh', 'hmm',
      'gets', 'nagets', 'gots', 'got it', 'sabi mo', 'talaga', 'really'
    ];
  ...
  isExplicitScholarRequest(prompt) {
    ...
    // NOT a scholar request if it's a question about research process
  ...
  isExplicitImageRequest(prompt) {
    ...
    // NOT image if it's a question
    if (this.isQuestion(prompt)) return false;
    
    // NOT image if it's about academic topics
    const academicWords = ['research', 'study', 'thesis', 'paper', 'article', 
      'abstract', 'analysis', 'data', 'literature', 'pananaliksik'];
  ...
  isExplicitRealtimeRequest(prompt) {
    ...
    // NOT realtime if it's about values/principles
    const nonTimeWords = ['ugali', 'prinsipyo', 'values', 'principles', 
      'character', 'morals', 'ethics', 'lahat ng oras', 'all the time'];
    if (nonTimeWords.some(w => lower.includes(w))) return false;
    
    // Only explicit time/news/weather/price requests
    const timePatterns = [

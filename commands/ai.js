...
module.exports = {
  name: ['ai', 'opera', 'ask', 'gemini', 'vision', 'gscholar', 'scholar', 'googlescholar', 'research', 'generate', 'image', 'img', 'show', 'analyze', 'summary', 'summarize'],
  ...
  getRandomLoadingMessage() {
    const messages = [
      'Let me analyze that for you! Give me a moment...',
      'Hmm, interesting! Let me think about that...',
      'Okay, basahin ko muna... Give me a sec!',
      'Ayan, basahin ko ng mabuti... Wait lang!',
      'Got it! Let me process this for you...',
      'Sige, intindihin ko muna... Sandali lang!',
      'Oh, this is interesting! Let me check...',
      'Wait lang, binabasa ko pa...',
      'Okay, let me take a look at this...',
      'Hmm, let me digest this first...',
      'Sige, aralin ko muna...',
      'Gotcha! Let me break this down...',
      'Wait, basahin ko ulit para sure...',
      'Interesting! Let me analyze this properly...',
      'Sandali lang, binabasa ko pa ang document...'
    ];
  ...
  getRandomGreeting() {
    const messages = [
      'Hi there!', 'Hello!', 'Hey!', 'Kamusta!', 'Kumusta ka!',
      'Hi! How can I help?', 'Hello! What can I do for you?',
      'Hey there!', 'Good to see you!', 'Hi friend!',
      'Mustaa!', 'Uy!', 'Oy!', 'Ayy!', 'Hoy!'
    ];
    return messages[Math.floor(Math.random() * messages.length)];
  },

  getRandomFriendlyCloser() {
    const messages = [
      'Hope that helps! Let me know if you need more. 😊',
      'Got questions? Just ask! I\'m here to help. 💪',
      'Yun lang! If may tanong ka pa, tanong lang!',
      'Sana nakatulong! If may iba pa, sabihin mo lang. ✨',
      'That\'s it! Anything else I can help with?',
      'Ayan! Kung may gusto ka pang malaman, tanong mo lang!',
      'Happy to help! Just say the word. 😊',
      'Sana nagets mo! If hindi, tanong lang ulit!',
      'That\'s the summary! Let me know if you want more details.',
      'Okay na yan! If may follow-up question, ask away!'
    ];
    return messages[Math.floor(Math.random() * messages.length)];
  },
  ...
  getRandomConfirmation() {
    const messages = [
      'Oh, gets ko na!', 'Ah, okay gets!', 'Ayy, naintindihan ko na!',
      'Ah, ganun pala!', 'Okay, nakuha ko na!', 'Ahhh, gets!',
      'I see!', 'Oh, I understand now!', 'Ah, that makes sense!',
      'Yun pala!', 'Ah, ganun!', 'Okay, gets na gets!',
      'Ah, oo naman!', 'Yun nga!', 'Tama!', 'Sakto!'
    ];
    return messages[Math.floor(Math.random() * messages.length)];
  },
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      if (event?.message?.reply_to?.mid) {
        ...
        if (replyData.message && this.isDocumentText(replyData.message)) {
          documentText = replyData.message;
        }
        if (!prompt) prompt = 'Please respond to what I said.';
      }

      if (prompt && this.isDocumentText(prompt)) {
        documentText = prompt;
      }

      if (this.isDocumentAnalysisRequest(prompt) || documentText || imageUrl) {
        await this.handleDocumentAnalysis(senderId, prompt, documentText, imageUrl, token);
        return;
      }
      ...
      if (!isReply && prompt) {
        ...
        if (history && history.lastResponse) {
          ...
          if (returnToTopic) {
            ...
            const isFollowUp = this.isFollowUpRequest(lowerPrompt) || this.isContextualQuestion(lowerPrompt, history.lastPrompt);
      ...
      if (!prompt && !isReply && !imageUrl) {
        await sendMessage(senderId, {
          text: 'Hello! I am Teacher Arlene - your friendly AI assistant!\n\nWhat I can do:\n✅ Chat with you naturally\n✅ Analyze documents\n✅ Answer questions\n✅ Generate images\n✅ Find songs and lyrics\n✅ Academic research\n✅ Real-time info\n✅ Translate\n✅ Summarize\n\nJust say what you need! 😊'
        }, token);
        return;
      }
  ...
  isDocumentAnalysisRequest(prompt) {
    if (!prompt) return false;
    const lower = prompt.toLowerCase();
    const keywords = [
      'summarize', 'summary', 'analyze', 'analysis', 'basa', 'basahin', 'intindi', 'unawa',
      'job order', 'contract', 'legal', 'document', 'paper', 'article', 'report',
      'employment', 'agreement', 'policy', 'memorandum', 'circular', 'order',
      'forfeited', 'termination', 'renewal', 'expiration', 'compulsory', 'retirement',
      'consanguinity', 'affinity', 'administrative offense', 'wellness leave',
      'government service', 'code of conduct', 'ogcc', 'pca'
    ];
    return keywords.some(k => lower.includes(k)) || this.isDocumentText(prompt);
  },

  isDocumentText(text) {
    if (!text) return false;
    const indicators = [
      'job order', 'contract', 'agreement', 'shall', 'hereby', 'whereas',
      'termination', 'expiration', 'renewal', 'forfeiture', 'consanguinity',
      'affinity', 'administrative offense', 'retirement', 'compulsory',
      'government service', 'code of conduct', 'memorandum circular',
      'wellness leave', 'ogcc', 'pca', 'group accident insurance'
    ];
    const lower = text.toLowerCase();
    const matches = indicators.filter(i => lower.includes(i));
    return matches.length >= 3 || (text.length > 200 && matches.length >= 1);
  },

  async handleDocumentAnalysis(senderId, prompt, documentText, imageUrl, token) {
    try {
      let textToAnalyze = documentText || prompt || '';

      const removeWords = ['summarize', 'summary', 'analyze', 'analysis', 'basa', 'basahin', 'intindi', 'unawa', 'document', 'contract', 'legal', 'job order'];
      for (const word of removeWords) {
        textToAnalyze = textToAnalyze.replace(new RegExp(word, 'gi'), '');
      }
      textToAnalyze = textToAnalyze.trim();

      if (imageUrl) {
        const imageAnalysis = await this.callGeminiAPI('Analyze this document/image and extract all text and information.', imageUrl);
        if (imageAnalysis && imageAnalysis.length > 50) {
          textToAnalyze = imageAnalysis + '\n\n' + textToAnalyze;
        }
      }
      ...
      if (!textToAnalyze || textToAnalyze.length < 20) {
        ...
        await sendMessage(senderId, {
          text: `${greeting} I can help you analyze documents! Just send me the text or a photo.\n\nExample: analyze: [paste your document text here]\n\nOr just send a photo and I'll read it for you! 😊`
        }, token);
        return;
      }

      const loadingMessage = this.getRandomLoadingMessage();
      await sendMessage(senderId, { text: loadingMessage }, token);

      const analysisPrompt = `You are a friendly, warm, and approachable document analyst. Someone just shared a document with you and wants to understand what it says. Talk to them like a friendly teacher or a trusted friend.

DOCUMENT TEXT:
${textToAnalyze}

Please help them understand this document in a clear, friendly, and conversational way.

IMPORTANT: Be HUMAN and NATURAL. Use casual language. Mix Tagalog and English naturally. Be warm and approachable.

Here's how to structure your response:

1. Start with a friendly greeting or acknowledgment. If they just shared something, acknowledge it warmly.

2. Give a simple, easy-to-understand summary of what the document is about. Use plain language.

3. Break down the key points in simple terms. Use bullet points but make it conversational.

4. Highlight any important clauses or things they should be careful about. Warn them gently if needed.

5. Mention dates, deadlines, and financial implications if any.

6. End with a friendly note and offer to explain further. Ask if they have questions.

Be helpful, patient, and conversational - like a teacher helping a student understand a complex document. Use simple words and avoid legal jargon when possible.

If this is a Job Order or government contract, specifically explain in simple terms:
- What kind of employment is this? (Is it government service or not?)
- When does it end? Can it be renewed?
- What happens if it gets terminated early?
- Do they have any benefits? (Leave, insurance, etc.)
- What are their obligations?
- Any financial risks? (Forfeiture, refunds, etc.)
- Is there anything they need to be careful about?

Make it clear, organized, but still warm and conversational. Use natural language. Be engaging. Ask if they have questions.

MIX LANGUAGES naturally. Use Tagalog, English, Bisaya - whatever feels natural in conversation. Make it sound like a real person talking, not a robot.`;

      const response = await this.callAPI(analysisPrompt, senderId);
      let cleanResponse = this.cleanResponse(response || 'Hmm, I had trouble analyzing this. Could you try again?');

      const now = new Date().toLocaleString('en-PH', { timeZone: 'Asia/Manila' });
      const closer = this.getRandomFriendlyCloser();
      cleanResponse += `\n\n---\nAnalyzed: ${now}\n\n${closer}`;
  ...
  isRealtimeQuestion(prompt) {
    if (!prompt) return false;
    if (this.isDocumentAnalysisRequest(prompt)) return false;
    ...
    const newsKeywords = ['balita', 'news', 'update', 'latest', 'pinakahuling', 'nangyari', 'happening', 'events', 'pangyayari', 'ganap'];
    if (newsKeywords.some(k => lower.includes(k)) && !this.isDocumentText(prompt)) {
      return true;
    }

    const weatherKeywords = ['panahon', 'weather', 'ulan', 'rain', 'bagyo', 'typhoon', 'init', 'heat', 'lamig', 'cold', 'baha', 'flood', 'lindol', 'earthquake'];
    if (weatherKeywords.some(k => lower.includes(k)) && !this.isDocumentText(prompt)) {
      return true;
    }

    const priceKeywords = ['presyo ng', 'price of', 'gastos', 'cost', 'bilihin', 'kuryente', 'electricity', 'tubig', 'water', 'gasolina', 'gas', 'bigas', 'rice'];
    if (priceKeywords.some(k => lower.includes(k)) && !this.isDocumentText(prompt)) {
      return true;
    }
  ...
  async handleRealtimeQuestion(senderId, prompt, token) {
    if (this.isDocumentText(prompt) || this.isDocumentAnalysisRequest(prompt)) {
      await this.handleDocumentAnalysis(senderId, prompt, null, null, token);
      return;
    }
  ...
  isMusicRequest(prompt) {
    const lower = prompt.toLowerCase();
    const keywords = ['play', 'song', 'music', 'track', 'audio', 'listen', 'sound', 'kanta', 'tugtog', 'music video', 'mv', 'soundtrack', 'playlist', 'album', 'single', 'remix', 'cover', 'official audio', 'official music', 'stream', 'pakinggan', 'patugtog', 'music link', 'song link'];
    return keywords.some(k => lower.includes(k));
  },
...
1. DOCUMENT / CONTRACT / JOB ORDER / LEGAL PAPER
   - Extract all text from the document
   - Identify the type of document
   - Provide a detailed summary
   - List all key provisions
   - Highlight important clauses
   - Format as structured analysis

2. ACTIVITY SHEET / WORKSHEET / QUIZ / HOMEWORK
...
3. MATH PROBLEMS / EQUATIONS
   - Read the problem carefully
   - Show step-by-step solution
   - Provide final answer with proper units

4. SCIENCE / DIAGRAMS / LABELS
...
- For DOCUMENTS: Be thorough and detailed
- For MEME and GENERAL: Keep it SHORT and DIRECT
  ...
  buildFinalPrompt(prompt, previousResponse, previousPrompt, isReply, wantsDetailed) {
    ...
    if (previousResponse) {
      ...
      if (this.isContextualQuestion(lower, previousPrompt)) {
        ...
        final += 'Continue the conversation naturally like a friendly teacher.\n\n';
      }

      if (this.isTranslationRequest(prompt)) {
        const lang = this.detectTargetLanguage(prompt);
        final += 'Translate the previous response to ' + lang + '.\n';
        ...
        final += 'Rewrite your previous response in a natural, conversational way.\n';
        final += 'Make it sound like a friendly teacher talking to a student.\n';
        final += 'Use simple words, add personality, be warm and engaging.\n\n';
        ...
        final += 'Provide a detailed explanation with more information and examples.\n';
        final += 'Make it comprehensive but still conversational.\n\n';
        ...
        final += 'Provide a concise summary of the key points.\n';
        final += 'Keep it short but clear and conversational.\n\n';
        ...
        final += 'Explain in simple terms using layman\'s language.\n';
        final += 'Make it easy to understand for someone with no background.\n\n';
        ...
        final += 'Use real-life examples that are easy to relate to.\n\n';
        ...
        final += 'Confirm or clarify your previous response.\n';
        final += 'Be reassuring and provide additional proof if needed.\n\n';
        ...
        final += 'Acknowledge the user\'s agreement positively.\n';
        final += 'Offer to provide more information if they want.\n\n';
        ...
        final += 'Acknowledge the user\'s concern respectfully.\n';
        final += 'Provide clarification or additional evidence.\n';
        final += 'Be understanding and helpful.\n\n';
      } else if (lower.includes('ulit') || lower.includes('repeat') || 
                 lower.includes('again') || lower.includes('paki-ulit')) {
        final += 'Repeat or re-explain your previous response in a clearer way.\n';
        final += 'Be patient and use simpler words this time.\n\n';
      } else if (lower.includes('gets') || lower.includes('nagets') || 
                 lower.includes('naintindihan') || lower.includes('understand') ||
                 lower.includes('gots')) {
        final += 'The user understands your previous response.\n';
        final += 'Respond positively and offer to help with anything else.\n\n';
      } else {
        final += 'Continue the conversation naturally.\n';
        final += 'The user says: ' + prompt + '\n';
        final += 'Respond as a friendly, helpful teacher.\n\n';
      }
    } else {
      final = prompt;
    }
    ...
    if (wantsDetailed) {
      final += 'Provide a detailed, comprehensive answer.\n';
      final += 'Include examples and thorough explanations.\n\n';
    } else {
      final += 'Provide a concise but complete answer.\n';
      final += 'Be direct but still conversational.\n\n';
    }

    final += 'GUIDELINES:\n';
    final += '- Be accurate and helpful.\n';
    final += '- Be conversational and friendly like a teacher.\n';
    final += '- Use natural language, not robotic.\n';
    final += '- Ask clarifying questions if needed to help better.\n';
    final += '- Show empathy and understanding.\n';
    final += '- Use examples to explain complex ideas.\n';
    final += '- Make the user feel comfortable asking questions.\n';
    final += '- If unsure, be honest and offer to help find answers.\n';
    final += '- Mix Tagalog, English, and Bisaya naturally.\n';
    final += '- Make it sound like a real person talking, not a robot.\n';

...
module.exports = {
  ...
  // ================================================================
  // 🆕 ENHANCED: CONTENT TYPE DETECTION (for speech/letter)
  // ================================================================
  detectContentType(prompt) {
    if (!prompt) return 'general';
    const lower = prompt.toLowerCase();
    
    // Speech-related keywords (Tagalog, Bisaya, English)
    const speechKeywords = [
      ...
      'pag-alala', 'pag-alamat', 'pagpupugay', 'pagkilala', 'paggalang',
      'thank you speech', 'message of gratitude', 'condolence message',
      'sympathy message', 'funeral speech', 'eulogy speech',
      'pamamaalam', 'pagpapaalam', 'pagdiriwang', 'paggunita'
    ];
    ...
    // Check for explicit speech or letter
    let hasSpeech = speechKeywords.some(k => lower.includes(k));
    let hasLetter = letterKeywords.some(k => lower.includes(k));
    
    // If both, decide based on which appears more or prioritize letter if 'liham' or 'sulat' present
    ...
    // If no explicit, check for action verbs that imply content creation
    ...
    if (hasCreate && hasContent) {
      // If there's a mention of 'sulat' or 'liham' in the content, it's a letter
      if (lower.includes('liham') || lower.includes('sulat') || lower.includes('letter')) {
        return 'letter';
      }
      // Default to speech for gratitude/condolence messages
      return 'speech';
    }
    
    // Also detect if the user is asking for a sample or example of a speech/letter
    if ((lower.includes('sample') || lower.includes('example') || lower.includes('halimbawa')) &&
        (lower.includes('speech') || lower.includes('talumpati') || lower.includes('liham') || lower.includes('letter'))) {
      return 'speech'; // default to speech if ambiguous
    }
  ...
  // ================================================================
  // 🆕 FIXED: BUILD SPEECH PROMPT (FLUENT & COMPLETE)
  // ================================================================
  buildSpeechPrompt(prompt, wantsDetailed, language = 'english') {
    ...
    // Allow mixed language (Tagalog/Bisaya/English)
    final += `
...
START WITH A PROPER GREETING AND MAKE IT COMPLETE AND LONG.`;
  ...
  // ================================================================
  // 🆕 FIXED: BUILD LETTER PROMPT (COMPLETE & LONG)
  // ================================================================

...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      // ======= FIX: PROPER PROMPT EXTRACTION =======
      let prompt = args.join(' ').trim();
      
      // If args is empty but we have message text, extract it
      if (!prompt && event?.message?.text) {
        const text = event.message.text || '';
        // Remove command prefixes if present
        const prefixes = ['ai ', 'ask ', 'gemini ', 'opera ', 'vision '];
        ...
        // If still empty, use everything after first word
        if (!prompt) {
          const words = text.split(' ');
          prompt = words.slice(1).join(' ').trim();
      ...
      // 1. Lyrics request
      if (prompt && this.isLyricsRequest(prompt)) {
        await this.handleLyricsSearch(senderId, prompt, token);
        return;
      }

      // 2. Realtime question
      if (prompt && this.isRealtimeQuestion(prompt)) {
        await this.handleRealtimeQuestion(senderId, prompt, token);
        return;
      }

      // 3. Generate/Image request
      if (prompt && (this.isGenerateCommand(prompt) || this.isImageRequest(prompt))) {
        await this.handleImageGeneration(senderId, prompt, token);
        return;
      }

      // 4. Music request
      if (prompt && this.isMusicRequest(prompt)) {
        await this.handleMusicSearch(senderId, prompt, token);
        return;
      }

      // 5. Scholar/Research request
      if (prompt && (this.isScholarCommand(prompt) || this.isResearchQuery(prompt))) {
      ...
      // 9. Final check before AI response
      ...
      // 10. Owner question
      if (prompt && this.isOwnerQuestion(prompt)) {
        await sendMessage(senderId, {
      ...
      // 11. User info question
      if (prompt && this.isUserInfoQuestion(prompt)) {

  ...
  async handleDocumentAnalysis(senderId, prompt, documentText, imageUrl, token) {
    try {
      ...
      if (!textToAnalyze || textToAnalyze.length < 20) {
        await sendMessage(senderId, {
          text: 'Hi! I can help you analyze documents. Just send me the text or a photo of the document.\n\nExample: analyze: [paste your document text here]\n\nOr just send a photo of the document and I will read it for you!'
        }, token);
        return;
      }

      await sendMessage(senderId, { text: 'Let me analyze that for you! Give me a moment...' }, token);

      const analysisPrompt = `You are a friendly and helpful document analyst. Someone just shared a document with you and wants to understand what it says.
...
1. Start with a warm greeting and a simple summary of what the document is about.

2. Break down the key points in simple, easy-to-understand language. Use bullet points but keep it conversational.

3. Highlight any important clauses or things they should be careful about.

4. Mention dates, deadlines, and financial implications if any.

5. End with a friendly note offering to explain further if they have questions.

6. Ask if they need clarification on anything specific.
...
If this is a Job Order or government contract, specifically explain:
...
Make it clear, organized, but still warm and conversational. Use natural language. Be engaging. Ask if they have questions.`;
      ...
      let cleanResponse = this.cleanResponse(response || 'I had trouble analyzing this document. Could you please try again?');

      const now = new Date().toLocaleString('en-PH', { timeZone: 'Asia/Manila' });
      cleanResponse += `\n\n---\nAnalyzed: ${now}\n\nIf you have any questions about this document, just ask!`;
      ...
      await sendMessage(senderId, {
        text: 'Oops! Something went wrong while analyzing the document. Could you please try again with a clearer text or image?'
      }, token);
    }
  },

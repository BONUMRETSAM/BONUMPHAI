...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      if (!imageUrl && event?.message?.attachments) {
        ...
        if (imageUrl && !cleanPrompt) cleanPrompt = 'Analyze this image.';
      }
      ...
      const wantsDetailed = this.wantsDetailedAnswer(cleanPrompt);
      let aiResponse = '';
  ...
  isCasualConversation(prompt) {
    ...
    // Check if it's a short casual message with no complex content
    const isShort = prompt.length < 30;
    const hasCasual = casualPhrases.some(p => lower.includes(p));
    
    // Also check if it's a simple conversational question
  ...
  localHumanize(response, originalPrompt) {
    ...
    // Add conversational prefixes based on context
    ...
    // If Filipino text, add Taglish flavor
    if (isFilipino) {
      humanized = this.addFilipinoFlavor(humanized);
    }

    // Add casual question endings for questions
    if (humanized.includes('?')) {
      const questionEndings = [' no?', ' ah?', ' ha?', ' ba?', ' diba?', ' talaga?', ' noh?', ' ahh?'];
      if (!questionEndings.some(e => humanized.endsWith(e))) {
        // Don't modify if already has ending
      }
    }

    // Add a friendly opening if it's a greeting
    ...
    // Add personality to very short responses
  ...
  addFilipinoFlavor(text) {
    ...
    // Add "ha", "diba", "no" at ends of questions
  ...
  addCasualFlavor(response) {
    ...
    // If response is very short or generic, add personality
  ...
  isFilipinoText(text) {
    ...
    return filipinoCount > words.length * 0.1; // At least 10% Filipino words
  },

  // ============ EXISTING METHODS WITH HUMANIZE INTEGRATION ============

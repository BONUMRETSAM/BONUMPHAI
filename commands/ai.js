  ...
  async handleDocumentAnalysis(senderId, prompt, documentText, imageUrl, token) {
    try {
      ...
      if (!textToAnalyze || textToAnalyze.length < 20) {
        await sendMessage(senderId, {
          text: 'Document Analysis\n\nPlease provide the document text or send an image of the document.\n\nExample:\nai summarize: [paste document text here]\n\nOr send a photo of the document.'
        }, token);
        return;
      }

      await sendMessage(senderId, { text: 'Analyzing document... Please wait.' }, token);

      const analysisPrompt = `You are a legal document analyst. Analyze this document and provide a clear, organized summary.
...
KEY POINTS
- [List all important provisions]
- [Use bullet points]
- [Be specific and accurate]

IMPORTANT CLAUSES
- [List critical clauses that affect the parties]
- [Include penalties, forfeitures, liabilities]
...
KEY TAKEAWAYS
- [What the parties should know]
- [Risks and obligations]
- [Important considerations]

RULES:
1. Be ACCURATE - quote exact terms when important
2. Be CLEAR - use simple language
3. Be ORGANIZED - use the format above
4. Be COMPLETE - cover all important provisions
5. Use PLAIN TEXT - no markdown or symbols

IMPORTANT: If this is a government document like a Job Order or Service Contract, specifically highlight:
- Employment status (is it government service or not)
...
- Approval requirements`;
      ...
      let cleanResponse = this.cleanResponse(response || 'Unable to analyze document. Please try again.');
      ...
      await sendMessage(senderId, {
        text: 'Error analyzing document. Please try again with a clearer text or image.'
      }, token);
    }
  },

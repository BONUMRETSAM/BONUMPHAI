...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // FIXED: Check commands more precisely - only trigger on exact command starts
      const commandWords = ['generate', 'image', 'img', 'show', 'gscholar', 'scholar', 'googlescholar', 'research', 'play', 'music', 'lyrics'];
      const normalizedPrompt = prompt.toLowerCase().trim();
      
      // Check if it's a command (starts with command word)
      const isExactCommand = commandWords.some(cmd => normalizedPrompt.startsWith(cmd + ' ') || normalizedPrompt === cmd);
      
      // Check if it's a lyrics request - only if it starts with lyrics or clearly asking for lyrics
      if (this.isExactLyricsRequest(prompt) || normalizedPrompt.startsWith('lyrics ')) {
        await this.handleLyricsSearch(senderId, prompt, token);
        return;
      }

      // Check if it's a scholar/research request - only if it starts with command or explicit research request
      if (this.isExactScholarRequest(prompt)) {
        await this.handleScholarSearch(senderId, prompt, token);
        return;
      }

      // Check if it's a generate/image request - only if it starts with command or explicit request
      if (this.isExactGenerateRequest(prompt)) {
        await this.handleImageGeneration(senderId, prompt, token);
        return;
      }

      // Check if it's a music request - only if it starts with play/music or explicit request
      if (this.isExactMusicRequest(prompt)) {
        await this.handleMusicSearch(senderId, prompt, token);
        return;
      }

      // Handle real-time questions - keep this as is
  ...
  // NEW: Exact command detection methods
  ...
  isExactGenerateRequest(prompt) {
    ...
    // Only trigger if it starts with these commands
  ...
  // MODIFIED: Less aggressive scholar detection
  ...
  // MODIFIED: Less aggressive research detection
  isResearchQuery(prompt) {
    const lower = prompt.toLowerCase().trim();
    // Only trigger on explicit research commands, not random mentions
    const explicitPatterns = [
  ...
  // MODIFIED: Less aggressive image detection
  ...
  // MODIFIED: Less aggressive music detection
  ...
  // MODIFIED: Less aggressive lyrics detection
  ...
  // MODIFIED: Less aggressive generate detection
  ...
  async handleImageGeneration(senderId, prompt, token) {
    ...
    // Remove explicit request phrases

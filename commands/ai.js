...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      if (imageUrl) {
        ...
        aiResponse = await this.callGeminiAPI(geminiPrompt, imageUrl);
        aiResponse = this.cleanImageResponse(aiResponse);
      } else {
        let finalPrompt = '';
      ...
      if (!aiResponse || aiResponse === 'No response.' || aiResponse === '') {
        aiResponse = 'I apologize, but I am having trouble generating a response right now. Please try again in a moment.';
      }
  ...
  cleanImageResponse(response) {
    if (!response) return response;
    
    let cleaned = response;
    
    cleaned = cleaned.replace(/^I'?m?\s+a?\s*Gemini.*?model.*?\n\n?/i, '');
    cleaned = cleaned.replace(/^Here is my analysis.*?\n/i, '');
    cleaned = cleaned.replace(/^Let me analyze.*?\n/i, '');
    cleaned = cleaned.replace(/^The image appears to be.*?\n/i, '');
    cleaned = cleaned.replace(/^Based on my analysis.*?\n/i, '');
    cleaned = cleaned.replace(/^I can see that.*?\n/i, '');
    cleaned = cleaned.replace(/^This looks like.*?\n/i, '');
    cleaned = cleaned.replace(/^I'?m?\s+an?\s+AI.*?\n/i, '');
  ...
  async callGeminiAPI(prompt, imageUrl) {
    try {
      ...
      processed = this.cleanImageResponse(processed);

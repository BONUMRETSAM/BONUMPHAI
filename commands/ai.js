...
module.exports = {
  ...
  isExplicitScholarRequest(prompt) {
    ...
    if (explicitCommands.some(pattern => pattern.test(lower))) {
      return true;
    }
    ...
    if (hasResearchWord) {
      const questionIndicators = ['?', 'ano', 'what', 'bakit', 'why', 'paano', 'how', 'kailan', 'when', 'saan', 'where', 'sino', 'who', 'may tanong', 'question', 'ask'];
      if (questionIndicators.some(ind => lower.includes(ind))) {
        return false;
      }
      
      const processKeywords = ['process', 'method', 'methodology', 'approach', 'steps', 'gumawa', 'gawin', 'create', 'write', 'sulat', 'abstract', 'introduction', 'conclusion', 'format', 'structure'];
  ...
  isExplicitImageRequest(prompt) {
    ...
    if (hasImageWord) {
      const questionIndicators = ['?', 'ano', 'what', 'bakit', 'why', 'paano', 'how', 'may tanong', 'question'];
      if (questionIndicators.some(ind => lower.includes(ind))) {
        return false;
      }
      
      const academicWords = ['research', 'study', 'paper', 'article', 'thesis', 'analysis', 'data'];
  ...
  isExplicitRealtimeRequest(prompt) {
    ...
    if (lower.includes('oras') && !timePatterns.some(p => p.test(lower))) {
      const nonTimeIndicators = ['ugali', 'prinsipyo', 'values', 'principles', 'character', 'morals', 'ethics'];
      if (nonTimeIndicators.some(w => lower.includes(w))) {
        return false;
      }
    }

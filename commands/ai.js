...
// MAX_CHUNK is defined at the top, but I'll keep the original constant
...
· STAY ON TOPIC.`;
  ...
  const pagePatterns = [/pp.?\s(\d+-\d+)/i, /pages?\s(\d+-\d+)/i, /(\d+-\d+)\spp/i, /(\d+-\d+)\s\(/i, /:\s*(\d+-\d+)/i];
  ...
  // ============================================================
  // FIXED: RESUME BUILDER WITH EXACT FORMAT FROM THE IMAGE
  // ============================================================
  ...
  if (lower.includes('cashier') || lower.includes('taga-cash') || lower.includes('retail')) {
  jobType = 'cashier';
  ...
  jobReferences = [
  { name: 'Angelika Mae Lopez', position: 'HR Assistant', company: '', contact: '09911451130' },
  { name: 'Czerina Mae Espenesin', position: 'Accounting Assistant | Mabex Manufacturing Inc.', company: '', contact: '09928196248' },
  { name: 'Menchie Donayre', position: 'Graphic Designer', company: '', contact: '09563892622' }
  ];
  } else if (lower.includes('teacher') || lower.includes('guro') || lower.includes('educator')) {
  jobType = 'teacher';
  jobTitle = 'Teacher / Educator';
  jobSummary = Dedicated and passionate Teacher with extensive experience in delivering engaging and effective instruction to diverse learners. Proven ability to create a positive and inclusive classroom environment that fosters academic growth and personal development. Skilled in curriculum planning, classroom management, and student assessment. Committed to inspiring students to reach their full potential and achieve academic success.;
  jobExperiences = [
  {
  title: 'Teacher',
  company: '(School Name) – (Location)',
  years: '(Year) – (Year)',
  responsibilities: [
  ...
  jobEducation = [
  { level: 'Bachelor of Elementary Education', school: '(University Name)', year: '(Year Graduated)' },
  { level: 'Senior High School', school: '(School Name)', year: '(Year Started) – (Year Graduated)' },
  ...
  jobReferences = [
  { name: '(Name)', position: 'School Principal', company: '(School Name)', contact: '(Contact Number)' },
  { name: '(Name)', position: 'Master Teacher', company: '(School Name)', contact: '(Contact Number)' },
  { name: '(Name)', position: 'Supervisor', company: '(Company)', contact: '(Contact Number)' }
  ];
  } else {
  // GENERAL RESUME - MATCHES THE IMAGE FORMAT
  jobType = 'general';
  ...
  // BUILD THE FINAL RESUME - EXACT FORMAT FROM THE IMAGE
  let final = '';
  ...
  final += ${ref.name}\n;
  if (ref.position) final += ${ref.position};
  if (ref.company) final +=  | ${ref.company};
  final += \n${ref.contact}\n\n;
  }
  final += ================================================================================\n\n;
  final += IMPORTANT RULES:\n;

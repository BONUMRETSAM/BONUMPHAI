...
module.exports = {
    ...
    try {
      ...
      if (!isReply && prompt) {
        ...
        if (history && history.lastResponse) {
          ...
          // Check for topic return requests
          ...
          // Store topic history if this is a new topic
      ...
      if (imageUrl) {
        ...
        // Build the final prompt with context and smart detection
      ...
      // ===== NEW: SHORTEN CASUAL CONVERSATIONS =====
  ...
  isCasualConversation(prompt) {
    ...
    // Common casual phrases and greetings
    const casualPatterns = [
      ...
      'kailan', 'when', 'sino', 'who', 'alin', 'which',
      'anu', 'bkt', 'pano', 'san', 'sinu', 'aln',
      
      // Casual statements about previous conversation
      ...
      'yes', 'no', 'ok', 'sige', 'cge', 'ge', 'y', 'n'
    ];

    // Check if the message contains casual patterns
    const isCasual = casualPatterns.some(pattern => lower.includes(pattern));
    
    // Very short messages (3 words or less) without formal indicators
    const wordCount = prompt.split(/\s+/).length;
    const hasFormalIndicators = this.isFormalQuestion(prompt);
    
    // If it's very short and not formal, it's casual
    if (wordCount <= 3 && !hasFormalIndicators) {
      return true;
    }

    // Check if it's a follow-up to a casual conversation
  ...
  isFormalQuestion(prompt) {
    ...
    // Keywords that indicate formal/academic questions
  ...
  buildSmartPrompt(
    ...
    // ===== ADD INSTRUCTIONS BASED ON RESPONSE TYPE =====
    final += '=== RESPONSE INSTRUCTIONS ===\n';
    final += `Response Type: ${responseType}\n`;
    final += `Detail Level: ${detailLevel}\n\n`;

    // CASUAL conversations - short, direct, friendly
    ...
    // ELABORATE responses - detailed and comprehensive
    ...
    // CLARIFY responses - clear and easy to understand
    ...
    // FORMAL responses - academic, structured
    ...
    // DETAILED responses - comprehensive but conversational
    ...
    // CONTINUE responses - follow-up conversations

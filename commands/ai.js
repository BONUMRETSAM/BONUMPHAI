...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      const isQuestion = this.isQuestion(prompt);
      const isAcknowledgment = this.isAcknowledgment(prompt);
      const isCasualGreeting = this.isCasualGreeting(prompt);
      const isFarewell = this.isFarewell(prompt);

      // Handle casual greetings and farewells immediately
      if (isCasualGreeting) {
        await this.handleCasualGreeting(senderId, prompt, token);
        return;
      }

      if (isFarewell) {
        await this.handleFarewell(senderId, prompt, token);
        return;
      }

      if (this.isExplicitScholarRequest(prompt) && !isQuestion) {
        await this.handleScholarSearch(senderId, prompt, token);
        return;
      }

      if (this.isLyricsRequest(prompt) && !isQuestion) {
        await this.handleLyricsSearch(senderId, prompt, token);
        return;
      }

      if (this.isExplicitRealtimeRequest(prompt) && !isQuestion) {
        await this.handleRealtimeQuestion(senderId, prompt, token);
        return;
      }

      if (this.isExplicitImageRequest(prompt) && !isQuestion) {
        await this.handleImageGeneration(senderId, prompt, token);
        return;
      }

      if (this.isMusicRequest(prompt) && !isQuestion) {
      ...
      // Make response conversational based on input
      aiResponse = this.makeConversationalResponse(aiResponse, prompt);
  ...
  // ===== NEW: CASUAL GREETING HANDLER =====
  isCasualGreeting(prompt) {
    const lower = prompt.toLowerCase().trim();
    const greetings = [
      'oy', 'uy', 'ei', 'hey', 'hello', 'hi', 'musta', 'kamusta', 
      'kumusta', 'kumusta ka', 'kamusta ka', 'musta na', 'kamusta na',
      'hiya', 'hoy', 'oi', 'musta ka', 'kumusta na', 'kumusta ka na',
      'hey you', 'hello there', 'hi there', 'ey', 'hoy', 'oyy', 'uyy'
    ];
    // Check if the entire message is just a greeting or very short greeting
    const wordCount = prompt.split(/\s+/).length;
    if (wordCount <= 3 && greetings.some(g => lower.includes(g))) {
      return true;
    }
    return false;
  },
  ...
  isFarewell(prompt) {
    const lower = prompt.toLowerCase().trim();
    const farewells = [
      'paalam', 'bye', 'goodbye', 'bye bye', 'see you', 'see ya',
      'matutulog na', 'matulog na', 'matulog na ako', 'matutulog na ako',
      'gabi na', 'sleep na', 'sleep na ako', 'good night', 'goodnight',
      'magpapahinga na', 'pahinga na', 'rest na', 'dito na lang',
      'tapos na', 'done na', 'sige na', 'cge na', 'ge na'
    ];
    const wordCount = prompt.split(/\s+/).length;
    if (wordCount <= 5 && farewells.some(f => lower.includes(f))) {
      return true;
    }
    return false;
  },

  async handleCasualGreeting(senderId, prompt, token) {
    const lower = prompt.toLowerCase().trim();
    const isTagalog = this.isTagalog(prompt);
    const history = conversationHistory[senderId];
    
    let response = '';
    
    if (isTagalog) {
      const tagalogGreetings = [
        'Oy! Musta ka na? Tagal ko nang di ka nakakausap ah!',
        'Uy! Kamusta ka? Ang saya makita kang nag-message ulit!',
        'Hoy! Kumusta na? Ano na bago sa\'yo?',
        'Oi! Musta ka na? Sana okay ka lang!',
        'Ey! Kamusta na buhay-buhay?'
      ];
      response = tagalogGreetings[Math.floor(Math.random() * tagalogGreetings.length)];
      
      // Add follow-up if there's history
      if (history && history.lastResponse) {
        const followUps = [
          ' May bago ka bang gustong itanong?',
          ' Balik tayo sa pinag-uusapan natin kanina?',
          ' May gusto ka bang ipagpatuloy na topic?',
          ' Tanong ka lang kung may gusto kang malaman!'
        ];
        response += followUps[Math.floor(Math.random() * followUps.length)];
      }
    } else {
      const englishGreetings = [
        'Hey! How are you? Long time no chat!',
        'Hi there! Good to see you messaging again!',
        'Hello! How\'s everything going?',
        'Hey! What\'s up? How have you been?'
      ];
      response = englishGreetings[Math.floor(Math.random() * englishGreetings.length)];
      
      if (history && history.lastResponse) {
        const followUps = [
          ' Got any new questions for me?',
          ' Want to continue our previous topic?',
  ...
  async handleFarewell(senderId, prompt, token) {
    const lower = prompt.toLowerCase().trim();
    const isTagalog = this.isTagalog(prompt);
    const hasSleep = lower.includes('tulog') || lower.includes('sleep') || lower.includes('gabi');
    const hasRest = lower.includes('pahinga') || lower.includes('rest');
    ...
    if (isTagalog) {
      if (hasSleep || hasRest) {
        const farewells = [
          'Sige, matulog ka na! Magpahinga ka nang mabuti. Bukas ulit!',
          'Ah sige, matulog ka na. Ingat at magandang gabi!',
          'Okay, pahinga ka na. Hanggang sa muli!',
          'Sige, tulog na. Magandang gabi at matamis na panaginip!'
        ];
        response = farewells[Math.floor(Math.random() * farewells.length)];
      } else {
        const farewells = [
          'Sige, paalam! Ingat ka palagi!',
          'Okay, bye! Hanggang sa susunod!',
          'Sige, mag-iingat ka! Chat ulit tayo!'
        ];
        response = farewells[Math.floor(Math.random() * farewells.length)];
      }
    } else {
      if (hasSleep || hasRest) {
        const farewells = [
          'Go ahead and sleep! Get some good rest. See you tomorrow!',
          'Good night! Sleep well and have sweet dreams!',
          'Okay, take a rest. Talk to you again soon!'
        ];
        response = farewells[Math.floor(Math.random() * farewells.length)];
      } else {
        const farewells = [
          'Goodbye! Take care always!',
    ...
    // Clear history on farewell
    delete conversationHistory[senderId];
    
    await sendMessage(senderId, { text: response }, token);
  },

  // ===== UPDATED: CONVERSATIONAL RESPONSE MAKER =====
  makeConversationalResponse(response, prompt) {
    ...
    const isQuestion = this.isQuestion(prompt);
    const isVeryShort = prompt.split(' ').length <= 2;
    
    // If it's just an acknowledgment like "ahhh yun pala yun", "wala na", "okay", "sige"
    if (isAcknowledgment) {
      if (isTagalog) {
        const tagalogResponses = [
          'Oo, yun na yun! May iba ka pang gustong malaman?',
          'Gets! Tara, ano pa gusto mong itanong?',
          'Ah okay! Sige, may iba ka pang katanungan?',
          'Yun na nga! Handa akong sumagot sa iba mo pang tanong.',
          'Tama yan! Ano pang gusto mong malaman?'
        ];
        return tagalogResponses[Math.floor(Math.random() * tagalogResponses.length)];
      } else {
        const englishResponses = [
          'Yes, that\'s it! Anything else you\'d like to know?',
    ...
    // For very short casual messages like "oy", "musta", "hello"
    if (isVeryShort && isCasual) {
      if (isTagalog) {
        const tagalogShort = [
          'Oy! Kamusta ka?',
          'Uy! Musta na?',
        ...
        return tagalogShort[Math.floor(Math.random() * tagalogShort.length)];
      } else {
        const englishShort = [
          'Hey! How are you?',
          'Hi there! What\'s up?',
        ...
        return englishShort[Math.floor(Math.random() * englishShort.length)];
      }
    }
    
    // If Tagalog and not formal, make it conversational
    if (isTagalog && !this.isFormalQuestion(prompt)) {
      // Remove overly formal intros
      let cleaned = response;
      cleaned = cleaned.replace(/^I understand your question\./i, '');
      cleaned = cleaned.replace(/^Let me explain\./i, '');
      cleaned = cleaned.replace(/^Here is my answer\./i, '');
      cleaned = cleaned.replace(/^Based on my understanding\./i, '');
      cleaned = cleaned.replace(/^To answer your question\./i, '');
      cleaned = cleaned.trim();
      
      // If response is long but question is short, shorten it
      if (isShort && cleaned.length > 150) {
        const sentences = cleaned.split(/(?<=[.!?])\s+/);
        cleaned = sentences.slice(0, 2).join(' ');
      }
      
      // Ensure response is in Tagalog
      if (!this.isTagalog(cleaned) && cleaned.length > 0) {
        // Keep the response as-is but add a Tagalog intro
        cleaned = 'Eto na: ' + cleaned;
      }
    ...
    // For casual English
    if (isCasual && !this.isFormalQuestion(prompt)) {
      let cleaned = response;
      cleaned = cleaned.replace(/^I understand your question\./i, '');
      cleaned = cleaned.replace(/^Let me explain\./i, '');
      cleaned = cleaned.replace(/^Here is my answer\./i, '');
  ...
  isTagalog(prompt) {
    const tagalogWords = [
      ...
      'ang', 'ng', 'sa', 'ay', 'na', 'pa', 'ba', 'daw', 'raw',
      'kasi', 'dahil', 'kaya', 'nga', 'naman', 'lang', 'lng',
      'oo', 'opo', 'hindi', 'dili', 'wala', 'meron', 'may',
      'ito', 'iyan', 'iyon', 'dito', 'diyan', 'doon',
      'ano', 'bakit', 'paano', 'kailan', 'saan', 'sino',
      'gusto', 'kailangan', 'pwedeng', 'pwede', 'dapat',
      'talaga', 'seryoso', 'grabe', 'sus', 'hay', 'ay',
      'oh', 'ah', 'eh', 'ha', 'heh', 'hmm', 'hm', 'mmm',
      'hahaha', 'hehehe', 'lol', 'lmao', 'wow', 'shet', 'gagi',
      'lala', 'hala', 'ayoko', 'sige', 'cge', 'okay', 'ok', 'ge',
      'gets', 'nagets', 'naintindihan', 'understand', 'gots',
      'sabi mo', 'sinabi mo', 'mao', 'diay', 'jud', 'gyud',
      'sakto', 'tama', 'correct', 'right', 'aw', 'yun', 'yon',
      'ganun', 'ganyan', 'ganito', 'parang', 'kumbaga',
      'musta', 'kamusta', 'kumusta', 'hoy', 'oy', 'uy', 'oi', 'ey'
    ];
    ...
    for (const word of tagalogWords) {
      if (lower.includes(word)) tagalogCount++;
    }
    
    // If more than 2 tagalog words or tagalog count > 25% of total words
    return tagalogCount >= 2 || (totalWords > 0 && tagalogCount / totalWords > 0.25);
  },

  // ===== UPDATED: ACKNOWLEDGMENT DETECTION =====
  isAcknowledgment(prompt) {
    const lower = prompt.toLowerCase().trim();
    const patterns = [
      'ahhh', 'ah', 'oh', 'eh', 'ay', 'ha', 'heh', 
      'yun pala', 'yon pala', 'ganun pala', 'ganyan pala',
      'yun na', 'yon na', 'yun na yun', 'yon na yon',
      'gets', 'nagets', 'gots', 'got it', 'okay', 'ok',
      'sige', 'cge', 'ge', 'salamat', 'thanks', 'thank you',
      'wala na', 'tapos na', 'done', 'finished', 'okay na',
      'tama', 'correct', 'right', 'sakto',
      'mao ba', 'mao diay', 'mao ni', 'mao na', 'ingon ana',
      'aw', 'ow', 'wow', 'hmm', 'hm', 'mmm',
      'talaga', 'really', 'sure', 'sigurado',
      'gets ko', 'nagets ko', 'naintindihan ko',
      'so', 'sow', 'eh', 'a', 'ah okay', 'oh okay',
      'yes', 'no', 'hindi', 'dili', 'oo', 'opo',
      'ah ganun', 'ah ganyan', 'ay ganun', 'ay ganyan'
    ];
    
    // Check if the message is very short and contains acknowledgment patterns
    const wordCount = prompt.split(/\s+/).length;
    if (wordCount <= 4 && patterns.some(p => lower.includes(p))) {
      return true;
    }
    
    // Check for specific acknowledgment phrases
    const ackPhrases = [
      'yun pala yun', 'yon pala yon', 'yun na yun', 'yon na yon',
      'ahhh yun pala', 'ah yun pala', 'ganun pala', 'ganyan pala',
      'gets ko na', 'nagets ko na', 'naintindihan ko na',
      'okay na', 'sige na', 'cge na', 'wala na', 'tapos na',
      'ahhh gets', 'ah okay', 'oh okay', 'aw okay',
      'so yun', 'so yon', 'so ganun', 'so ganyan',
      'mao ba diay', 'mao diay', 'mao ni diay'
    ];
    
    if (ackPhrases.some(p => lower.includes(p))) {
      return true;
    }
  ...
  // ===== UPDATED: CONTEXTUAL COMMAND CHECKERS =====
  isQuestion(prompt) {
    const lower = prompt.toLowerCase();
    if (prompt.includes('?')) return true;
    const questionWords = ['ano', 'what', 'bakit', 'why', 'paano', 'how', 'kailan', 'when', 'saan', 'where', 'sino', 'who', 'alin', 'which', 'may tanong', 'question', 'ask', 'pwede', 'can', 'could', 'paki', 'please'];
    return questionWords.some(w => lower.includes(w));
  },

  isCasualConversation(prompt) {
    const lower = prompt.toLowerCase();
    const casualWords = ['hello', 'hi', 'hey', 'kamusta', 'musta', 'kumusta', 'okay', 'ok', 'sige', 'cge', 'ge', 'yes', 'no', 'haha', 'hehe', 'lol', 'wow', 'shet', 'gagi', 'grabe', 'sus', 'hay', 'ay', 'oh', 'ah', 'eh', 'hmm', 'gets', 'nagets', 'gots', 'got it', 'sabi mo', 'talaga', 'really', 'hoy', 'oy', 'uy', 'oi', 'ey', 'musta na', 'kamusta na', 'kumusta na'];
    const wordCount = prompt.split(' ').length;
    return casualWords.some(w => lower.includes(w)) && wordCount <= 6;
  },

  isFormalQuestion(prompt) {
    const lower = prompt.toLowerCase();
    const formalWords = ['research', 'study', 'thesis', 'paper', 'article', 'analysis', 'data', 'methodology', 'hypothesis', 'experiment', 'clinical', 'scientific', 'academic', 'scholarly', 'peer-reviewed', 'literature', 'bibliography', 'citation', 'references', 'journal', 'publication'];
    const wordCount = prompt.split(' ').length;
    return formalWords.some(w => lower.includes(w)) && wordCount > 5;
  },

  isExplicitScholarRequest(prompt) {
    const lower = prompt.toLowerCase();
    
    const processWords = ['process', 'method', 'methodology', 'approach', 'steps', 'gumawa', 'gawin', 'create', 'write', 'sulat', 'format', 'structure', 'abstract', 'introduction', 'conclusion', 'parts', 'components'];
    if (processWords.some(w => lower.includes(w))) {
      return false;
    }
    
    const explicit = [
      /^gscholar\s+/i, /^scholar\s+/i, /^googlescholar\s+/i,
      /^research\s+(about|on|for|regarding|into|of|in)/i,
      /^find\s+(research|study|paper|article|thesis)/i,
      /^search\s+(research|study|paper|article|thesis)/i,
      /^give\s+me\s+(research|study|paper|article|thesis)/i,
      /^show\s+me\s+(research|study|paper|article|thesis)/i,
      /^hanapin\s+ang\s+(research|study|paper|article|thesis)/i,
      /^maghanap\s+ng\s+(research|study|paper|article|thesis)/i
    ];
    return explicit.some(p => p.test(lower));
  },

  isExplicitImageRequest(prompt) {
    const lower = prompt.toLowerCase();
    
    if (this.isQuestion(prompt)) return false;
    
    const academicWords = ['research', 'study', 'thesis', 'paper', 'article', 'abstract', 'analysis', 'data', 'literature', 'pananaliksik'];
    if (academicWords.some(w => lower.includes(w))) return false;
    
    const explicit = [
      /^generate\s+/i, /^show\s+me\s+(image|picture|photo|pic)/i,
      /^give\s+me\s+(a|an)?\s*(image|picture|photo|pic)/i,
      /^i\s+want\s+(a|an)?\s*(image|picture|photo|pic)/i,
      /^patingin\s+ng\s+(larawan|litrato|imahe)/i
    ];
    return explicit.some(p => p.test(lower));
  },
  ...
  isExplicitRealtimeRequest(prompt) {
    ...
    const nonTimeWords = ['ugali', 'prinsipyo', 'values', 'principles', 'character', 'morals', 'ethics', 'lahat ng oras', 'all the time'];
    if (nonTimeWords.some(w => lower.includes(w))) return false;
    
    const timePatterns = [
      /^what time/i, /^what is the time/i, /^anong oras/i,
      /^current time/i, /^real time/i, /^oras ngayon/i
    ];
    if (timePatterns.some(p => p.test(lower))) return true;
...
1. ACTIVITY SHEET / WORKSHEET / QUIZ / HOMEWORK / ASSIGNMENT
   ...
   - For explain why: Provide clear 1-2 sentences   - For math: Show step-by-step solution

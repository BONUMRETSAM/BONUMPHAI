...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // Check for humanize request FIRST
      const isHumanizeRequest = this.isHumanizeRequest(prompt);
      const isCasualConversation = this.isCasualConversation(prompt);
      const wantsHumanResponse = isHumanizeRequest || isCasualConversation;

      // Normalize prompt - remove humanize keywords for processing
      let cleanPrompt = prompt;
      if (isHumanizeRequest) {
        cleanPrompt = this.removeHumanizeKeywords(prompt);
      }

      // Check if it's an exact command (starts with command word)
      const commandWords = ['generate', 'image', 'img', 'show', 'gscholar', 'scholar', 'googlescholar', 'research', 'play', 'music', 'lyrics'];
      const normalizedPrompt = cleanPrompt.toLowerCase().trim();
      
      const isExactCommand = commandWords.some(cmd => normalizedPrompt.startsWith(cmd + ' ') || normalizedPrompt === cmd);

      // Check for lyrics request
      if (this.isExactLyricsRequest(cleanPrompt) || normalizedPrompt.startsWith('lyrics ')) {
        await this.handleLyricsSearch(senderId, cleanPrompt, token);
        return;
      }

      // Check for scholar/research request
      if (this.isExactScholarRequest(cleanPrompt)) {
        await this.handleScholarSearch(senderId, cleanPrompt, token);
        return;
      }

      // Check for generate/image request
      if (this.isExactGenerateRequest(cleanPrompt)) {
        await this.handleImageGeneration(senderId, cleanPrompt, token);
        return;
      }

      // Check for music request
      if (this.isExactMusicRequest(cleanPrompt)) {
        await this.handleMusicSearch(senderId, cleanPrompt, token);
        return;
      }

      // Handle real-time questions
      if (this.isRealtimeQuestion(cleanPrompt)) {
        await this.handleRealtimeQuestion(senderId, cleanPrompt, token);
        return;
      }
      ...
      if (event?.message?.reply_to?.mid) {
        ...
        if (!cleanPrompt) cleanPrompt = 'Please respond to what I said.';
      }

      // Handle image attachments - IMPORTANT: This handles activity sheets, math problems, etc.
      if (!imageUrl && event?.message?.attachments) {
        ...
        if (imageUrl && !cleanPrompt) {
          // If no specific prompt, default to analyzing the image for educational content
          cleanPrompt = 'Analyze this image. If it contains an activity sheet, worksheet, homework, assignment, math problem, logical puzzle, or any educational content, provide a comprehensive analysis and solution.';
        }
      }

      // Handle conversation history
      if (!isReply && cleanPrompt) {
        const history = conversationHistory[senderId];
        if (history && history.lastResponse) {
          const lowerPrompt = cleanPrompt.toLowerCase();
          const returnToTopic = this.isReturnToTopicRequest(cleanPrompt);
          if (returnToTopic) {
            const topic = this.extractTopicFromReturn(cleanPrompt);
            ...
            const isNewTopic = this.isNewTopic(lowerPrompt, history.lastPrompt, cleanPrompt);
      ...
      // User info question
      if (this.isUserInfoQuestion(cleanPrompt)) {
        await this.handleUserInfo(senderId, cleanPrompt, token);
        return;
      }

      const wantsDetailed = this.wantsDetailedAnswer(cleanPrompt) || this.isEducationalQuestion(cleanPrompt);
      let aiResponse = '';

      if (imageUrl) {
        // IMPORTANT: Enhanced image analysis for educational content
        aiResponse = await this.callGeminiAPI(cleanPrompt, imageUrl);
      } else {
        const finalPrompt = this.buildFinalPrompt(cleanPrompt, previousResponse, previousPrompt, isReply, wantsDetailed, wantsHumanResponse);
        const response = await this.callAPI(finalPrompt, senderId);
        aiResponse = this.cleanResponse(response || 'No response from API.');
      }

      // HUMANIZE: Make response more conversational if requested
      if (wantsHumanResponse && !this.isEducationalQuestion(cleanPrompt) && !imageUrl) {
        aiResponse = await this.humanizeResponse(aiResponse, cleanPrompt);
      }

      if (!imageUrl && !isReply && !wantsDetailed && !wantsHumanResponse) {
      ...
      if (isReply && this.isTranslationRequest(cleanPrompt)) {
        const targetLanguage = this.detectTargetLanguage(cleanPrompt);
  ...
  // ============ HUMANIZE FEATURE METHODS ============

  isHumanizeRequest(prompt) {
    const lower = prompt.toLowerCase().trim();
    const keywords = [
      'humanize', 'humanize:', 'humanize ',
      'make it human', 'make it conversational', 'make it natural',
      'parang tao', 'gawing tao', 'gawing natural', 'gawing conversational',
      'human-like', 'human like', 'conversational',
      'casual', 'casual tone', 'normal conversation', 'normal talk',
      'parang kausap', 'kausap lang', 'normal lang',
      'speak naturally', 'speak like human', 'talk like human',
      'sounds human', 'sound human',
      'maganda sana', 'parang totoong tao'
    ];
  ...
  isCasualConversation(prompt) {
    const lower = prompt.toLowerCase().trim();
    const casualPhrases = [
      'kamusta', 'musta', 'kumusta', 'musta ka', 'kamusta ka',
      'hello', 'hi', 'hey', 'yo', 'musta na', 'kamusta na',
      'musta naman', 'kamusta naman', 'musta ka na',
      'musta na kayo', 'kamusta na kayo',
      'hi po', 'hello po', 'good morning', 'good afternoon', 'good evening',
      'magandang umaga', 'magandang hapon', 'magandang gabi',
      'musta life', 'kamusta life', 'how are you', 'how are u',
      'how\'s life', 'how\'s everything', 'how\'s it going',
      'what\'s up', 'whats up', 'wassup', 'sup',
      'musta na buhay', 'kamusta na buhay',
      'musta araw', 'kamusta araw', 'musta araw mo', 'kamusta araw mo'
    ];
    const isShort = prompt.length < 30;
    const hasCasual = casualPhrases.some(p => lower.includes(p));
    const simpleQuestions = ['kamusta', 'musta', 'okay', 'ok', 'sige', 'cge'];
    const isSimple = simpleQuestions.some(q => lower === q || lower.startsWith(q + ' '));
    return (hasCasual && isShort) || isSimple || (lower.length < 15 && hasCasual);
  },

  removeHumanizeKeywords(prompt) {
    let cleaned = prompt;
    const keywords = [
      'humanize:', 'humanize ', 'humanize',
      'make it human', 'make it conversational', 'make it natural',
  ...
  async humanizeResponse(response, originalPrompt) {
    try {
      if (response.length < 100 && this.isCasualConversation(originalPrompt)) {
        return this.addCasualFlavor(response);
      }

      const humanizePrompt = `Rewrite this response to sound more human, conversational, and natural. 
      Make it sound like a real person talking casually. 
      Use Filipino/Taglish if the original was in Filipino.
      Add a friendly tone, use simple language, and make it engaging.
      Keep the same information but make it sound natural.
      ...
      try {
        const humanized = await this.callAPI(humanizePrompt, 'humanize');
        if (humanized && humanized.length > 0) {
          return this.cleanResponse(humanized);
        }
      } catch (error) {
        console.error('[Humanize] API failed:', error.message);
      }

      return this.localHumanize(response, originalPrompt);
  ...
  localHumanize(response, originalPrompt) {
    let humanized = response;

    const isFilipino = this.isFilipinoText(originalPrompt);
    
    const casualPrefixes = [
      'Ah, gets! ', 'Ayun! ', 'Eto na ha: ', 'Sige, ', 
      'Oh, ', 'Naku, ', 'Hala, ', 'Basta, ', 
      'Actually, ', 'So, ', 'Well, ', 'You know, ',
      'Ha? Ah, ', 'Eh kasi, ', 'Kumbaga, '
    ];
    
    const casualSuffixes = [
      ' gets?', ' diba?', ' a.', ' na lang.', ' ganun.',
      ' ba.', ' no?', ' ah.', ' talaga?', ' ha.',
      ' ganon.', ' ganyan.', ' kasi eh.', ' naman eh.'
    ];

    humanized = humanized
      .replace(/^In summary,/g, 'So basically,')
      .replace(/^To summarize,/g, 'So to sum it up,')
      .replace(/^Therefore,/g, 'So,')
      .replace(/^Thus,/g, 'So,')
      .replace(/^However,/g, 'But,')
      .replace(/^Furthermore,/g, 'Tapos,')
      .replace(/^Moreover,/g, 'At saka,')
      .replace(/^Additionally,/g, 'Plus,')
      .replace(/^In conclusion,/g, 'So in the end,')
      .replace(/^Consequently,/g, 'So,')
      .replace(/^For example,/g, 'For example,')
      .replace(/^For instance,/g, 'Like,')
      .replace(/^Specifically,/g, 'Particularly,')
      .replace(/^Generally,/g, 'Generally,')
      .replace(/^Basically,/g, 'Basically,')
      .replace(/^Essentially,/g, 'Essentially,');
    ...
    if (isFilipino) {
      humanized = this.addFilipinoFlavor(humanized);
    }

    if (humanized.includes('?')) {
      const questionEndings = [' no?', ' ah?', ' ha?', ' ba?', ' diba?', ' talaga?', ' noh?', ' ahh?'];
    }

    if (this.isCasualConversation(originalPrompt)) {
      const openings = ['Hi! ', 'Hello! ', 'Hey there! ', 'Oh, hi! '];
      if (!openings.some(o => humanized.startsWith(o))) {
        const randomOpen = openings[Math.floor(Math.random() * openings.length)];
        humanized = randomOpen + humanized;
      }
    }
    ...
    if (humanized.length < 30) {
      ...
      if (this.isFilipinoText(originalPrompt) && !shortResponses.some(r => humanized.includes(r))) {
        humanized = shortResponses[Math.floor(Math.random() * shortResponses.length)];
      }
    }
  ...
  addFilipinoFlavor(text) {
    ...
    let modified = text;
    for (const [eng, fil] of Object.entries(filipinoWords)) {
      const regex = new RegExp(`\\b${eng}\\b`, 'gi');
      modified = modified.replace(regex, fil);
    }

    if (modified.includes('?')) {
      const endings = [' ha?', ' diba?', ' no?', ' ba?', ' ah?'];
      const currentEnding = modified.slice(-3);
      if (!endings.some(e => currentEnding.includes(e))) {
        const randomEnd = endings[Math.floor(Math.random() * endings.length)];
        modified = modified.replace(/\?$/, randomEnd);
      }
    }
  ...
  addCasualFlavor(response) {
    const casualResponses = [
      'Ayun! Sige, okay yan.',
      'Ah ganun ba? Sige, gets ko na.',
      'Oo naman! Okay lang yan.',
      'Sige, sige. Gets ko naman.',
      'Ah okay! Salamat sa pag-share.',
      'Ayun oh! Ganyan dapat.',
      'Hala, seryoso? Sige, okay.',
      'Oo, tama ka dyan!',
      'Naku, ganun pala. Sige.',
      'Ah ganon? Sige, noted.',
      'Okay gets! Salamat.',
      'Ayos! Ganyan nga.',
      'Sige, sige. Tuloy lang.',
      'Ah okay, naintindihan ko.',
      'Oo naman! Walang problema.'
    ];
    
    if (response.length < 20 || response.match(/^(yes|no|okay|sige|oo|hindi|ok|yes|no)$/i)) {
  ...
  isFilipinoText(text) {
    const filipinoWords = [
      ...
      'ko', 'mo', 'niya', 'natin', 'ninyo', 'nila',
      'sa', 'ang', 'ng', 'ay', 'na', 'pa', 'ba', 'din', 'rin',
      'lang', 'lng', 'naman', 'nman', 'kasi', 'dahil', 'kaya',
      'talaga', 'tlga', 'tlag', 'sige', 'cge', 'opo', 'oo',
      'hindi', 'dili', 'wala', 'meron', 'may', 'mayroon',
      'tulad', 'parang', 'ganun', 'ganyan', 'ganito', 'ganon',
      'ano', 'bakit', 'paano', 'kailan', 'saan', 'sino', 'alin',
      'kamusta', 'musta', 'kumusta', 'ngayon', 'bukas', 'kahapon',
      'araw', 'gabi', 'umaga', 'hapon', 'tanghali',
      'bahay', 'kain', 'inuman', 'tulog', 'gala', 'lakad',
      'maganda', 'pangit', 'mabuti', 'masama', 'mahirap', 'madali',
      'malaki', 'maliit', 'mataba', 'payat', 'matangkad', 'maikli'
    ];
    const lower = text.toLowerCase();
    const words = lower.split(/\s+/);
    const filipinoCount = words.filter(w => filipinoWords.some(fw => w.includes(fw))).length;
    return filipinoCount > words.length * 0.1;
  },

  // ============ EXACT COMMAND METHODS ============

  isExactLyricsRequest(prompt) {
    const lower = prompt.toLowerCase().trim();
    const lyricsCommands = ['lyrics ', 'lyric ', 'letra ng ', 'kanta ni ', 'song lyrics of '];
    return lyricsCommands.some(cmd => lower.startsWith(cmd));
  },

  isExactScholarRequest(prompt) {
    const lower = prompt.toLowerCase().trim();
    const scholarCommands = ['gscholar ', 'scholar ', 'googlescholar ', 'research '];
    return scholarCommands.some(cmd => lower.startsWith(cmd));
  },

  isExactGenerateRequest(prompt) {
    const lower = prompt.toLowerCase().trim();
    const generateCommands = ['generate ', 'image ', 'img ', 'show '];
    if (generateCommands.some(cmd => lower.startsWith(cmd))) {
      return true;
    }
  ...
  isExactMusicRequest(prompt) {
    const lower = prompt.toLowerCase().trim();
    const musicCommands = ['play ', 'music ', 'song '];
    if (musicCommands.some(cmd => lower.startsWith(cmd))) {
      return true;
    }
    const explicitPatterns = [
  ...
  // ============ ENHANCED GEMINI API FOR EDUCATIONAL CONTENT ============
  async callGeminiAPI(prompt, imageUrl) {
    try {
      ...
      // Fallback: Try text-based analysis without image
      try {
        const fallbackPrompt = `The user sent an image but the image analysis failed. The user asked: ${prompt || 'Please describe what you see'}. Please provide a helpful response based on what you know about the topic.`;
        ...
        return 'Unable to analyze the image at this time. Please try again later or describe the image in text.';
      }
    }
  },

  // ============ ENHANCED GEMINI PROMPT FOR ACTIVITY SHEETS AND PROBLEMS ============
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
DETECT: Look for structured questions with numbers or letters.

For EACH question:
- Read the question carefully
- Identify the question type:
  a) Multiple Choice - Choose the correct answer
  b) Fill in the Blanks - Provide the missing word(s)
  c) True or False - Mark ✓ (True) or ✗ (False) with brief explanation
  d) Matching Type - Match Column A to Column B correctly
  e) Sequencing/Ordering - Arrange in the correct sequence
  f) Problem Solving - Show step-by-step solution
  g) Essay/Explain Why - Provide clear 2-3 sentence explanation
  h) Identification - Identify what is being described

For PROBLEM SOLVING questions:
- Show COMPLETE step-by-step solution
- Show the formula used
- Show calculations
- Show final answer with proper units
...
- Show each step with explanation
...
RESPONSE FORMAT for Activity Sheets:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
2. MATH PROBLEMS / EQUATIONS / LOGICAL PROBLEMS
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
DETECT: Numbers, variables, equations, word problems, logic puzzles.

For WORD PROBLEMS:
- Write the GIVEN information
- Write what is ASKED/REQUIRED
- Write the FORMULA/SOLUTION
- Show COMPLETE STEP-BY-STEP CALCULATIONS
- Provide FINAL ANSWER with units

For EQUATIONS:
- Show how to solve step by step
- Show all work
- Check the answer
...
RESPONSE FORMAT for Math/Logic:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📐 MATH/LOGIC PROBLEM SOLUTION
...
Final Answer:
[The complete answer with units/explanation]

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
3. SCIENCE / DIAGRAMS / LABELS
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
DETECT: Scientific diagrams, body parts, cell structures, chemical equations, physics diagrams.
...
- Describe key elements (2-3 sentences)
...
- Explain the joke or context briefly
...
IMPORTANT RULES:
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
1. ALWAYS detect the content type FIRST
2. For ACTIVITY SHEETS and MATH problems: Provide COMPLETE, ACCURATE solutions
3. Show ALL steps in problem solving
4. Double-check your answers
5. For multiple choice: Choose the BEST answer
6. For true/false: Explain WHY it's true or false
7. Use PLAIN TEXT only. No symbols, no markdown.
8. Be clear and easy to understand
9. If unsure about something, state that clearly
10. For educational content, be thorough and helpful

USER QUESTION/INSTRUCTION: ${userPrompt || 'Please analyze this image and provide a comprehensive response.'}

Now analyze the image carefully and provide your response following the appropriate format above.`;
  ...
  buildFinalPrompt(prompt, previousResponse, previousPrompt, isReply, wantsDetailed, wantsHuman) {
    let final = '';
    ...
    if (wantsHuman) {
      final += 'SPECIAL REQUEST: Make your response SOUND HUMAN and CONVERSATIONAL.\n';
      final += 'Use casual language, friendly tone, and natural expressions.\n';
      final += 'Make it sound like you\'re talking to a friend.\n';
      final += 'Use Filipino/Taglish if appropriate.\n';
      final += 'Keep it warm and engaging.\n\n';
    }

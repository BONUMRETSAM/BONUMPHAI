...
- STAY ON TOPIC.`;
  ...
  isMathProblem(prompt) {
    ...
    const isExample = lower.includes('example') || lower.includes('sample') || lower.includes('give me') || lower.includes('show me');
  ...
  buildResumeImprovePrompt(prompt, previousResume, detectedLanguage) {
    ...
    if (detectedLanguage === 'tagalog' || detectedLanguage === 'bisaya' || detectedLanguage === 'cebuano') {
      final += `Ikaw ay isang AI assistant na gumagawa ng PERSONALIZED na RESUME o CV.\n\n`;
      final += `Ito ang dating resume ng user:\n\n${previousResume}\n\n`;
      final += `USER REQUEST: "${prompt}"\n\n`;
      final += `AUTOMATIKONG i-improve ang resume sa itaas.\n`;
      final += `HUWAG magbigay ng generic examples lamang.\n`;
      final += `DIREKTANG ilagay ang mga pinakamahusay na objective, skills, at achievements sa resume.\n\n`;
      final += `Gumawa ng BAGONG RESUME na may BEST OBJECTIVES, SKILLS, at ACHIEVEMENTS.\n`;
      final += `Ang resume ay dapat nasa ${langName.toUpperCase()} na wika.\n\n`;
      final += `SUNDIN ANG PORMAT:\n\n`;
      final += `\n`;
      final += `(Pangalan Mo)\n`;
      ...
      final += `(Contact Number Mo)\n\n`;
      final += `OBJECTIVE:\n`;
      final += `To secure a challenging position where I can effectively utilize my skills and experience to contribute to organizational growth while pursuing my professional development.\n\n`;
      final += `EDUCATIONAL BACKGROUND:\n`;
      final += `(Kurso at Paaralan, Taon ng pagtatapos)\n\n`;
      final += `WORK EXPERIENCE:\n`;
      final += `(Pangalan ng Kumpanya) - (Posisyon) - (Taon)\n`;
      final += `(Mga responsibilidad at nagawa)\n\n`;
      final += `SKILLS:\n`;
      final += `• Excellent communication and interpersonal skills\n`;
      final += `• Strong leadership and team management abilities\n`;
      final += `• Proficient in [specific software/tools]\n`;
      final += `• Problem-solving and critical thinking\n`;
      final += `• Time management and organizational skills\n\n`;
      final += `ACHIEVEMENTS:\n`;
      final += `• [Achievement 1]\n`;
      final += `• [Achievement 2]\n`;
      final += `• [Achievement 3]\n\n`;
      ...
      final += `(Pangalan) - (Posisyon) - (Contact Number)\n`;
      final += `\n\n`;
      final += `Tiyakin na KUMPLETO at PROPESYONAL ang resume.\n`;
      final += `TUMUGON NGAYON SA ${langName.toUpperCase()}.`;
    } else {
      final += `You are an AI assistant that creates PERSONALIZED RESUME or CV.\n\n`;
      ...
      final += `DIRECTLY put the best objectives, skills, and achievements in the resume.\n\n`;
      final += `Create a NEW RESUME with BEST OBJECTIVES, SKILLS, and ACHIEVEMENTS.\n`;
      final += `The resume should be in ${langName.toUpperCase()} language.\n\n`;
      final += `FOLLOW THIS FORMAT:\n\n`;
      final += `\n`;
      final += `(Your Full Name)\n`;
  ...
  async sendChunks(senderId, text, token) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      await sendMessage(senderId, { text: prefix + chunk }, token);
      await new Promise(resolve => setTimeout(resolve, 500));
    }
  }
};

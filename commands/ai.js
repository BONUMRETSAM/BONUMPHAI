...
module.exports = {
  ...
  solveMathProblem(prompt) {
    ...
    if (prompt.includes('x^3') && prompt.includes('y^3') && prompt.includes('x+y')) {
      ...
      // Solve using formula: x^3 + y^3 = (x+y)((x+y)^2 - 3xy)
      // sumCubes = sum * (sum^2 - 3xy)
      // xy = (sum^2 - sumCubes/sum) / 3
  ...
  // ==================== COMPARISON LOGIC SOLVER ====================
  solveComparisonLogic(prompt) {
    const lower = prompt.toLowerCase();
    
    // Check if it's a comparison logic question
    if (!lower.includes('anong meron sa isa na wala sa tatlo') && 
        !lower.includes('what has one that the other three dont') &&
        !lower.includes('ano ang meron sa isa') &&
    ...
    const patterns = [
      // PLATO, BASO, TINIDOR, KUTSARA - Category difference
      {
        items: ['Plato', 'Baso', 'Tinidor', 'Kutsara'],
        answer: 'Plato',
        reason: 'Ang plato ay isang pinggan (dish/plate), samantalang ang baso, tinidor, at kutsara ay mga kagamitan (utensils) na ginagamit sa pagkain at inumin.'
      },
      // PLATO, BASO, TINIDOR, KUTSARA - Prongs difference
      {
        items: ['Plato', 'Baso', 'Tinidor', 'Kutsara'],
        answer: 'Tinidor',
        reason: 'Ang tinidor ang may mga ngipin (prongs), samantalang ang plato, baso, at kutsara ay walang ngipin.'
      }
    ];

    // Check if items match any pattern
    for (const pattern of patterns) {
      const match = pattern.items.every(item => 
        items.some(i => i.toLowerCase() === item.toLowerCase())
      );
      if (match) {
        return `Sagot: ${pattern.answer}\n\nPaliwanag:\n${pattern.reason}`;
      }
    }

    // Generic comparison logic
    return this.genericComparisonLogic(items);
  },

  genericComparisonLogic(items) {
    // Common item categories
    const itemData = {
      'Plato': { category: 'dish', hasProngs: false, type: 'lalagyan' },
      'Baso': { category: 'utensil', hasProngs: false, type: 'pang-inom' },
      'Tinidor': { category: 'utensil', hasProngs: true, type: 'pangkuha' },
      'Kutsara': { category: 'utensil', hasProngs: false, type: 'pangkuha' },
      'Mesa': { category: 'furniture', type: 'kasangkapan' },
      'Silya': { category: 'furniture', type: 'kasangkapan' },
      'Pinto': { category: 'structure', type: 'istruktura' },
      'Bintana': { category: 'structure', type: 'istruktura' }
    };
    ...
    for (const item of items) {
      const info = itemData[item];
      if (info && categories[info.category] === 1) {
        uniqueItem = item;
        const categoryNames = {
          'dish': 'pinggan (dish)',
          'utensil': 'kagamitan (utensil)',
          'furniture': 'kasangkapan (furniture)',
          'structure': 'istruktura (structure)'
        };
        reason = `Ang ${item.toLowerCase()} ay ang tanging ${categoryNames[info.category] || info.category} sa grupo.`;
        break;
      }
    }
    ...
    if (!uniqueItem) {
      for (const item of items) {
        const info = itemData[item];
        if (info && info.hasProngs) {
          uniqueItem = item;
          reason = `Ang ${item.toLowerCase()} ang may mga ngipin (prongs), samantalang ang iba ay wala.`;
          break;
        }
      }
    }
  ...
  solveLogicPuzzle(prompt) {
    // Check if it's an alphabet sequence puzzle
    const lower = prompt.toLowerCase();
    ...
    if (restrictionMatch) {
      ...
      if (nextLetter === forbidden) {
        ...
        // Find alternative that implies forbidden letter
  ...
  solveMoneyProblem(prompt) {
    ...
    const moneyKeywords = ['sukli', 'change', 'pera', 'money', 'bayad', 'payment', 'bibilhin', 'gastos', 'presyo', 'pesos', 'piso'];
...
USER QUESTION: ${userPrompt || 'Analyze this image and provide a comprehensive response.'}`;
  ...
  buildFinalPrompt(prompt, previousResponse, previousPrompt, isReply, wantsDetailed) {
    ...
    if (wantsDetailed) {
      final += '\n\nUSER WANTS DETAILED ANSWER: Provide a comprehensive, thorough, and detailed explanation.\n';
      final += 'Include examples, context, and complete information.\n';
    } else {
      final += '\n\nUSER WANTS CONCISE ANSWER: Provide a SHORT, DIRECT, and ACCURATE response.\n';

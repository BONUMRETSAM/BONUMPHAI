...
module.exports = {
  ...
  // ============================================================
  // MAIN EXECUTE - FIXED WITH DIRECT RESUME FALLBACK
  // ============================================================
  async execute(senderId, args, token, event) {
    try {
      ...
      // ============================================================
      // RESUME HANDLING - DIRECT GENERATION (NO API NEEDED)
      // ============================================================
      if (this.isResumeRequest(prompt)) {
        console.log('[Resume] Detected resume request:', prompt);
        
        // Try API first, fallback to direct
        let aiResponse = null;
        try {
          const resumePrompt = this.buildResumeByJob(prompt, detectedLanguage);
          const response = await this.callAPI(resumePrompt);
          aiResponse = this.cleanResponse(response || '');
        } catch (apiError) {
          console.log('[Resume] API failed, using direct builder');
          aiResponse = null;
        }
        
        // If API failed or returned empty, use direct builder
        if (!aiResponse || aiResponse.length < 50) {
          aiResponse = this.buildDirectResume(prompt, detectedLanguage);
        }
      ...
      // FALLBACK: Try direct resume if it's a resume request
      if (prompt && this.isResumeRequest(prompt)) {
        try {
          const fallbackResponse = this.buildDirectResume(prompt, 'english');
          await this.sendChunks(senderId, fallbackResponse, token);
          return;
        } catch (fallbackError) {
          console.error('[Resume] Fallback failed:', fallbackError.message);
        }
      }
  ...
  // ============================================================
  // DIRECT RESUME BUILDER - NEW METHOD
  // ============================================================
  buildDirectResume(prompt, detectedLanguage) {
    const lower = prompt.toLowerCase();
    ...
    const jobPatterns = {
      'supervisor': ['supervisor', 'team lead', 'team leader', 'operations supervisor', 'shift supervisor'],
      'manager': ['manager', 'general manager', 'operations manager', 'branch manager', 'store manager'],
    ...
    for (const [type, keywords] of Object.entries(jobPatterns)) {
    ...
    const resumes = {
      supervisor: {
        ...
        status: 'Married',
        experiences: [
          { title: 'Operations Supervisor', company: 'ABC Corporation – Makati City', years: '2020 – Present',
            resp: ['Spearheaded daily operations and managed a team of 15+ employees, consistently achieving operational targets and improving team efficiency by 25%.',
          ...
          { title: 'Team Leader', company: 'XYZ Company – Pasig City', years: '2017 – 2019',
            resp: ['Directed a team of 10 staff members, successfully achieving daily sales targets and operational goals while fostering a positive work environment.',
        ...
        education: ['Bachelor of Science in Business Administration | University of Santo Tomas | 2014',
                    'Senior High School | Muntinlupa National High School – Muntinlupa City | 2010 – 2012',
                    'Muntinlupa National High School (MNHS) | Muntinlupa City, Philippines | 2006 – 2010',
                    'Muntinlupa Elementary School (MES) | Muntinlupa City, Philippines | 2000 – 2006'],
        skills: ['Team Leadership & Performance Management', 'Strategic Planning & Operations Management', 
                 'Staff Training, Development & Coaching', 'Conflict Resolution & Problem-Solving',
                 'Excellent Communication & Interpersonal Skills', 'Data Analysis & Reporting'],
        references: ['Mr. Juan Miguel A. Dela Cruz | General Manager | ABC Corporation | 09171234567',
                     'Ms. Maria Lourdes C. Santos | HR Director | ABC Corporation | 09182345678',
                     'Mr. Jose Rizal M. Garcia | Senior Operations Manager | XYZ Company | 09273456789']
      },
      laborer: {
        ...
        status: 'Married',
        experiences: [
          { title: 'Construction Laborer', company: 'Mega Builders Inc. – Muntinlupa City', years: '2021 – Present',
            resp: ['Performed a wide range of construction tasks including excavation, loading, material handling, and site preparation to support project timelines.',
          ...
          { title: 'Warehouse Worker', company: 'Logistics Hub Corp. – Alabang, Muntinlupa', years: '2019 – 2020',
            resp: ['Loaded and unloaded goods from delivery trucks, ensuring accurate inventory records and proper handling of materials.',
        ...
        education: ['Senior High School | Muntinlupa National High School – Muntinlupa City | 2009',
                    'Muntinlupa National High School (MNHS) | Muntinlupa City, Philippines | 2005 – 2009',
                    'Muntinlupa Elementary School (MES) | Muntinlupa City, Philippines | 1999 – 2005'],
        skills: ['Heavy Lifting & Physical Stamina', 'Hand & Power Tool Operation',
                 'Workplace Safety & Compliance', 'Warehouse & Inventory Management',
                 'Team Collaboration & Reliability', 'Basic Construction Skills'],
        references: ['Mr. Ramon S. Santos | Site Supervisor | Mega Builders Inc. | 09171234567',
                     'Mr. Carlos L. Lopez | Warehouse Manager | Logistics Hub Corp. | 09282345678',
                     'Mr. Pedro R. Reyes | Project Manager | Mega Builders Inc. | 09183456789']
      },
      cashier: {
        ...
        status: 'Single',
        experiences: [
          { title: 'Cashier', company: 'SM Hypermarket – SM Tunasan', years: '',
            resp: ['Processed cash, credit, and digital payments accurately and efficiently.',
          ...
          { title: 'Cashier', company: 'SuperCity Alabang Landmark – Alabang, Muntinlupa', years: '',
            resp: ['Processed cash, credit, and digital payments accurately and efficiently.',
                   'Greeted customers warmly, answered inquiries, and assisted with purchases to ensure a positive shopping experience.',
                   'Conducted opening and closing cash counts daily to guarantee precise financial tracking and accountability.'] },
          { title: 'Service Crew (Cashier)', company: 'Jollibee Shell SLT Alabang', years: '',
            resp: ['Took accurate food and beverage orders from customers efficiently in a high-volume setting.',
        ...
        education: ['Bachelor of Science in Criminology | Pamantasan ng Lungsod ng Muntinlupa | 2018',
                    'Muntinlupa National High School (MNHS) | Muntinlupa City, Philippines | 2010 – 2014',
                    'Muntinlupa Elementary School (MES) | Muntinlupa City, Philippines | 2004 – 2010'],
        skills: ['Cash Handling & POS Operation', 'Teamwork & Collaboration', 'Sales & Promotions',
                 'Time Management & Adaptability', 'Effective Communication'],
        references: ['Angelika Mae Lopez | HR Assistant | 09911451130',
                     'Czerina Mae Espenesin | Accounting Assistant | Mabex Manufacturing Inc. | 09928196248',
                     'Menchie Donayre | Graphic Designer | 09563892622']
      },
      // Add other job types here...
    };
    
    const data = resumes[jobType] || resumes.cashier;
    ...
    final += `- Date of Birth: ${data.birthdate}\n`;
    final += `- Religion: Roman Catholic\n`;
    final += `- Civil Status: ${data.status}\n`;
...
- STAY ON TOPIC.`;
  ...
  getFallbackResponse(prompt) {
    const lower = prompt.toLowerCase();
    if (this.isResumeRequest(prompt)) {
      return this.buildDirectResume(prompt, 'english');
    }
    return `I apologize, but I'm having trouble connecting to my AI services right now. Please try again later. Your request was: "${prompt}"`;
  },
  ...
  async executeApiCall(config, prompt, senderId) {
    ...
    while (retries > 0) {
      try {
        ...
        const data = response.data;
        if (data[config.successField] !== true && data[config.successField] !== undefined) {
          throw new Error(`API returned ${config.successField}: ${data[config.successField]}`);
        }

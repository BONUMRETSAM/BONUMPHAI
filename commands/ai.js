...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // NEW: Make response conversational based on input
  ...
  // ===== NEW: CONVERSATIONAL RESPONSE MAKER =====
  makeConversationalResponse(response, prompt) {
    ...
    // If it's a very short casual response
    if (isShort && isCasual && !isQuestion) {
      // Just return the response as-is, it's already short
      return response;
    }
  ...
  isTagalog(prompt) {
    ...
    // If more than 2 tagalog words or tagalog count > 30% of total words
    return tagalogCount >= 3 || (totalWords > 0 && tagalogCount / totalWords > 0.3);
  },

  // ===== NEW: ACKNOWLEDGMENT DETECTION =====
  ...
  // ===== NEW: CONTEXTUAL COMMAND CHECKERS =====
  isQuestion(prompt) {
    const lower = prompt.toLowerCase();
  ...
  isCasualConversation(prompt) {
    const lower = prompt.toLowerCase();
    const casualWords = ['hello', 'hi', 'hey', 'kamusta', 'musta', 'okay', 'ok', 'sige', 'cge', 'ge', 'yes', 'no', 'haha', 'hehe', 'lol', 'wow', 'shet', 'gagi', 'grabe', 'sus', 'hay', 'ay', 'oh', 'ah', 'eh', 'hmm', 'gets', 'nagets', 'gots', 'got it', 'sabi mo', 'talaga', 'really'];
    const wordCount = prompt.split(' ').length;
    return casualWords.some(w => lower.includes(w)) && wordCount <= 5;
  },

...
module.exports = {
    ...
    try {
      ...
      if (imageUrl) {
        ...
        if (previousResponse && previousPrompt) {
          finalPrompt += 'Previous: ' + previousPrompt.substring(0, 60) + ' -> ' + previousResponse.substring(0, 100) + '\n\n';
        }
        
        finalPrompt += 'Question: ' + prompt;
        
        if (isCasualConversation && !wantsDetailed && !wantsElaboration && !wantsClarification) {
          finalPrompt += '\n\nReply casually in 1-2 sentences. Be friendly and direct.';
        } else if (wantsElaboration) {
          finalPrompt += '\n\nProvide detailed explanation with examples. Be comprehensive.';
        } else if (wantsClarification) {
          finalPrompt += '\n\nProvide clear and simple explanation. Make it easy to understand.';
        } else if (wantsDetailed) {
          finalPrompt += '\n\nProvide complete and detailed answer. Be thorough.';
        } else {
          finalPrompt += '\n\nAnswer directly and accurately. Be concise.';
        }
      ...
      if (aiResponse && aiResponse.endsWith('...')) {
        aiResponse = aiResponse.slice(0, -3);
      }

      if (!aiResponse || aiResponse === 'No response.' || aiResponse === '') {
        aiResponse = this.getFallbackAnswer(prompt);
      }
      ...
      const fallback = this.getFallbackAnswer(prompt);
      if (fallback) {
  ...
  getFallbackAnswer(prompt) {
    ...
    if ((lower.includes('abstract') && lower.includes('thesis')) || (lower.includes('abstract') && lower.includes('research'))) {
      return 'Tungkol sa pagbuo ng abstract ng thesis o research:\n\nAng abstract ay KARANIWANG nabubuo PAGKATAPOS ng buong research. Pero may mga pagkakataon na puwede itong gawin sa umpisa bilang gabay.\n\nDalawang paraan:\n\n1. SA UMPISA (Proposal Stage)\n- Gumagawa ng preliminary abstract\n- Nagsisilbing gabay sa research\n- Tinatawag na "proposal abstract"\n- Puwedeng baguhin later\n\n2. PAGKATAPOS (Final Stage)\n- Ibinabatay sa aktwal na data at findings\n- Mas accurate at kumpleto\n- Ito ang final abstract na isasama sa thesis\n\nPinakamainam: Gawin ang abstract PAGKATAPOS ng research para mas accurate at kumpleto ang nilalaman.';
    }
    
    if (lower.includes('kailan') || lower.includes('when') || lower.includes('paano') || lower.includes('how')) {
      return 'I understand your question. Unfortunately, I\'m having trouble connecting to my main AI service. Please try again in a moment, or use specific commands like:\n\n- gscholar [topic] - for research articles\n- generate [topic] - for images\n- play [song] - for music\n- lyrics [song] - for lyrics';
    }
  ...
  isExplicitImageRequest(prompt) {
    ...
    const topicPatterns = [
      ...
      'balik sa', 'balikan ang', 'balik tayo', 'balik na tayo'
    ];
    if (topicPatterns.some(p => lower.includes(p))) {
      return false;
    }
    
    const questionWords = ['?', 'ano', 'what', 'bakit', 'why', 'paano', 'how', 
      'kailan', 'when', 'saan', 'where', 'sino', 'who', 'may tanong', 'question', 'ask',
      'paki', 'please', 'pwede', 'can', 'could', 'explain', 'tell'];
    if (questionWords.some(w => lower.includes(w))) {
      return false;
    }
    
    const academicWords = ['research', 'study', 'thesis', 'paper', 'article', 
      'abstract', 'conclusion', 'introduction', 'analysis', 'data', 'literature',
      'pananaliksik', 'pag-aaral', 'akademiko', 'iskolar', 'aral', 'guro', 'school',
      'teacher', 'professor', 'student', 'class', 'lesson', 'assignment', 'homework'];
    if (academicWords.some(w => lower.includes(w))) {
      return false;
    }
    
    const explicitCommands = [
      /^generate\s+/i,
      /^show\s+me\s+(image|picture|photo|pic)/i,
    ...
    if (explicitCommands.some(p => p.test(lower))) {
      return true;
    }
  ...
  isExplicitScholarRequest(prompt) {
    ...
    const processWords = ['process', 'method', 'methodology', 'approach', 'steps', 
      'gumawa', 'gawin', 'create', 'write', 'sulat', 'format', 'structure',
      'abstract', 'introduction', 'conclusion', 'parts', 'components'];
    ...
    const explicitCommands = [
      ...
      /^articles?\s+(about|on|for|regarding)/i,
      /^papers?\s+(about|on|for|regarding)/i,
      /^studies?\s+(about|on|for|regarding)/i
    ];
  ...
  isExplicitRealtimeRequest(prompt) {
    ...
    const nonTimeWords = ['ugali', 'prinsipyo', 'values', 'principles', 'character', 
      'morals', 'ethics', 'lahat ng oras', 'all the time'];
    if (nonTimeWords.some(w => lower.includes(w))) {
      return false;
    }
  ...
  isExplicitMusicRequest(prompt) {
    ...
    if (hasMusicWord) {
      const questionIndicators = ['?', 'ano', 'what', 'bakit', 'why', 'paano', 'how'];
  ...
  buildGeminiPrompt(userPrompt) {
    ...
    prompt += 'CONTENT TYPE DETECTION & RESPONSE FORMATS:\n\n';
    prompt += '1. ACTIVITY SHEET / WORKSHEET / QUIZ / HOMEWORK / ASSIGNMENT\n';
    prompt += '   Identify the subject (Math, Science, English, TLE, etc.)\n';
    prompt += '   Identify the type (Multiple Choice, Fill in the Blanks, Sequencing, True/False, Essay, Problem Solving)\n';
    prompt += '   Read and understand each question carefully\n';
    prompt += '   Provide ACCURATE answers based on the content\n';
    prompt += '   For sequencing: Arrange in correct order\n';
    prompt += '   For true/false: Mark ✓ or ✗ with brief explanation\n';
    prompt += '   For explain why: Provide clear 1-2 sentences\n';
    prompt += '   For math: Show step-by-step solution\n';
    prompt += '   FORMAT: Use structured format with "CONTENT TYPE:", "ANSWERS:", "EXPLANATIONS:"\n\n';
    prompt += '2. MATH PROBLEMS / EQUATIONS\n';
    prompt += '   Read the problem carefully\n';
    prompt += '   Show step-by-step solution\n';
    prompt += '   Provide final answer with proper units\n';
    prompt += '   FORMAT: Use "CONTENT TYPE:", "SOLUTION:", "FINAL ANSWER:"\n\n';
    prompt += '3. SCIENCE / DIAGRAMS / LABELS\n';
    prompt += '   Identify parts and their functions\n';
    prompt += '   Explain processes\n';
    prompt += '   Provide definitions and key concepts\n';
    prompt += '   FORMAT: Use "CONTENT TYPE:", "PARTS/FUNCTIONS:", "KEY CONCEPTS:"\n\n';
    prompt += '4. TEXTBOOK / NOTES / EDUCATIONAL CONTENT\n';
    prompt += '   Extract key concepts\n';
    prompt += '   Summarize main ideas\n';
    prompt += '   Provide examples and applications\n';
    prompt += '   FORMAT: Use "CONTENT TYPE:", "KEY CONCEPTS:", "SUMMARY:"\n\n';
    prompt += '5. MEME / HUMOROUS IMAGE\n';
    prompt += '   Identify the meme or subject\n';
    prompt += '   Extract any text present\n';
    prompt += '   Explain the joke or context briefly (1-2 sentences)\n';
    prompt += '   Keep it SHORT and DIRECT\n';
    prompt += '   FORMAT: "CONTENT TYPE: Meme" then brief description\n\n';
    prompt += '6. GENERAL IMAGE (Photo, Art, Screenshot)\n';
    prompt += '   Identify what is shown (person, place, object, event)\n';
    prompt += '   Briefly describe what you see (2-3 sentences)\n';
    prompt += '   Extract text if present\n';
    prompt += '   Keep it SIMPLE and DIRECT\n';
    prompt += '   FORMAT: "CONTENT TYPE: General" then brief description\n\n';
    prompt += '7. SCREENSHOT / UI / WEBSITE\n';
    prompt += '   Identify what is shown (app, website, game)\n';
    prompt += '   Extract text if present\n';
    prompt += '   Brief description of what is happening\n';
    prompt += '   FORMAT: "CONTENT TYPE: Screenshot" then brief description\n\n';
    prompt += '8. PERSON / PORTRAIT\n';
    prompt += '   Identify the person if known\n';
    prompt += '   Brief description\n';
    prompt += '   FORMAT: "CONTENT TYPE: Portrait" then brief description\n\n';
    prompt += 'IMPORTANT RULES:\n';
    prompt += 'DETECT the content type FIRST before responding\n';
    prompt += 'Use the APPROPRIATE format based on content type\n';
    prompt += 'For MEME and GENERAL images: KEEP IT SHORT and DIRECT\n';
    prompt += 'For ACTIVITY SHEETS and MATH: Provide detailed answers\n';
    prompt += 'DO NOT use the same format for all images\n';
    prompt += 'Use plain text only. No symbols, no markdown.\n\n';
    prompt += 'USER QUESTION: ' + (userPrompt || 'Analyze this image and provide a comprehensive response.');
    return prompt;
  },
  ...
  async handleScholarSearch(senderId, prompt, token) {
    ...
    const removePhrases = ['give me', 'show me', 'find', 'search', 'i want', 'articles about', 'papers about', 'studies about', 'research about', 'about'];
    for (const phrase of removePhrases) {
      if (query.toLowerCase().startsWith(phrase)) {
        query = query.slice(phrase.length).trim();
        break;
      }
    }

    if (!query) {
      await sendMessage(senderId, { text: 'Google Scholar Search\n\nUsage: gscholar [search query]\n\nExamples:\ngscholar coconut hybridization\nresearch machine learning\ngive me research articles about artificial intelligence' }, token);
      return;
    }

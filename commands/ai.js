...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      if (!isReply && prompt) {
        ...
        if (history && history.lastResponse) {
          ...
          if (returnToTopic) {
            ...
            // This might be related but not clearly a follow-up
            // Check word overlap
      ...
      // Determine if user wants detailed answer
  ...
  isNewTopic(prompt, previousPrompt, originalPrompt) {
    ...
    // Check for casual phrases that indicate new topic
    ...
    // Very short prompts are likely new topics
  ...
  buildFinalPrompt(prompt, previousResponse, previousPrompt, isReply, wantsDetailed) {
    ...
    if (previousResponse && previousPrompt) {
      ...
      // Check for elaboration request
                 ...
                 lower.includes('ipaliwanag') || lower.includes('more details') || 
                 lower.includes('mas detalyado')) {
      ...
      // Check for correction/improvement request
      ...
      // Check for confirmation request
                 ...
                 lower.includes('really') || lower.includes('are you sure')) {
      ...
      // Check for agreement
                 ...
                 lower.includes('agree') || lower.includes('got it') ||
                 lower.includes('gets')) {
        final += 'User is acknowledging your previous response.\n';
      ...
      // No previous context, just use the prompt
      final = prompt;
    }
    ...
    final += '- Provide a DIRECT answer to the user\'s specific question.\n';

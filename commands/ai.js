...
- STAY ON TOPIC.`;
  ...
  isLetterWithDetails(prompt) {
    ...
    return matchCount >= 2 || lower.length > 50;
  },
  ...
  buildLetterPrompt(prompt, detectedLanguage) {
    ...
    if (detectedLanguage === 'tagalog' || detectedLanguage === 'bisaya' || detectedLanguage === 'cebuano') {
      final += `Ikaw ay isang AI assistant na tumutulong gumawa ng PERSONALIZED na sulat.\n\n`;
      final += `HINDI ka dapat magbigay ng generic template lamang.\n`;
      final += `KAILANGAN mong humingi ng mga detalye sa user.\n\n`;
      final += `MGA TANONG NA KAILANGAN MONG ITANONG:\n`;
      final += `1. Anong uri ng sulat? (Application, Resignation, Cover Letter, etc.)\n`;
      final += `2. Anong posisyon? (e.g., Teacher, Engineer, Nurse, Cashier)\n`;
      final += `3. Saang kumpanya/organisasyon? (Pangalan ng kumpanya)\n`;
      final += `4. Ano ang iyong kurso at pinag-aralan? (Course at School)\n`;
      final += `5. Ilang taon kang nagtrabaho? (Kung may experience)\n`;
      final += `6. Ano ang iyong mga kakayahan o skills? (e.g., Communication, Leadership)\n`;
      final += `7. Bakit ka interesado sa posisyon?\n\n`;
      final += `Pagkatapos mong makuha ang lahat ng detalye, gumawa ng KOMPLETO at PROPESYONAL na sulat.\n`;
      final += `Ang sulat ay dapat nasa ${langName.toUpperCase()} na wika.\n\n`;
      final += `USER REQUEST: "${prompt}"\n\n`;
      final += `MAGSIMULA KA NA SA PAGTANONG SA ${langName.toUpperCase()} NGAYON.`;
    } else {
      final += `You are an AI assistant that creates PERSONALIZED letters.\n\n`;
      final += `DO NOT give a generic template only.\n`;
      final += `You MUST ASK for details from the user.\n\n`;
      final += `QUESTIONS YOU NEED TO ASK:\n`;
      final += `1. What type of letter? (Application, Resignation, Cover Letter, etc.)\n`;
      final += `2. What position? (e.g., Teacher, Engineer, Nurse, Cashier)\n`;
      final += `3. What company/organization? (Company name)\n`;
      final += `4. What is your educational background? (Course and School)\n`;
      final += `5. How many years of work experience? (If any)\n`;
      final += `6. What are your skills? (e.g., Communication, Leadership)\n`;
      final += `7. Why are you interested in the position?\n\n`;
      final += `After getting all details, create a COMPLETE and PROFESSIONAL letter.\n`;
      final += `The letter should be in ${langName.toUpperCase()} language.\n\n`;
      final += `USER REQUEST: "${prompt}"\n\n`;
      final += `START ASKING THE USER IN ${langName.toUpperCase()} NOW.`;
    }
    return final;
  },
  ...
  buildLetterResponse(prompt, detectedLanguage) {
    ...
    let final = '';
    if (detectedLanguage === 'tagalog' || detectedLanguage === 'bisaya' || detectedLanguage === 'cebuano') {
      final += `Gumawa ka ng KOMPLETO at PERSONALIZED na sulat.\n\n`;
      final += `Gamitin ang mga detalye na ibinigay ng user.\n`;
      final += `Ang sulat ay dapat nasa ${langName.toUpperCase()} na wika.\n\n`;
      final += `MGA DETALYE NG USER: "${prompt}"\n\n`;
      final += `Gumawa ng isang PROPESYONAL, MALINAW, at KOMPLETONG sulat.\n\n`;
      final += `Format ng Application Letter:\n`;
      final += `[Petsa]\n\n`;
      final += `[Pangalan ng Tatanggap]\n[Posisyon]\n[Kumpanya]\n[Address]\n\n`;
      final += `Mahal na [Ginoo/Ginang],\n\n`;
      final += `[Introduksyon - Sino ka at bakit ka sumusulat]\n\n`;
      final += `[Katawan - Educ background, experience, skills, achievements]\n\n`;
      final += `[Bakit ka interesado sa posisyon at kumpanya]\n\n`;
      final += `[Pagtatapos - Pasasalamat, availability for interview]\n\n`;
      final += `Lubos na gumagalang,\n[Pangalan mo]\n\n`;
      final += `Tiyakin na ang sulat ay TAILORED sa detalye ng user.\n\n`;
      final += `TUMUGON NGAYON SA ${langName.toUpperCase()}.`;
    } else {
      final += `Create a COMPLETE and PERSONALIZED letter.\n\n`;
      final += `Use the details provided by the user.\n`;
      final += `The letter should be in ${langName.toUpperCase()} language.\n\n`;
      final += `USER DETAILS: "${prompt}"\n\n`;
      final += `Create a PROFESSIONAL, CLEAR, and COMPLETE letter.\n\n`;
      final += `Format of Application Letter:\n`;
      final += `[Date]\n\n`;
      final += `[Recipient's Name]\n[Position]\n[Company]\n[Address]\n\n`;
      final += `Dear [Mr./Ms./Dr.],\n\n`;
      final += `[Introduction - Who you are and why you are writing]\n\n`;
      final += `[Body - Educational background, experience, skills, achievements]\n\n`;
      final += `[Why you are interested in the position and company]\n\n`;
      final += `[Closing - Thank you, availability for interview]\n\n`;
      final += `Respectfully yours,\n[Your Name]\n\n`;
      final += `Make sure the letter is TAILORED to the user's details.\n\n`;

...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // ===== DYNAMIC LOGIC PUZZLE SOLVER =====
      const logicAnswer = this.solveLogicPuzzleDirectly(prompt);
      if (logicAnswer) {
      ...
      // ===== DYNAMIC MONEY PROBLEM SOLVER =====
      const moneyAnswer = this.solveMoneyProblemDirectly(prompt);
      if (moneyAnswer) {
  ...
  // ==================== DYNAMIC LOGIC PUZZLE SOLVER ====================
  solveLogicPuzzleDirectly(prompt) {
    // Extract sequence
    const sequence = this.extractSequence(prompt);
    ...
    // Find next element
    const nextElement = this.findNextElement(sequence, pattern);
    if (!nextElement) return null;
    
    // Check for restriction
    const restriction = this.extractRestriction(prompt);
    
    if (restriction && nextElement === restriction) {
      const alternative = this.findAlternative(sequence, pattern, restriction);
      if (alternative) {
        return this.formatLogicAnswer(sequence, nextElement, alternative, restriction);
      }
    }
    
    return this.formatLogicAnswer(sequence, nextElement);
  },

  extractSequence(prompt) {
    // Letter sequences: A B C D, A,B,C,D, ABCD
    const letterMatch = prompt.match(/[A-Z]\s*[A-Z]\s*[A-Z]\s*[A-Z]/g);
    if (letterMatch) {
      return letterMatch[0].split(/\s+/);
    }
    
    // Number sequences: 1 2 3 4, 1,2,3,4
    const numberMatch = prompt.match(/\d+\s*\d+\s*\d+\s*\d+/g);
    if (numberMatch) {
      return numberMatch[0].split(/\s+/);
    }
    
    // Mixed sequences
    const mixedMatch = prompt.match(/[A-Z]\d+\s*[A-Z]\d+\s*[A-Z]\d+/g);
    if (mixedMatch) {
      return mixedMatch[0].split(/\s+/);
    }
    
    // Try to find any sequence of 3+ items
    const words = prompt.split(/\s+/);
    for (let i = 0; i < words.length - 2; i++) {
      const chunk = words.slice(i, i + 4);
      if (this.looksLikeSequence(chunk)) {
        return chunk;
      }
    }
  ...
  looksLikeSequence(items) {
    if (items.length < 3) return false;
    
    // Check if all are letters
    const allLetters = items.every(item => /^[A-Z]$/.test(item));
    if (allLetters) {
      const alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
      let start = alphabet.indexOf(items[0]);
      if (start !== -1) {
        let isAlpha = true;
        for (let i = 0; i < items.length; i++) {
          if (items[i] !== alphabet[start + i]) {
            isAlpha = false;
            break;
          }
        }
    ...
    const allNumbers = items.every(item => /^\d+$/.test(item));
    if (allNumbers) {
  ...
  detectPattern(sequence) {
    // Check alphabetical
    const alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
    let start = alphabet.indexOf(sequence[0]);
    if (start !== -1) {
      let isAlphabet = true;
      for (let i = 0; i < sequence.length; i++) {
        if (sequence[i] !== alphabet[start + i]) {
          isAlphabet = false;
          break;
        }
      }
      if (isAlphabet) {
        return { type: 'alphabet', start: start };
      }
    }
    ...
    if (numbers.every(n => !isNaN(n))) {
      ...
      if (isArithmetic) {
        return { type: 'arithmetic', diff: diff };
      }
    }
  ...
  findNextElement(sequence, pattern) {
    if (pattern.type === 'alphabet') {
      const alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
      const nextIndex = pattern.start + sequence.length;
    ...
    if (pattern.type === 'arithmetic') {
      const last = parseInt(sequence[sequence.length - 1]);
      return (last + pattern.diff).toString();
    }
    
    return null;
  },

  extractRestriction(prompt) {
    // "without writing the letter X"
    const match = prompt.match(/without writing the letter\s*([A-Z])/i);
    if (match) return match[1].toUpperCase();
    
    // "bawal isulat ang X"
    const match2 = prompt.match(/bawal isulat ang\s*([A-Z])/i);
    if (match2) return match2[1].toUpperCase();
    
    // "without writing X"
    const match3 = prompt.match(/without writing\s*([A-Z])/i);
    if (match3) return match3[1].toUpperCase();
    
    return null;
  },

  findAlternative(sequence, pattern, forbidden) {
    if (pattern.type === 'alphabet') {
      const alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
      ...
      // Fallback: skip the forbidden letter
      const nextIndex = pattern.start + sequence.length + 1;
      if (nextIndex < alphabet.length) {
        return alphabet[nextIndex];
      }
    }
    ...
    if (pattern.type === 'arithmetic') {
      const forbiddenNum = parseInt(forbidden);
      if (!isNaN(forbiddenNum)) {
        return (forbiddenNum + pattern.diff).toString();
      }
    }
  ...
  formatLogicAnswer(sequence, nextElement, alternative = null, restriction = null) {
    let response = '';
    ...
    if (restriction && alternative) {
      ...
      response += `Ang dapat na susunod na letra ay ${nextElement}.\n`;
      response += `Pero bawal isulat ang "${restriction}".\n\n`;
      ...
      response += `Kaya sa pamamagitan ng paglalagay ng ${alternative}, ang implicit na sequence ay ${sequence.join(' ')} ${nextElement}.`;
    } else {
      response = `Sagot: ${nextElement}\n\n`;
      response += `Paliwanag:\n`;
      response += `Ang sequence ay: ${sequence.join(' ')} _\n`;
      response += `Ang pattern ay alphabet order.\n`;
      response += `Kaya ang susunod na elemento ay ${nextElement}.`;
    }
  ...
  // ==================== DYNAMIC MONEY PROBLEM SOLVER ====================
  solveMoneyProblemDirectly(prompt) {
    const lower = prompt.toLowerCase();
    
    // Check if it's a money/change problem
    ...
    // Determine amount and payment based on context
    let amount, payment;
    
    // Check for "may X pesos" pattern (amount)
    const mayMatch = prompt.match(/may\s*(\d+)/i);
    if (mayMatch) {
      amount = parseInt(mayMatch[1]);
      // Payment is the other number
      payment = numValues.find(n => n !== amount);
    }
    
    // Check for "bayaran X" pattern (payment)
    const bayadMatch = prompt.match(/bayaran\s*(\d+)/i) || prompt.match(/bayad\s*(\d+)/i);
    if (bayadMatch && !payment) {
      payment = parseInt(bayadMatch[1]);
      amount = numValues.find(n => n !== payment);
    }
    
    // Check for "meron akong X" pattern
    const meronMatch = prompt.match(/meron akong\s*(\d+)/i) || prompt.match(/naa koy\s*(\d+)/i);
    if (meronMatch && !amount) {
      amount = parseInt(meronMatch[1]);
      payment = numValues.find(n => n !== amount);
    }
    
    // Fallback: first number is amount, second is payment
    if (!amount || !payment) {
      // Check if there's a "coin" or "pesos" after a number
      for (let i = 0; i < numValues.length; i++) {
        const str = numValues[i].toString();
        const index = prompt.indexOf(str);
        const after = prompt.substring(index + str.length, index + str.length + 10);
        if (after.includes('coin') || after.includes('pesos') || after.includes('piso')) {
          if (!amount) amount = numValues[i];
          else payment = numValues[i];
        }
      }
    }
    ...
    // Make sure amount is the smaller one (what you need to pay)
    // and payment is what you have or what you're giving
    if (amount > payment) {
      // Swap if amount is larger than payment
  ...
  formatMoneyAnswer(amount, payment) {
    ...
    // Option 3: Next higher bill (if payment is larger than amount)
    let nextBill = null;
    ...
    if (nextBill && nextBill !== amount) {
      ...
      const billName = nextBill >= 100 ? `${nextBill} pesos na bill` : `${nextBill} pesos`;
      response += `✅ Magbayad ng ${billName} - SUKLI ${change2} pesos\n`;
    }
    
    // Option 4: Current payment (if giving more than needed)
    if (payment > amount && payment !== nextBill) {
      const paymentName = payment >= 100 ? `${payment} pesos` : `${payment} pesos`;
      response += `✅ Magbayad ng ${paymentName} - SUKLI ${change} pesos (ito ang ginawa mo)\n`;
    }
    
    // Option 5: Coin combination for small change
    ...
    if (change > 0) {
      const bestOption = this.getBestPaymentOption(amount, change);
      if (bestOption) {
        response += `\n\n💡 Pinaka-praktikal: ${bestOption}`;
      }
    }
  ...
  getBestPaymentOption(amount, change) {
    // Exact payment is always best
    if (amount <= 1000) {
      return `Magbayad ng eksaktong ${amount} pesos - walang sukli.`;
    }
    
    // Find next higher bill
    const bills = [1000, 500, 200, 100, 50, 20];
    for (const bill of bills) {
      if (bill >= amount && bill - amount <= 100) {
        return `Magbayad ng ${bill} pesos - sukli ${bill - amount} pesos (isang pirasong pera lang).`;
      }
    }
    
    // If change is small
    if (change <= 50) {
      return `Magbayad ng ${amount + change} pesos - sukli ${change} pesos (maliit lang ang sukli).`;
    }

    ...
    try {
      ...
      if (this.isResumeRequest(prompt)) {
        ...
        const jobKeywords = ['teacher', 'agriculture', 'nurse', 'engineer', 'accountant', 
                             ...
                             'cashier', 'retail', 'sales clerk', 'receptionist', 'staff',
...
- STAY ON TOPIC.`;
  ...
  buildResumeByJob(prompt, detectedLanguage) {
    ...
    else if (lower.includes('agriculture') || lower.includes('ags') || lower.includes('farming') || lower.includes('pagsasaka')) jobType = 'agriculture';
    else if (lower.includes('nurse') || lower.includes('nars') || lower.includes('medical')) jobType = 'nurse';
    else if (lower.includes('engineer') || lower.includes('inhinyero')) jobType = 'engineer';
    else if (lower.includes('accountant') || lower.includes('accounting') || lower.includes('taga-account')) jobType = 'accountant';
    else if (lower.includes('cashier') || lower.includes('taga-cash') || lower.includes('cash')) jobType = 'cashier';
    else if (lower.includes('call center') || lower.includes('bpo') || lower.includes('agent')) jobType = 'callcenter';
    else if (lower.includes('secretary') || lower.includes('admin') || lower.includes('administrative')) jobType = 'secretary';
    else if (lower.includes('manager') || lower.includes('supervisor') || lower.includes('team lead')) jobType = 'manager';
    else if (lower.includes('cook') || lower.includes('chef') || lower.includes('kusinero')) jobType = 'cook';
    else if (lower.includes('driver') || lower.includes('tsuper')) jobType = 'driver';
    else if (lower.includes('sales') || lower.includes('marketing') || lower.includes('benta')) jobType = 'sales';
    else if (lower.includes('hr') || lower.includes('human resource') || lower.includes('recruiter')) jobType = 'hr';
    else if (lower.includes('it') || lower.includes('programmer') || lower.includes('developer') || lower.includes('web')) jobType = 'it';
    else if (lower.includes('janitor') || lower.includes('cleaner') || lower.includes('maintenance')) jobType = 'janitor';
    else if (lower.includes('laborer') || lower.includes('construction') || lower.includes('worker')) jobType = 'laborer';
    else if (lower.includes('security') || lower.includes('guard') || lower.includes('safety')) jobType = 'security';
    else if (lower.includes('barista') || lower.includes('server') || lower.includes('waitress')) jobType = 'service';
    else if (lower.includes('receptionist') || lower.includes('front desk')) jobType = 'receptionist';
    else if (lower.includes('factory') || lower.includes('warehouse') || lower.includes('production')) jobType = 'factory';
    ...
    final += `- Margins: 1 inch on all sides (top, bottom, left, right)\n`;
    final += `- Line Spacing: 1.15 or 1.5 for readability\n`;
    final += `- Use thick lines (===================) between major sections\n`;
    final += `- Use thin lines (-------------------) for sub-sections if needed\n\n`;
    final += `FOLLOW THIS EXACT FORMAT - COPY IT EXACTLY AS SHOWN IN THE SCREENSHOT:\n\n`;
    final += `\n`;
    final += `                    [RESUME - FOR PRINTING ON SHORT BOND PAPER]\n`;
    final += `\n\n`;
    final += `PROFESSIONAL SUMMARY\n\n`;
    final += `(A strong and compelling professional summary tailored to the job - 3 to 5 sentences)\n\n`;
    final += `\n\n`;
    ...
    final += `- Responsibility 4 (optional)\n\n`;
    final += `**Job Title 2**\n`;
    final += `Company Name – Location\n`;
    final += `- Responsibility 1\n`;
    final += `- Responsibility 2\n`;
    final += `- Responsibility 3\n\n`;
    final += `**Job Title 3** (if applicable)\n`;
    final += `Company Name – Location\n`;
    final += `- Responsibility 1\n`;
    final += `- Responsibility 2\n\n`;
    final += `\n\n`;
    ...
    final += `- PROFESSIONAL SUMMARY must be 3-5 sentences, compelling and tailored to the job.\n`;

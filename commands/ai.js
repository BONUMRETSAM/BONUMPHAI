...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      const isTagalog = detectedLanguage === 'tagalog';
      const isBisaya = detectedLanguage === 'bisaya' || detectedLanguage === 'cebuano';
      ...
      if (!isReply && prompt && !imageUrl) {
        ...
        if (history && history.lastResponse) {
          ...
          const isFollowUp = this.isFollowUpRequest(lowerPrompt) || 
                            ...
                            this.isModificationRequest(lowerPrompt);
      ...
      if (imageUrl) {
        // New image analysis with language detection
        aiResponse = await this.callGeminiAPI(prompt, imageUrl, detectedLanguage);
        
        // Check if the image contains text in a specific language
        const imageLanguage = this.detectTextLanguage(aiResponse);
        if (imageLanguage !== 'english' && imageLanguage !== detectedLanguage) {
          // Re-analyze with detected image language
          aiResponse = await this.callGeminiAPI(prompt, imageUrl, imageLanguage);
        }
        ...
        conversationHistory[senderId] = {
          ...
          language: imageLanguage !== 'english' ? imageLanguage : detectedLanguage,
          timestamp: Date.now(),
        ...
        if (history?.hasImageContext && !imageUrl) {
          ...
          // Ensure response in the correct language
          if (responseLanguage !== 'english') {
            aiResponse = await this.ensureLanguageResponse(aiResponse, responseLanguage);
          }
        } else {
          const finalPrompt = this.buildFinalPrompt(prompt, previousResponse, previousPrompt, isReply, wantsDetailed, detectedLanguage);
          const response = await this.callAPI(finalPrompt, senderId);
          aiResponse = this.cleanResponse(response || 'No response from API.');
        }
  ...
  async ensureLanguageResponse(text, language) {
    ...
    try {
      ...
      const translatePrompt = `Translate this text to ${langName}. Only provide the translation, no other text. Do not include the original text. Here is the text to translate: ${text}`;
      const response = await this.callAPI(translatePrompt, 'translation');
      return response || text;
    } catch (error) {
      console.error('[Language Translation] Failed:', error.message);
      return text;
    }
  },

  // Updated isModificationRequest method
  ...
  // Updated buildImageFollowUpPrompt method with multi-language support
  buildImageFollowUpPrompt(prompt, previousResponse, previousPrompt, wantsDetailed, language = 'english') {
    let final = '';
    const langName = this.getLanguageName(language);
    
    final += `PREVIOUS IMAGE ANALYSIS CONTEXT (${langName}):\n`;
    final += 'The user previously sent an image and you analyzed it.\n';
    final += 'Your previous analysis was:\n' + previousResponse + '\n\n';
    final += 'Now the user is asking a follow-up question about that image analysis.\n';
    final += 'User says: ' + prompt + '\n\n';
    final += `IMPORTANT: Respond in ${langName}.\n\n`;
    ...
    if (lower.includes('short') || lower.includes('concise') || lower.includes('brief') || 
        ...
        lower.includes('ikli') || lower.includes('gawing maikli') || lower.includes('mas maikli') ||
        lower.includes('pakiikli') || lower.includes('mubo') || lower.includes('muboa') ||
        ...
        lower.includes('kurz')) {
      final += `The user wants you to make your previous response SHORTER and more CONCISE in ${langName}.\n`;
      final += 'Provide only the KEY POINTS in a very brief format.\n';
      final += 'Remove all unnecessary details and explanations.\n\n';
               ...
               lower.includes('gawing malinaw') || lower.includes('mas malinaw') || lower.includes('paki linaw') ||
      ...
      final += `The user wants you to make your previous response CLEARER and easier to understand in ${langName}.\n`;
      final += 'Rephrase it in simpler terms and provide a clearer explanation.\n\n';
               ...
               lower.includes('simplehan') || lower.includes('gawing simple') || lower.includes('mas simple') ||
               lower.includes('mas madali') || lower.includes('madali') || lower.includes('pasimplehon') ||
               lower.includes('pasimpleha') || lower.includes('pasimpleen') || lower.includes('pasimplehan') ||
               lower.includes('simple') || lower.includes('simplificar') || lower.includes('simplifier') ||
               lower.includes('einfach')) {
      final += `The user wants a SIMPLER explanation of your previous response in ${langName}.\n`;
      final += 'Use basic language and avoid technical terms.\n';
      final += 'Explain it as if talking to a beginner.\n\n';
      ...
      final += `The user wants MORE DETAILS about your previous response in ${langName}.\n`;
      final += 'Provide additional information and elaborate on each point.\n\n';
      ...
      final += `The user wants a SUMMARY of your previous response in ${langName}.\n`;
      final += 'Provide only the most important points in a brief format.\n\n';
    } else if (lower.includes('example') || lower.includes('halimbawa') || lower.includes('sample') ||
               lower.includes('for example') || lower.includes('like what') || lower.includes('such as')) {
      final += `The user wants EXAMPLES related to your previous response in ${langName}.\n`;
      final += 'Provide relevant examples to illustrate your points.\n\n';
    } else if (lower.includes('translate') || lower.includes('isalin') || lower.includes('salin') ||
               lower.includes('translation')) {
      const targetLang = this.detectTargetLanguage(prompt);
      final += `The user wants you to TRANSLATE your previous response to ${targetLang}.\n`;
      final += `Provide the translation in ${targetLang} only.\n\n`;
    } else if (lower.includes('what do you mean') || lower.includes('what does it mean') ||
               lower.includes('ano ibig sabihin') || lower.includes('ano meaning') ||
               lower.includes('what is that') || lower.includes('what is this') ||
               lower.includes('ano yan') || lower.includes('ano yun') || lower.includes('ano ito') ||
               lower.includes('unsa pasabot') || lower.includes('unsa meaning')) {
      final += `The user is asking for clarification about what you meant in your previous response in ${langName}.\n`;
      final += 'Explain your previous response in simpler terms.\n\n';
    } else {
      final += `The user is asking about the image you previously analyzed in ${langName}.\n`;
      final += 'Provide a response based on your previous analysis.\n';
      final += 'Acknowledge that this is a follow-up to the image analysis.\n\n';
    }
    
    if (wantsDetailed) {
      final += `USER WANTS DETAILED ANSWER: Provide a comprehensive explanation in ${langName}.\n\n`;
    } else {
      final += `USER WANTS CONCISE ANSWER: Be direct and to the point in ${langName}.\n\n`;
    }
    
    final += 'IMPORTANT GUIDELINES:\n';
    final += '- This is a follow-up to a previous IMAGE ANALYSIS.\n';
    final += '- Base your response on the previous analysis context.\n';
    ...
    final += `- Respond in ${langName}.\n`;
    final += '- Provide a direct and helpful response.\n';
  ...
  // Updated extractTopicKey method
  ...
  // Updated isReturnToTopicRequest method
  ...
  // Updated extractTopicFromReturn method
  ...
  // Updated isRealtimeQuestion method
  ...
  // Updated handleRealtimeQuestion method with language support
    ...
    try {
      ...
      if (data && data.answer) {
        ...
        if (detectedLanguage !== 'english') {
          cleanResponse = await this.ensureLanguageResponse(cleanResponse, detectedLanguage);
        }
  ...
  // Updated isExactTimeRequest method
  ...
  // Updated handleTimeRequest method with language support
  ...
  // Updated isLyricsRequest method
  ...
  // Updated handleLyricsSearch method
  ...
  // Updated isGenerateCommand method
  ...
  // Updated handleImageGeneration method

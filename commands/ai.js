...
module.exports = {
    ...
    try {
      ...
      if (!isReply && prompt) {
        ...
        if (history && history.lastResponse) {
          ...
          // ✅ Check if this is a completely new question
  ...
  // ✅ Improved: Detect if it's a completely new question
  isNewQuestion(prompt, previousPrompt) {
    ...
    // If numbers are different, it's a new question
    ...
    const moneyWords = ['pesos', 'pera', 'money', 'sukli', 'change', 'bayad', 'payment'];
    const hasMoneyWords = moneyWords.some(w => lowerPrompt.includes(w));
    const hasPrevMoneyWords = moneyWords.some(w => lowerPrevious.includes(w));
    
    // If both are money questions but numbers changed, it's new
    if (hasMoneyWords && hasPrevMoneyWords && promptNumbers.length > 0 && prevNumbers.length > 0) {
      return true;
    }
    
    // Check for new topic indicators
    const newTopicIndicators = ['halimbawa', 'example', 'ibang', 'another', 'new', 'bagong', 'panibago'];
    if (newTopicIndicators.some(i => lowerPrompt.includes(i))) {
      return true;
    }
...
USER QUESTION: ${userPrompt || 'Analyze this image and provide a comprehensive response.'}`;
  ...
  buildFinalPrompt(prompt, previousResponse, previousPrompt, isReply, wantsDetailed) {
    ...
    // ✅ CRITICAL FIX: Add practical thinking instructions for all real-life scenarios
    const lowerPrompt = prompt.toLowerCase();
    if (lowerPrompt.includes('sukli') || lowerPrompt.includes('change') || lowerPrompt.includes('pera') || 
      ...
      final += '\n\nIMPORTANT: This is a money/change problem. Think PRACTICALLY.\n';
      final += 'CONSIDER: What bills/coins are available (1, 5, 10, 20, 50, 100, etc.)\n';
      final += 'PROVIDE MULTIPLE OPTIONS: Exact payment, overpayment with smallest change, etc.\n';
      final += 'SUGGEST the most practical option.\n';
      final += 'Example: If someone has 15 and pays 9, they can pay 9 exactly (no change), pay 10 and get 1 peso change, or pay 15 and get 6 pesos change.\n\n';
    }
    ...
    if (lowerPrompt.includes('logic') || lowerPrompt.includes('puzzle') || lowerPrompt.includes('riddle')) {
      ...
      final += 'CONSIDER: The obvious answer, the hidden meaning, and the practical answer.\n';
      final += 'PROVIDE the most logical and practical answer.\n\n';
    }

    if (lowerPrompt.includes('natulog') || lowerPrompt.includes('kumatok') || lowerPrompt.includes('gising') || 
        lowerPrompt.includes('sleep') || lowerPrompt.includes('knock') || lowerPrompt.includes('wake')) {
      final += '\n\nIMPORTANT: This is a real-life scenario. Think PRACTICALLY.\n';
      final += 'CONSIDER: What would you do first in real life.\n';
      final += 'PROVIDE the most logical and practical answer.\n\n';
    }

    if (lowerPrompt.includes('una') || lowerPrompt.includes('first') || lowerPrompt.includes('bubuksan') || 
        lowerPrompt.includes('open') || lowerPrompt.includes('gagawin') || lowerPrompt.includes('do')) {
      final += '\n\nIMPORTANT: This is a sequence/order question. Think PRACTICALLY.\n';
      final += 'CONSIDER: What comes first in real life.\n';
      final += 'PROVIDE the most logical and practical answer.\n\n';
    }

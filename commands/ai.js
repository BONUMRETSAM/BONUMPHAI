...
module.exports = {
  name: ['ai', 'opera', 'ask', 'gemini', 'vision', 'gscholar', 'scholar', 'googlescholar', 'research', 'generate', 'image', 'img', 'show', 'math', 'solve', 'calculate'],
  description: 'Multi-modal AI with text, image analysis, Google Scholar, image generation, music search, lyrics, and MATH SOLVING!',
  usage: 'ai [message] or send/reply to image or generate [query] or play [song] or lyrics [song] or math [equation]',
  version: '4.0.0',
  author: 'codex',
  category: 'AI',
  cooldown: 3,
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // ================================================================
      // 🆕 MATH SOLVER - Handles equations, group data, statistics
      // ================================================================
      if (this.isMathRequest(prompt)) {
        await this.handleMathRequest(senderId, prompt, token, detectedLanguage);
        return;
      }
  ...
  isMathRequest(prompt) {
    ...
    const commands = ['math', 'solve', 'calculate', 'compute', 'equation', 'formula', 
                     'statistics', 'stats', 'probability', 'algebra', 'geometry', 
                     'trigonometry', 'calculus', 'arithmetic', 'group data', 
                     'frequency distribution', 'mean', 'median', 'mode', 'variance', 
                     'standard deviation', 'correlation', 'regression'];
    
    if (commands.some(cmd => lower.startsWith(cmd) || lower.includes(cmd))) return true;
    
    const mathPatterns = [
      /[\d\+\-\*\/\^\(\)]+/,
      /\b[0-9]+\s*[+\-*/]\s*[0-9]+/,
      /\b(mean|median|mode|average|sum|count|min|max)\b/i,
      /\b[sd|σ|sigma|variance|range|quartile|percentile]\b/i,
      /[Σ∫√πθϕλ]/,
      /\bf(x)\s*=\s*/i,
      /\by\s*=\s*[\d\+\-\*\/\^x]+/i,
      /\d+\s*percent\s*of/i,
    ...
    for (const pattern of mathPatterns) {
      if (pattern.test(lower)) {
        if (!this.isCasualConversation(prompt)) {
          return true;
        }
      }
    }
  ...
  async handleMathRequest(senderId, prompt, token, language) {
    try {
      let mathProblem = prompt;
      const mathCommands = ['math', 'solve', 'calculate', 'compute', 'equation', 'formula', 
                           'statistics', 'stats', 'probability', 'algebra', 'geometry', 
                           'trigonometry', 'calculus', 'arithmetic'];
      
      for (const cmd of mathCommands) {
        if (mathProblem.toLowerCase().startsWith(cmd)) {
          mathProblem = mathProblem.slice(cmd.length).trim();
          break;
        }
      }
      
      if (!mathProblem) {
        const message = language === 'tagalog' ? 'Magbigay ng math problem na gusto mong i-solve.' :
                       language === 'bisaya' ? 'Paghatag ug math problem nga gusto nimo i-solve.' :
                       'Please provide a math problem to solve.';
        await sendMessage(senderId, { text: message }, token);
        return;
      }

      if (mathProblem.toLowerCase().includes('group data') || 
          mathProblem.toLowerCase().includes('frequency distribution') ||
          mathProblem.toLowerCase().includes('grouped data')) {
        await this.handleGroupData(senderId, mathProblem, token, language);
        return;
      }

      if (this.isStatisticsProblem(mathProblem)) {
        await this.handleStatistics(senderId, mathProblem, token, language);
        return;
      }
      ...
      try {
        if (mathProblem.includes('=') && !mathProblem.includes('==')) {
          ...
          if (parts.length === 2) {
            ...
            if (simplified.isNode) {
              const expr = simplified.toString();
              if (expr.includes('x') && !expr.includes('^2')) {
                ...
                for (const term of terms) {
                  ...
                  if (termStr.includes('x')) {
                    const coeff = termStr.replace('x', '');
                    a += coeff === '' ? 1 : parseFloat(coeff);
                  } else {
                    b += parseFloat(termStr);
                  }
                }
                if (a !== 0) {
                  const x = -b / a;
                  result = `x = ${x}`;
                  solution = this.generateStepByStep(mathProblem, result);
                } else {
                  const evaluated = math.evaluate(mathProblem);
        ...
        console.log('[Math] math.js failed, trying API:', mathError.message);
        const response = await this.callAPI(`Solve this math problem step by step: ${mathProblem}`);
        const cleaned = this.cleanResponse(response);
        if (cleaned && cleaned.length > 10) {
          await this.sendChunks(senderId, `📐 Math Solution\n\nProblem: ${mathProblem}\n\n${cleaned}`, token);
          return;
        } else {
          throw new Error('Could not solve the math problem');
        }
      }

      const lang = this.getLanguageName(language);
      let finalResponse = `📐 Math Solution\n\nProblem: ${mathProblem}\n\n`;
      ...
      finalResponse += `✅ Solved using math.js\n⏱️ ${new Date().toLocaleString('fil-PH', { timeZone: 'Asia/Manila' })}`;

      await this.sendChunks(senderId, finalResponse, token);

    } catch (error) {
      console.error('[Math] Error:', error.message);
      const errorMsg = language === 'tagalog' ? `Hindi ko ma-solve ang math problem. Subukan muli.` :
                       language === 'bisaya' ? `Dili nako ma-solve ang math problem. Sulayi pag-usab.` :
                       `I couldn't solve the math problem. Please try again.`;
  ...
  // 🆕 GROUP DATA HANDLER
  // ================================================================
  async handleGroupData(senderId, prompt, token, language) {
    try {
      let data = [];
      let className = '';
      
      const numberPattern = /(\d+)(?:\s*,\s*|\s+)(\d+)(?:\s*,\s*|\s+)(\d+)/;
      const match = prompt.match(numberPattern);
      
      if (match) {
        const allNumbers = prompt.match(/\d+/g);
        if (allNumbers && allNumbers.length > 3) {
          data = allNumbers.map(Number);
      ...
      if (data.length === 0) {
        if (prompt.toLowerCase().includes('example') || prompt.toLowerCase().includes('halimbawa')) {
          const sampleData = this.generateSampleGroupData(prompt);
          data = sampleData.data;
          className = sampleData.className;
        } else {
          const msg = language === 'tagalog' ? 'Mangyaring magbigay ng data para sa group data analysis.' :
                      language === 'bisaya' ? 'Palihug paghatag og data para sa group data analysis.' :
                      'Please provide data for group data analysis.';
          await sendMessage(senderId, { text: msg }, token);
          return;
        }
      }

      const result = this.analyzeGroupData(data);
      
      const lang = this.getLanguageName(language);
      let response = `📊 GROUP DATA ANALYSIS\n\n`;
      ...
      response += `• Mean (Average): ${result.mean.toFixed(2)}\n`;
      response += `• Median: ${result.median.toFixed(2)}\n`;
      response += `• Mode: ${result.mode.join(', ')}\n`;
      response += `• Range: ${result.range}\n`;
      response += `• Variance: ${result.variance.toFixed(2)}\n`;
      response += `• Standard Deviation: ${result.stdDev.toFixed(2)}\n\n`;
      
      response += `🔍 STEP-BY-STEP SOLUTION:\n\n`;
      response += this.generateGroupDataSteps(data, result);
      
      await this.sendChunks(senderId, response, token);

    } catch (error) {
      console.error('[Group Data] Error:', error.message);
      const errorMsg = language === 'tagalog' ? 'Error sa pag-analyze ng group data.' :
                       language === 'bisaya' ? 'Error sa pag-analyze sa group data.' :
                       'Error analyzing group data.';
      await sendMessage(senderId, { text: errorMsg }, token);
    }
  },

  generateSampleGroupData(prompt) {
    const lower = prompt.toLowerCase();
    let data = [];
    let className = '';
    
    if (lower.includes('resume') || lower.includes('applicant')) {
      const education = ['High School', 'Bachelor\'s', 'Master\'s', 'PhD'];
      const counts = [15, 45, 25, 10];
      data = [];
      ...
      className = 'Applicants by Education Level';
    } else if (lower.includes('salary') || lower.includes('income')) {
      data = [25000, 30000, 35000, 40000, 45000, 50000, 55000, 60000, 65000, 70000,
              28000, 32000, 38000, 42000, 48000, 52000, 58000, 62000, 68000, 75000];
      className = 'Employee Salaries';
    } else if (lower.includes('score') || lower.includes('exam') || lower.includes('test')) {
      data = [85, 90, 75, 88, 92, 78, 85, 91, 87, 83,
              80, 89, 76, 94, 82, 88, 79, 86, 93, 84];
      className = 'Exam Scores';
    } else if (lower.includes('age')) {
      data = [22, 25, 28, 30, 32, 35, 38, 40, 42, 45,
              24, 27, 29, 31, 33, 36, 39, 41, 43, 46];
      className = 'Age Distribution';
    } else if (lower.includes('grade')) {
      data = [88, 92, 85, 90, 87, 93, 86, 91, 89, 84,
              95, 87, 90, 88, 92, 86, 94, 89, 91, 85];
      className = 'Student Grades';
    } else {
      data = [23, 25, 28, 30, 32, 35, 38, 40, 42, 45,
              24, 27, 29, 31, 33, 36, 39, 41, 43, 46,
              22, 26, 28, 30, 34, 37, 40, 42, 44, 47];
      className = 'Sample Data Distribution';
    }
    
    return { data, className };
  },

  analyzeGroupData(data) {
  ...
  generateGroupDataSteps(data, result) {
    const sorted = [...data].sort((a, b) => a - b);
    let steps = '';
    
    steps += `1. Sort the data:\n   ${sorted.join(', ')}\n\n`;
    steps += `2. Count (n) = ${result.n}\n\n`;
    steps += `3. Sum = ${data.join(' + ')} = ${result.sum}\n\n`;
    steps += `4. Mean = Sum / n = ${result.sum} / ${result.n} = ${result.mean.toFixed(2)}\n\n`;
    
    if (result.n % 2 === 0) {
      steps += `5. Median = (${sorted[result.n/2 - 1]} + ${sorted[result.n/2]}) / 2 = ${result.median.toFixed(2)}\n\n`;
    } else {
      steps += `5. Median = ${sorted[Math.floor(result.n/2)]} (middle value)\n\n`;
    }
    ...
    const freqStr = Object.entries(frequency).map(([num, freq]) => `${num}: ${freq}`).join(', ');
    steps += `6. Frequency: ${freqStr}\n   Mode(s): ${result.mode.join(', ')}\n\n`;
    
    steps += `7. Range = ${sorted[sorted.length-1]} - ${sorted[0]} = ${result.range}\n\n`;
    
    const deviations = sorted.map(num => (num - result.mean).toFixed(2));
    const squaredDeviations = sorted.map(num => Math.pow(num - result.mean, 2).toFixed(2));
    steps += `8. Variance = Σ(x - mean)² / n\n`;
    steps += `   Deviations: ${deviations.join(', ')}\n`;
    steps += `   Squared: ${squaredDeviations.join(', ')}\n`;
    steps += `   Sum of squares = ${squaredDeviations.reduce((a, b) => a + parseFloat(b), 0).toFixed(2)}\n`;
    steps += `   Variance = ${result.variance.toFixed(2)}\n\n`;
    
    steps += `9. Standard Deviation = √Variance = √${result.variance.toFixed(2)} = ${result.stdDev.toFixed(2)}\n\n`;
  ...
  // 🆕 STATISTICS HANDLER
  // ================================================================
  isStatisticsProblem(prompt) {
    const lower = prompt.toLowerCase();
    const statsTerms = ['mean', 'median', 'mode', 'variance', 'standard deviation', 
                       'sd', 'σ', 'sigma', 'range', 'quartile', 'percentile',
                       'correlation', 'regression', 'probability', 'distribution'];
    return statsTerms.some(term => lower.includes(term));
  },

  async handleStatistics(senderId, prompt, token, language) {
    try {
      const numbers = prompt.match(/\d+/g);
      if (!numbers || numbers.length < 2) {
        const msg = language === 'tagalog' ? 'Mangyaring magbigay ng data para sa statistics analysis.' :
                    language === 'bisaya' ? 'Palihug paghatag og data para sa statistics analysis.' :
                    'Please provide data for statistics analysis.';
        await sendMessage(senderId, { text: msg }, token);
        return;
      }
      ...
      const lang = this.getLanguageName(language);
      let response = `📊 STATISTICS ANALYSIS\n\n`;
      response += `Data: ${data.join(', ')}\n\n`;
      response += `📈 Results:\n`;
      response += `• Count: ${result.n}\n`;
      response += `• Sum: ${result.sum}\n`;
      response += `• Mean: ${result.mean.toFixed(4)}\n`;
      response += `• Median: ${result.median.toFixed(4)}\n`;
      response += `• Mode: ${result.mode.join(', ')}\n`;
      response += `• Range: ${result.range}\n`;
      response += `• Variance: ${result.variance.toFixed(4)}\n`;
      response += `• Standard Deviation: ${result.stdDev.toFixed(4)}\n\n`;
      ...
      const errorMsg = language === 'tagalog' ? 'Error sa statistics analysis.' :
                       language === 'bisaya' ? 'Error sa statistics analysis.' :
                       'Error in statistics analysis.';
      await sendMessage(senderId, { text: errorMsg }, token);
    }
  },
  ...
  // ================================================================
  generateStepByStep(problem, result) {
    let steps = '';
    
    if (problem.includes('+')) {
      steps = `Step 1: Identify the operation: Addition\n`;
      steps += `Step 2: Add the numbers\n`;
      steps += `Step 3: ${problem} = ${result.replace('Result: ', '')}\n`;
    } else if (problem.includes('-')) {
      steps = `Step 1: Identify the operation: Subtraction\n`;
      steps += `Step 2: Subtract the numbers\n`;
      steps += `Step 3: ${problem} = ${result.replace('Result: ', '')}\n`;
    } else if (problem.includes('*')) {
      steps = `Step 1: Identify the operation: Multiplication\n`;
      steps += `Step 2: Multiply the numbers\n`;
      steps += `Step 3: ${problem} = ${result.replace('Result: ', '')}\n`;
    } else if (problem.includes('/')) {
      steps = `Step 1: Identify the operation: Division\n`;
      steps += `Step 2: Divide the numbers\n`;
      steps += `Step 3: ${problem} = ${result.replace('Result: ', '')}\n`;
    } else {
      steps = `Step 1: Parse the expression\n`;
      steps += `Step 2: Apply the appropriate operations\n`;
      steps += `Step 3: ${problem} = ${result.replace('Result: ', '')}\n`;
    }

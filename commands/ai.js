...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // Check for lyrics request FIRST
      ...
      // Check for image generation - ONLY if explicit command
      if (this.isExplicitImageGeneration(prompt)) {
      ...
      // Check for image attachments - ONLY if actually an image
      ...
      if (!prompt && !isReply && !imageUrl) {
        await sendMessage(senderId, {
          text: '🤖 Teacher Arlene - Multi-Modal AI\n\n📚 Capabilities:\n• Text conversations & analysis\n• Image analysis (send photo)\n• Academic research (gscholar)\n• Image generation (generate/search)\n• Music search (play)\n• Lyrics search (lyrics)\n• Real-time info\n• Translation & summarization\n\n📝 Commands:\n• ai [question] - Ask anything\n• Send image for analysis\n• generate [search term] - Get images\n• gscholar [query] - Academic papers\n• play [song] - Music search\n• lyrics [song] - Song lyrics'
        }, token);
        return;
      }
      ...
      // Check if wants detailed answer
      const wantsDetailed = this.wantsDetailedAnswer(prompt);
      let aiResponse = '';

      // Process with Gemini if image
      if (imageUrl) {
      ...
      // Shorten if not detailed and no image
  ...
  isExplicitImageGeneration(prompt) {
    const lower = prompt.toLowerCase().trim();
    
    // Check if it's a question - if yes, don't generate images
    if (this.isQuestion(prompt)) {
      return false;
    }

    // Only generate images for explicit commands
    ...
    // Check if prompt starts with any explicit pattern
    ...
    // Also check for common image request phrases (but NOT as questions)
    const imagePhrases = [
      'show me', 'give me', 'picture of', 'image of', 'photo of',
      'need a picture', 'want a picture', 'need image', 'want image',
      'can i see', 'let me see', 'find image', 'get image', 'search image',
      'maghanap ng', 'gusto ko', 'patingin ng', 'ano itsura',
      'larawan ng', 'litrato ng', 'imahe ng'
    ];

    // Only trigger if the phrase is at the beginning or has clear intent
    for (const phrase of imagePhrases) {
      if (lower.includes(phrase)) {
        // Make sure it's not a question about something else
        const beforePhrase = lower.split(phrase)[0].trim();
        if (beforePhrase === '' || this.isGreeting(beforePhrase)) {
          return true;
        }
      }
    }
  ...
  isQuestion(text) {
    ...
    const questionWords = [
      ...
      'is', 'are', 'does', 'do', 'can', 'could', 'would', 'will',
      'should', 'may', 'might', 'has', 'have', 'did', 'was', 'were'
    ];
  ...
  // Helper: Check if text is a greeting
  isGreeting(text) {
    const greetings = ['hi', 'hello', 'hey', 'kamusta', 'musta', 'oy', 'hoy', 'good', 'morning', 'afternoon', 'evening'];
  ...
  isMusicRequest(prompt) {
    const lower = prompt.toLowerCase();
    const keywords = ['play', 'song', 'music', 'track', 'audio', 'listen', 'sound', 'kanta', 'tugtog', 'music video', 'mv', 'soundtrack', 'playlist', 'album', 'single', 'remix', 'cover', 'official audio', 'official music', 'stream', 'pakinggan', 'patugtog', 'music link', 'song link', 'hit song', 'popular song', 'new song', 'latest song', 'opm', 'pinoy music', 'tagalog song', 'bisaya song'];
    return keywords.some(k => lower.includes(k));
  },
  ...
  async handleImageGeneration(senderId, prompt, token) {
    ...
    const commands = ['generate', 'image', 'img', 'show', 'search'];
  ...
  isResearchQuery(prompt) {
    const lower = prompt.toLowerCase();
    const keywords = ['research', 'study', 'studies', 'paper', 'papers', 'article', 'articles', 'journal', 'journals', 'thesis', 'dissertation', 'literature', 'review', 'meta-analysis', 'systematic review', 'citation', 'citations', 'academic', 'scholar', 'scholarly', 'peer-reviewed'];

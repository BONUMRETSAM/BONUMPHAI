...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // ================================================================
      // RESUME HANDLING - Fixed: Questions go to AI, Generation shows sample
      // ================================================================
      if (this.isResumeRequest(prompt)) {
        // This is a generation request - show the sample resume
        const response = this.getSampleResume();
        await this.sendChunks(senderId, response, token);
        return;
      }

      // If it's a resume question (definition, purpose, etc.), it will fall through to AI
      ...
      if (this.isLetterRequest(prompt)) {
        const response = `❌ LETTER GENERATION FEATURE REMOVED

The letter/speech generation feature has been removed from this bot.
...
Please use other tools or services.
...
Thank you! 🙏`;
  ...
  // ================================================================
  // RESUME DETECTION - Questions go to AI, Generation shows sample
  // ================================================================
  isResumeRequest(prompt) {
    if (!prompt) return false;
    const lower = prompt.toLowerCase();
    
    // GENERATION keywords - these will return the SAMPLE RESUME
    const generationKeywords = [
      'gumawa ng resume', 'gumawa ng cv', 'create resume', 'make resume', 
      'generate resume', 'resume generator', 'cv generator',
      'gumawa ng resume ko', 'create my resume', 'make my resume',
      'resume for', 'cv for', 'resume template', 'cv template',
      'resume sample', 'cv sample', 'sample resume', 'sample cv',
      'resume format', 'cv format', 'best resume', 'professional resume',
      'gawan mo ng resume', 'gumawa ka ng resume', 'magresume',
      'resume maker', 'cv maker', 'build resume', 'build cv',
      'gumawa ako ng resume', 'gumawa nang resume', 'gumawa ng resume ko',
      'resume ko', 'cv ko', 'gumawa ng cv ko', 'create cv',
      'paano gumawa ng resume', 'how to make a resume', 'how to create a resume',
      'pano gumawa ng resume', 'how to write a resume', 'resume writing',
      'gumawa ng resume para sa', 'resume pang', 'resume para sa'
    ];
    
    // QUESTION keywords - these will go to AI/API
    const questionKeywords = [
      'ano ang resume', 'what is a resume', 'what is resume',
      'ano ang cv', 'what is cv', 'what is curriculum vitae',
      'ano ang purpose ng resume', 'what is the purpose of a resume',
      'resume meaning', 'cv meaning', 'curriculum vitae meaning',
      'para saan ang resume', 'bakit kailangan ng resume',
      'resume definition', 'cv definition',
      'ano ang laman ng resume', 'what to put in a resume',
      'resume tips', 'cv tips',
      'kailan kailangan ang resume', 'when do you need a resume',
      'saan ginagamit ang resume', 'where is resume used',
      'sino ang nangangailangan ng resume', 'who needs a resume',
      'ano ang pagkakaiba ng resume at cv', 'resume vs cv',
      'resume purpose', 'purpose of resume', 'importance of resume'
    ];
    
    const isGeneration = generationKeywords.some(k => lower.includes(k));
    const isQuestion = questionKeywords.some(k => lower.includes(k));
    
    // If it's a question about resume, let AI handle it
    if (isQuestion) return false;
    
    // If it's a generation request, return true to show sample resume
    if (isGeneration) return true;
    
    // If it has "resume" or "cv" alone
    const hasResumeWord = lower.includes('resume') || lower.includes('cv') || lower.includes('curriculum vitae');
    
    if (hasResumeWord) {
      const trimmed = lower.trim();
      if (trimmed === 'resume' || trimmed === 'cv' || trimmed === 'curriculum vitae') {
        return false; // Let AI handle - it's just asking what resume is
      }
      
      // Check if it's asking a question
      const questionWords = ['ano', 'what', 'meaning', 'definition', 'purpose', 'bakit', 'why', 'para saan', 'for what', 'pano', 'how to', 'paano', 'tips', 'guide', 'example', 'halimbawa', 'anu', 'ano ba', 'what is', 'what are', 'what does', 'when', 'kailan', 'where', 'saan', 'who', 'sino', 'benefits', 'benefit', 'advantage', 'importance', 'mahalaga', 'difference', 'pagkakaiba', 'gumagana', 'function', 'gamit'];
  ...
  // ================================================================
  // SAMPLE RESUME - EXACTLY FROM THE SCREENSHOT
  // ================================================================
  getSampleResume() {
    return `
================================================================================
...
📝 TANDAAN:
Ito ay isang SAMPLE RESUME lamang.
Gamitin ito bilang gabay sa paggawa ng iyong sariling resume.
Palitan ang mga impormasyon ayon sa iyong sariling:
- Personal na detalye (pangalan, kapanganakan, etc.)
- Karanasan sa trabaho
- Edukasyon
- Kasanayan
- Mga sanggunian

Maaari mong baguhin ang format at nilalaman base sa iyong pangangailangan.
`;
  },

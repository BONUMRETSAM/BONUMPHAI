...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      if (this.isReturnToTopicRequest(prompt)) {
        ...
        // Fallback: if no topic match but user mentions "last" or "previous"
      ...
      // ================================================================
      // 🆕 IMAGE ANALYSIS with keyword extraction
      // ================================================================
      if (imageUrl) {
        ...
        // Also index by each significant keyword
      ...
      // ================================================================
        ...
        newHistory.language = responseLanguage || detectedLanguage;
  ...
  findBestTopicMatch(prompt, history) {
    ...
    for (const [key, data] of Object.entries(history.topicHistory)) {
      ...
      // 1. Exact keyword match in response
      ...
      // 2. User words appear in the response
      ...
      // 3. Topic key appears in user prompt
      if (userLower.includes(key.toLowerCase())) {
        score += 10;
      }

      // 4. Original prompt appears in user prompt
      if (data.prompt && userLower.includes(data.prompt.toLowerCase())) {
        score += 8;
      }

      // 5. Special words: "activity", "worksheet", "sheet", "quiz", "assignment"
      ...
      // 6. Recency bonus (within 10 minutes)
      if (data.timestamp && (Date.now() - data.timestamp) < 600000) {
        score += 5;
      }

      // 7. "last" / "previous" reference → bonus to the most recent
  ...
  extractKeywordsFromResponse(response) {
    ...
    // Common topic words (Filipino/English)
    const topicWords = [
      ...
      'kasaysayan', 'history', 'lapu-lapu', 'magellan',
      ...
      'answer', 'explanation', 'solution', 'summary', 'conclusion',
      'meme', 'joke', 'humor', 'funny', 'comedy',
      'photo', 'picture', 'image', 'screenshot', 'screen capture',
      'portrait', 'selfie', 'landscape', 'scenery', 'nature',
      'city', 'building', 'architecture', 'art', 'drawing',
    ...
    // Extract noun phrases (2‑3 words) that are not stop‑words
  ...
  // ================================================================
  // 🆕 IMPROVED: RETURN‑TO‑TOPIC DETECTION (more flexible)
  // ================================================================
  isReturnToTopicRequest(prompt) {
    ...
    // References to "last response", "previous reply", etc.
    const refs = [
  ...
  // ================================================================
  // 🆕 IMPROVED: EXTRACT TOPIC (with __LAST_RESPONSE__ flag)
  // ================================================================
  extractTopicFromReturn(prompt) {
    if (!prompt) return null;
    const lower = prompt.toLowerCase();

    // If user explicitly says "last response" etc., return flag
    const refs = [
      'last response', 'last reply', 'last answer', 'last message',
      'previous response', 'previous reply', 'previous answer',
      'nauna mong sagot', 'nauna mong reply', 'huling sagot', 'huling reply',
    ...
    // Fallback: extract any keywords as topic
  ...
  // ================================================================
  // GEMINI (with clean Answer/Explanation format)
  // ================================================================
...
USER QUESTION: ${userPrompt || 'Analyze this image'}`;
  ...
  // ================================================================
  // NEW TOPIC / FOLLOW‑UP / CONTEXTUAL
  // ================================================================
  ...
  // ================================================================
  // 🆕 IMPROVED: FINAL PROMPT BUILDER (fix for "more explanation")
  // ================================================================
  buildFinalPrompt(prompt, previousResponse, previousPrompt, isReply, wantsDetailed, language = 'english') {
    ...
    if (previousResponse) {
      // 🔥 Force the AI to use the previous context
      ...
      // 🔥 More Explanation / Elaborate
      if (lower.includes('more explanation') || lower.includes('explain more') || 
          lower.includes('elaborate') || lower.includes('paki elaborate') ||
        ...
        final += `⚠️ IMPORTANT: The user wants you to ELABORATE on the PREVIOUS RESPONSE above.\n`;
        final += `- DO NOT give a generic explanation about "how to explain".\n`;
      ...
      if (lower.includes('example') || lower.includes('sample') || 
        ...
        final += `⚠️ IMPORTANT: The user wants EXAMPLES related to the PREVIOUS RESPONSE above.\n`;
      ...
      // 🔥 Scenario
      if (lower.includes('scenario') || lower.includes('situation') || 
          lower.includes('case') || lower.includes('situwasyon') ||
          lower.includes('pangyayari') || lower.includes('senaryo')) {
        
        final += `⚠️ IMPORTANT: The user wants a SCENARIO or SITUATION based on the PREVIOUS RESPONSE above.\n`;
  ...
  async handleUserInfo(senderId, prompt, token) {
    try {
      ...
      if (!response) {
        ...
        response = publicInfo.length > 0 ? `Narito ang iyong pampublikong impormasyon:\n${publicInfo.join('\n')}` : 'Hindi ko masasabi iyan dahil ito ay kompidensyal.';
      }

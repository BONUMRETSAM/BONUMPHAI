...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // ============================================
      // STEP 1: CHECK IF IMAGE URL IS IN EVENT
      // ============================================
      if (event && event._scanImageUrl) {
        imageUrl = event._scanImageUrl;
        console.log('[scan] Image URL found from event _scanImageUrl');
      }
      ...
      if (!imageUrl && event?.message?.attachments) {
        for (const attachment of event.message.attachments) {
          if (attachment.type === 'image' || attachment.type === 'photo') {
            ...
            if (imageUrl) {
              console.log('[scan] Image URL found from attachment');
            }
            break;
          }
        }
      ...
      if (!imageUrl && event?.message?.reply_to?.mid) {
        console.log('[scan] Reply detected');
        try {
          ...
          if (replyData && replyData.imageUrl) {
            ...
            console.log('[scan] Image URL found from reply');
          }
        } catch (error) {
      ...
      if (!imageUrl) {
        ...
        if (cachedImage) {
          ...
          console.log('[scan] Image URL found from cache');
        }
      }
...
3. Include Philippine local names (Tagalog, Bisaya, Ilocano) - VERY IMPORTANT
4. Be SPECIFIC and ACCURATE with scientific names
...
DISTRIBUTION:
[Geographic distribution - countries, regions, provinces]

ECOLOGICAL ROLE:
[Role in ecosystem, interactions with other species]

USES/BENEFITS:
[Economic uses, medicinal uses, cultural importance, practical applications]

CONSERVATION STATUS:
[IUCN status: Least Concern, Vulnerable, Endangered, Critically Endangered, Data Deficient, Not Evaluated]

DAO GROUP:
[DENR Administrative Order classification if applicable]
- DAO 2017-11: [Critically Endangered, Endangered, Vulnerable, Other Threatened Species, Not Listed]
- If not applicable: Not listed

ADDITIONAL INFO:
[Interesting facts, cultural significance, traditional uses, unique characteristics]

IMPORTANT: BE SPECIFIC AND ACCURATE. Use Philippine context when applicable. NO emojis, NO special characters, NO markdown.`;
  },

  // ============================================
  // PARSE RESPONSE (COMPLETE)
  // ============================================
  ...
  // ============================================
  // FORMAT RESPONSE (COMPLETE & CLEAN)
  // ============================================
  formatResponse(data) {

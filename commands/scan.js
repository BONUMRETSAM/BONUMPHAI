...
module.exports = {
    ...
    try {
      ...
      if (event?.message?.reply_to?.mid) {
        ...
        try {
          ...
          console.log('[scan] Reply data received');
      ...
      // ============================================
      // STEP 2: CHECK ATTACHMENTS IN CURRENT MESSAGE
      // ============================================
      ...
      // ============================================
      // STEP 3: CHECK IMAGE CACHE (if user sent image earlier)
      // ============================================
      if (!imageUrl) {
        // Check if we have a cached image from this user
        const imageCache = this.getImageCache(senderId);
        if (imageCache) {
          imageUrl = imageCache;
      ...
      // ============================================
      // STEP 4: IF NO IMAGE, CHECK TEXT QUERY
      // ============================================
      if (!imageUrl && prompt) {
  ...
  async getRepliedMessageData(mid, token) {
    try {
      ...
      console.log('[getRepliedMessageData] Response received');
      ...
      if (data?.attachments?.data) {
        for (const attachment of data.attachments.data) {
          if (attachment.type === 'image' || attachment.type === 'photo') {
            ...
            if (imageUrl) {
              console.log('[getRepliedMessageData] Found image URL');
              break;
            }
          }
        }
  ...
  // ============================================
  // IMAGE CACHE (fallback for images sent earlier)
  // ============================================
  getImageCache(senderId) {
    const cache = global.imageCache || {};
    const entry = cache[senderId];
    if (entry) {
      const now = Date.now();

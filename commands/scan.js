...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      console.log('[scan] Event received:', JSON.stringify(event, null, 2));

      // ============================================
      // STEP 1: CHECK IF REPLYING TO A MESSAGE
      // ============================================
      if (event?.message?.reply_to?.mid) {
        console.log('[scan] Reply detected, mid:', event.message.reply_to.mid);
        
        try {
          const replyData = await this.getRepliedMessageData(event.message.reply_to.mid, token);
          console.log('[scan] Reply data:', JSON.stringify(replyData, null, 2));
          
          if (replyData && replyData.imageUrl) {
            imageUrl = replyData.imageUrl;
            isImageReply = true;
            console.log('[scan] Image URL found from reply:', imageUrl);
          } else {
            console.log('[scan] No image found in replied message');
          }
        } catch (error) {
      ...
      // ============================================
      // STEP 2: CHECK IF IMAGE IS IN THE CURRENT MESSAGE
      // ============================================
      if (!imageUrl && event?.message?.attachments) {
        console.log('[scan] Checking attachments...');
        for (const attachment of event.message.attachments) {
          if (attachment.type === 'image' || attachment.type === 'photo') {
            imageUrl = attachment.payload?.url || attachment.url || null;
            if (imageUrl) {
              console.log('[scan] Image URL found from attachment:', imageUrl);
            }
            break;
          }
        }
      ...
      // ============================================
      // STEP 3: IF NO IMAGE, CHECK TEXT QUERY
      // ============================================
      if (!imageUrl && prompt) {
  ...
  // ============================================
  // GET REPLIED MESSAGE DATA (FIXED)
  // ============================================
  async getRepliedMessageData(mid, token) {
    try {
      ...
      console.log('[getRepliedMessageData] Fetching:', url);
      console.log('[getRepliedMessageData] Params:', JSON.stringify(params, null, 2));
      
      const response = await axios.get(url, { params });
      const data = response.data;
      
      console.log('[getRepliedMessageData] Response:', JSON.stringify(data, null, 2));
      ...
      if (data?.attachments?.data) {
        for (const attachment of data.attachments.data) {
          console.log('[getRepliedMessageData] Attachment type:', attachment.type);
          console.log('[getRepliedMessageData] Attachment data:', JSON.stringify(attachment, null, 2));
          ...
          if (attachment.type === 'image' || attachment.type === 'photo') {
            ...
            if (imageUrl) {
              console.log('[getRepliedMessageData] Found image URL:', imageUrl);
              break;
            }
          }
        }
      ...
      // If still no image, try to get from message text (if it contains a URL)
      if (!imageUrl && data?.message) {
        const urlMatch = data.message.match(/https?:\/\/[^\s]+\.(?:jpg|jpeg|png|gif|webp)/i);
        if (urlMatch) {
          imageUrl = urlMatch[0];
          console.log('[getRepliedMessageData] Found image URL in message text:', imageUrl);
        }
      }
      ...
      console.error('[getRepliedMessageData] Response:', error.response?.data);
  ...
  async identifyWithGemini(imageUrl) {
    try {
      ...
      console.log('[identifyWithGemini] Calling Gemini API...');
      ...
      console.log('[identifyWithGemini] Gemini response received');

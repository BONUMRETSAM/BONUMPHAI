...
module.exports = {
  ...
  async execute(senderId, args, token, event) {
    try {
      ...
      // ============================================
      // STEP 1: CHECK IF IMAGE URL IS IN EVENT (from handleMessage)
      // ============================================
      ...
      if (!imageUrl && event?.message?.attachments) {
        console.log('[scan] Checking current message attachments...');
        for (const attachment of event.message.attachments) {
          if (attachment.type === 'image' || attachment.type === 'photo') {
            imageUrl = attachment.payload?.url || attachment.url || null;
            if (imageUrl) {
              console.log('[scan] Image URL found from current message attachment');
            }
            break;
          }
        }
      ...
      // ============================================
      // STEP 3: CHECK IF REPLYING TO A MESSAGE
      // ============================================
      ...
      // ============================================
      // STEP 5: IF NO IMAGE, CHECK TEXT QUERY
      // ============================================
      if (!imageUrl && prompt) {
  ...
  async getRepliedMessageData(mid, token) {
    try {
      ...
      console.log('[getRepliedMessageData] Fetching message:', mid);
  ...
  async getCachedImage(senderId, token) {
    try {
      ...
      console.log('[getCachedImage] Fetching recent messages...');
            ...
            for (const attachment of msg.attachments.data) {
              if (attachment.type === 'image' || attachment.type === 'photo') {
                const imageUrl = attachment?.image_data?.url || 
                                ...
                                null;
                if (imageUrl) {
                  console.log('[getCachedImage] Found cached image');
                  return imageUrl;
                }
              }
            }
